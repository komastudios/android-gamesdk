...
GLPixelBuffer::GLPixelBuffer() {
    ...
    if (_width == 0 || _height == 0) {
        FatalError(TAG, "cannot create GLPixelBuffer with width or height of 0."
...
unsigned int GLPixelBuffer::Width() const {
    return _width;
}

unsigned int GLPixelBuffer::Height() const {
    return _height;
}

unsigned int GLPixelBuffer::Components() const {
    return _components;
}
...
void GLPixelBuffer::CopyFromFramebuffer() {
    ...
    if (width != _width || height != _height) {
        FatalError(TAG, "current viewport dimensions to not match pixel buffer "
                        "dimensions");
    }
