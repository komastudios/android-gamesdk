...
Result Swapchain::Initialize(Vulkan &_vulkan, VkImageUsageFlags usage_flags) {
  ...
  if (vk->surface_capabilities.currentExtent.width == 0xFFFFFFFF ||
    ...
    extent.width =
        std::max(extent.width, vk->surface_capabilities.minImageExtent.width);
    extent.width =
        std::min(extent.width, vk->surface_capabilities.maxImageExtent.width);

    extent.height = vk->window_height;
    extent.height =
        std::max(extent.height, vk->surface_capabilities.minImageExtent.height);
    extent.height =
        std::min(extent.height, vk->surface_capabilities.maxImageExtent.height);
  } else
  ...
  uint32_t qfi[2] = {vk->graphics.family_index, vk->present.family_index};
  ...
  VK_RETURN_FAIL(
      vk->createSwapchainKHR(vk->device, &create_info, nullptr, &swapchain));

  uint32_t count = 0;
  VK_RETURN_FAIL(
      vk->getSwapchainImagesKHR(vk->device, swapchain, &count, nullptr));
  ...
  for (auto &image : images) {
    ...
    VK_RETURN_FAIL(vk->createImageView(vk->device, &view_create_info, nullptr,
                                       &image_view));
  ...
  if (vk->use_swappy) {
    jni::SafeJNICall([&](jni::LocalJNIEnv *env) {
      ...
      SwappyVk_initAndGetRefreshCycleDuration(
          jni_env, activity, vk->physical_device, vk->device, swapchain,
...
Result Swapchain::Present(SwapchainImage &image, VkSemaphore render_semaphore) {
  auto &vk = vulkan.vk;
