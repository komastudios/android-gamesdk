...
VkResult init_global_layer_properties(struct vulkan_info &info) {
...
VkResult init_device_extension_properties(struct vulkan_info &info, layer_properties &layer_props) {
...
VkResult init_instance(struct vulkan_info &info, char const *const app_short_name) {
...
VkResult init_enumerate_device(struct vulkan_info &info, uint32_t gpu_count) {
...
VkResult init_debug_report_callback(struct vulkan_info &info, PFN_vkDebugReportCallbackEXT dbgFunc) {
    VkResult res;
...
void init_window_size(struct vulkan_info &info, int32_t default_width, int32_t default_height) {
...
void execute_queue_cmdbuf(struct vulkan_info &info, const VkCommandBuffer *cmd_bufs, VkFence &fence) {
    VkResult U_ASSERT_ONLY res;
...
void init_swap_chain(struct vulkan_info &info, VkImageUsageFlags usageFlags) {
...
void init_renderpass(struct vulkan_info &info, bool include_depth, bool clear, VkImageLayout finalLayout) {
...
void init_vertex_buffer(struct vulkan_info &info, const void *vertexData, uint32_t dataSize, uint32_t dataStride,
                        bool use_texture) {
    VkResult U_ASSERT_ONLY res;
    bool U_ASSERT_ONLY pass;
...
void init_shaders(struct vulkan_info &info, const char *vertShaderText, const char *fragShaderText) {
    VkResult U_ASSERT_ONLY res;
...
void init_pipeline(struct vulkan_info &info, VkBool32 include_depth, VkBool32 include_vi) {
    VkResult U_ASSERT_ONLY res;
...
void init_image(struct vulkan_info &info, texture_object &texObj, const char *textureName, VkImageUsageFlags extraUsages,
...
void init_texture(struct vulkan_info &info, const char *textureName, VkImageUsageFlags extraUsages,
                  VkFormatFeatureFlags extraFeatures) {
...
void init_submit_info(struct vulkan_info &info, VkSubmitInfo &submit_info, VkPipelineStageFlags &pipe_stage_flags) {
...
void init_clear_color_and_depth(struct vulkan_info &info, VkClearValue *clear_values) {
...
void init_render_pass_begin_info(struct vulkan_info &info, VkRenderPassBeginInfo &rp_begin) {
...
void destroy_pipeline(struct vulkan_info &info) { vkDestroyPipeline(info.device, info.pipeline, NULL); }

void destroy_pipeline_cache(struct vulkan_info &info) { vkDestroyPipelineCache(info.device, info.pipelineCache, NULL); }
...
void destroy_descriptor_pool(struct vulkan_info &info) { vkDestroyDescriptorPool(info.device, info.desc_pool, NULL); }
...
void destroy_command_pool(struct vulkan_info &info) { vkDestroyCommandPool(info.device, info.cmd_pool, NULL); }
...
void destroy_renderpass(struct vulkan_info &info) { vkDestroyRenderPass(info.device, info.render_pass, NULL); }
...
void destroy_instance(struct vulkan_info &info) { vkDestroyInstance(info.inst, NULL); }
