...
#pragma ide diagnostic ignored "cert-err58-cpp" // static storage warning
...
namespace ancer::reporting {
    ...
    namespace {
        ...
        private:
            ...
            WorkerThread<state> _worker = {"ReportSerializer_Immediate", samples::Affinity::Odd};
        };
        ...
        private:
            ...
            WorkerThread<state> _worker = {"ReportSerializer_Manual", samples::Affinity::Odd};
        };
        ...
        public:
            ...
            ~ReportSerializer_Periodic() override {
                Log::I(
                        TAG,
                        "ReportSerializer_Periodic::dtor - _numWrites: %d - stopping write thread...",
                        _num_writes);
        ...
        Duration _periodic_report_serializer_flush_period = DEFAULT_FLUSH_PERIOD_MILLIS;
    }
    ...
    void OpenReportLog(const std::string& file) {
        if ( _ofs.is_open()) {
            FatalError(
                    TAG,
                    "Called OpenReportLog on an already open report; please call CloseReportLog() first");
        }
    ...
    void SetPeriodicFlushModePeriod(Duration duration) {
        ...
        case ReportFlushMode::Periodic:
            dynamic_cast<ReportSerializer_Periodic*>(_report_serializer.get())->SetFlushPeriod(
    ...
    void WriteToReportLog(const reporting::Datum& d) {
        if ( !_report_serializer ) {
            FatalError(TAG, "Attempt to write to report log without opening log first.");
        }
    ...
    void FlushReportLogQueue() {
        if ( !_report_serializer ) {
            FatalError(TAG, "Attempt to flush report log without opening log first.");
        }
        _report_serializer->Flush();
    }
}
