...
VkImage ImageResource::ImageHandle() const { return _image.Handle(); }
...
ImageResource::ImageResource(const Image &image) : _image(image) {
  ...
  _aspects = VK_IMAGE_ASPECT_COLOR_BIT;  // TODO(sarahburns@google.com): pick
                                         // based on format
...
Builder &Builder::Linear(VkFormat format, bool src, bool dst) {
  _create_info.format = format;
  _create_info.tiling = VK_IMAGE_TILING_LINEAR;
  _create_info.samples = VK_SAMPLE_COUNT_1_BIT;
  _create_info.arrayLayers = 1;
  _create_info.mipLevels = 1;
  _create_info.usage = 0;
  if (src) {
    _create_info.usage |= VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
  }
  if (dst) {
    _create_info.usage |= VK_IMAGE_USAGE_TRANSFER_DST_BIT;
  }
  return *this;
}
...
#undef ADD_QUEUE
  ...
  VK_RETURN_FAIL(
      vk->createImage(vk->device, &_create_info, nullptr, &_image._image));
  ...
  VK_GOTO_FAIL(
      res = vk.AllocateMemory(requirements, memory_flags, _image._memory));
...
#endif
  {
    ...
    Context::ChangeImageLayout(vk, buf, _image.Resource(),
                               VK_IMAGE_LAYOUT_UNDEFINED);
    VK_GOTO_FAIL(vk.ReleaseTemporaryCommandBuffer(buf));
  }

  if ((_create_info.usage &
    ...
    VkImageViewCreateInfo iv_create_info = {
        ...
        {/* aspectMask     */ VK_IMAGE_ASPECT_COLOR_BIT,
         /* baseMipLevel   */ 0,
         /* levelCount     */ _create_info.mipLevels,
         /* baseArrayLayer */ 0,
         /* layerCount     */ _create_info.arrayLayers}};

    VK_GOTO_FAIL(res = vk->createImageView(vk->device, &iv_create_info, nullptr,
                                           &_image._view));
  }
  ...
  fail:
    ...
    if (_image._image != VK_NULL_HANDLE) vk.Destroy(_image._image, true);
    return res;
  }
...
image::Builder Image::Builder() { return image::Builder(*this); }

Result Image::Initialize(Vulkan &vk, EResourceUse ruse, VkImageUsageFlags usage,
                         uint32_t width, uint32_t height, uint32_t depth,
                         uint32_t levels, uint32_t layers, VkFormat format,
