...
bool memory_type_from_properties(struct vulkan_info &info, uint32_t typeBits, VkFlags requirements_mask, uint32_t *typeIndex) {
...
void set_image_layout(struct vulkan_info &info, VkImage image, VkImageAspectFlags aspectMask, VkImageLayout old_image_layout,
...
void process_command_line_args(struct vulkan_info &info, int argc, char *argv[]) {
    int i, n;
