...
namespace ancer::vulkan {
    // Number of descriptor sets needs to be the same at alloc, pipeline layout
    // creation, and descriptor set layout creation.
    constexpr size_t kNumDescriptorSets = 1;
    // Number of samples needs to be the same at image creation, renderpass
    // creation, and pipeline creation.
    constexpr VkSampleCountFlagBits kNumSamples = VK_SAMPLE_COUNT_1_BIT;
    ...
    constexpr int kNumViewports = 1;
    constexpr int kNumScissors = kNumViewports;
    // Amount of time, in nanoseconds, to wait for a command buffer to complete.
    constexpr int kFenceTimeout = 100000000;
    ...
    void ProcessCommandLineArgs(VulkanInfo &info, int argc,
                                char **argv);

    bool MemoryTypeFromProperties(VulkanInfo &info, uint32_t typeBits,
                                  VkFlags requirementsMask,
                                  uint32_t *typeIndex);

    void SetImageLayout(VulkanInfo &demo, VkImage image,
    ...
    void WritePpm(VulkanInfo &info, const char *basename);
