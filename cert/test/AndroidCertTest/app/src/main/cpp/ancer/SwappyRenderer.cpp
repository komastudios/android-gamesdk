...
std::unique_ptr<SwappyRenderer> SwappyRenderer::Create(
        const GLContextConfig &preferred_ctx_config,
        const GLContextConfig &fallback_ctx_config) {
    return std::make_unique<SwappyRenderer>(ConstructorTag{},
                                            preferred_ctx_config,
                                            fallback_ctx_config);
}

void SwappyRenderer::SetWindow(ANativeWindow* window, int32_t width, int32_t height) {
    _worker_thread.Run(
            [this, window, width, height](ThreadState* thread_state) {
...

                {
                    ...
                    for (const auto &op : _operations) {
                        op->OnGlContextReady(
                                thread_state->using_gl_context_config);
...
void SwappyRenderer::RequestDraw() {
    _worker_thread.Run(
            [this](ThreadState* thread_state) {
...
void SwappyRenderer::AddOperation(BaseOperation& operation) {
    std::lock_guard<std::mutex> lockGuard(_operations_lock);
    _operations.push_back(&operation);
}

void SwappyRenderer::RemoveOperation(BaseOperation& operation) {
    std::lock_guard<std::mutex> lockGuard(_operations_lock);
    _operations.erase(
...
void SwappyRenderer::ClearOperations() {
    std::lock_guard<std::mutex> lockGuard(_operations_lock);
    _operations.clear();
}

SwappyRenderer::ThreadState::ThreadState(GLContextConfig preferred_ctx_config,
...
SwappyRenderer::ThreadState::~ThreadState() {
    ClearSurface();
    if ( context != EGL_NO_CONTEXT) eglDestroyContext(display, context);
    if ( display != EGL_NO_DISPLAY) eglTerminate(display);
}

void SwappyRenderer::ThreadState::ClearSurface() {
...
bool SwappyRenderer::ThreadState::ConfigHasAttribute(EGLint attribute, EGLint value) {
    EGLint outValue = 0;
    EGLBoolean result = eglGetConfigAttrib(display, config, attribute, &outValue);
    return result && (outValue == value);
}

EGLBoolean SwappyRenderer::ThreadState::MakeCurrent(EGLSurface surface) {
    return eglMakeCurrent(display, surface, surface, context);
}

bool SwappyRenderer::ThreadState::TryCreateContext(GLContextConfig req_config) {
...
void SwappyRenderer::Draw(ThreadState* thread_state) {
