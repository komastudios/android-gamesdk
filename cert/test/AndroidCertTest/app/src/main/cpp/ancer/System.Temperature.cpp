...
/**
 * True if a thermal zone is, in particular thermal zone 0. Thermal zone 0 is our last resort in
 * case no thermal zone type contained any specific CPU-related pattern.
 */
bool IsThermalZone0(const ThermalZone &thermal_zone) {
  static const std::regex THERMAL_ZONE0_REGEX{".*/thermal_zone0"};
  return std::regex_match(thermal_zone.Path(), THERMAL_ZONE0_REGEX);
}
 ...
 *         seems to be CPU-related, it defaults on thermal zone 0.
 */
std::list<ThermalZone> FilterCPUThermalZones(std::list<ThermalZone> &&thermal_zones) {
  ...
  if (thermal_zones.empty() && thermal_zone0) {
...
std::vector<ancer::TemperatureInCelsiusMillis> ancer::CaptureTemperatures() {
  ...
  if (IsCaptureStatus(CaptureStatus::Ready)) {
    ...
    for (auto &temperature_file : _cpu_thermal_zones) {
      temperature_measurements.push_back(temperature_file.CaptureTemperature());
    }
    SetCaptureStatus(CaptureStatus::Ready);
  } else {
    for (auto &temperature_file : _cpu_thermal_zones) {
