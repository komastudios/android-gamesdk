 ...
 * file in any thermal_zone folder contained the substring "cpu" (case insensitive), then the first
 * retrieved thermal_zone will be assumed to be the CPU one.
 *
 *
 *
 * Links of interest:
...
#pragma clang diagnostic pop
  ...
  TemperatureInCelsiusMillis CaptureTemperature() {
    ancer::TemperatureInCelsiusMillis captured_temperature{kUnknownTemperatureMeasurement};
...
bool IsThermalZoneSubdir(const std::string &subdirectory) {
  static const std::regex kThermalZoneRegex{R"(thermal_zone\d+)"};
  return std::regex_match(subdirectory, kThermalZoneRegex);
}
...
bool IsCpuRelated(const ThermalZone &thermal_zone) {
  ...
  static const std::vector<std::regex> kCpuTypeRegexes{
      std::regex{".*cpu.*", std::regex::icase}
  };

  bool resolution{false};

  for (const auto &cpu_type_regex : kCpuTypeRegexes) {
...
ancer::TemperatureInCelsiusMillis ancer::CaptureMaxTemperature() {
  ancer::TemperatureInCelsiusMillis max_temperature{ancer::kUnknownTemperatureMeasurement};
...
ThermalStatus ancer::GetThermalStatus() {
  ...
  jni::SafeJNICall(
      [&status](jni::LocalJNIEnv *env) {
        static constexpr int kThermalStatusMin{0};
        static constexpr int kThermalStatusMax{6};
        ...
        if (status_int >= kThermalStatusMin && status_int <= kThermalStatusMax) {
