...
class MarchingCubesJobQueue {
 public:
  using WorkFn = ancer::MarchingCubesThreadPool::WorkFn;
 public:
    MarchingCubesJobQueue(ancer::ThreadAffinity affinity,
      bool pinned, int max_thread_count,
      ancer::MarchingCubesThreadPool::SleepConfig sleep_config) {

    _thread_pool = std::make_unique<ancer::MarchingCubesThreadPool>(
 ...
 private:
  std::mutex _lock;
  std::unique_ptr<ancer::MarchingCubesThreadPool> _thread_pool;
