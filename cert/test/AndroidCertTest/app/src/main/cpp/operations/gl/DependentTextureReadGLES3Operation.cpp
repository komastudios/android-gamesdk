  ...
  class Renderer {
   public:
    Renderer(const int left,
      ...
      InjectVerticesAndIndices(left, top, width, height);
      CreateGLObjects();
    }
...
//--------------------------------------------------------------------------------------------------
   ...
   private:
    void InjectVerticesAndIndices(const int left,
                                  const int top,
                                  const int width,
                                  const int height) {
      // Build mesh
      int n_verts_across = 2;
      int n_verts_tall = 2;

      for (int row = 0; row < n_verts_tall; ++row) {
        const auto across_row = static_cast<float>(row);
        for (int col = 0; col < n_verts_across; ++col) {
          const auto across_col = static_cast<float>(col);
