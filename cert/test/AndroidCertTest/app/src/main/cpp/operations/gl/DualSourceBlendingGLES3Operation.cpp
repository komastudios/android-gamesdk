...
// factor.
//
// Here, we consider a Test to be one permutation of that equation; a Test draws
// many color variants to the screen and the result is sampled. If any sample
// does not match the expected value, the Test fails. With each Test we log
// the greatest magnitude the sample varied from the expected value, which is
// usually 0 or 1 for a device that renders the correct result, but if the value
// exceeds 1, we also produce a screenshot of the worst-case run of that Test.
...
// "reference_screenshot_mode" is used only to generate a reference screenshot
// for a specific image variant. This should be false during normal use
// (i.e., when gathering data from devices).
struct configuration {
    bool reference_screenshot_mode;
};

JSON_CONVERTER(configuration) {
    JSON_OPTVAR(reference_screenshot_mode);
}
...
struct Test {
    ...
    // Defines how closely a pixel must match the expected value (range 0-255).
    // If a sample image's RGBA values exceed epsilon of the calculated
    // (expected) value, a screenshot will be taken for later analysis.
    int epsilon = 1;
...
struct TestFailureCapture {
    ...
    TestFailureCapture(RGBA src_palette_last_color, RGBA dst, RGBA src_1,
        std::string base64_image)
        : src_palette_last_color(src_palette_last_color), dst(dst),
          src_1(src_1), base64_image(base64_image) {}
};
...
struct TestResult {
    ...
    // failure will only be captured if greatest_variance value
    // of a sampled frame exceeds the test's epsilon value.
...
struct Tile {
    ...
    RGBA Expected() const {
        ...
        switch (blend_mode) {
            case GL_ADD:
                blend_result = glm::clamp(a + b, glm::vec4(0.0F),
                        glm::vec4(1.0F));
                break;
            case GL_SUBTRACT:
                blend_result = glm::clamp(a - b, glm::vec4(0.0F),
        ...
        return Vec4ToRGBA(blend_result);
    }
...
private:
    void InitTests() {
        const std::vector<BlendConfig> configs = {
            ...
            { GL_SUBTRACT, GL_ONE, GL_ZERO },

            { GL_SUBTRACT, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA },
            { GL_SUBTRACT, GL_SRC1_ALPHA_EXT, GL_ONE_MINUS_SRC1_ALPHA_EXT },
            { GL_SUBTRACT, GL_SRC_ALPHA, GL_ONE_MINUS_SRC1_ALPHA_EXT },
            { GL_SUBTRACT, GL_SRC1_ALPHA_EXT, GL_ONE_MINUS_SRC_ALPHA },

            { GL_SUBTRACT, GL_ONE, GL_SRC1_COLOR_EXT },
            { GL_SUBTRACT, GL_ONE, GL_ONE_MINUS_SRC1_COLOR_EXT },

            { GL_SUBTRACT, GL_SRC_ALPHA, GL_SRC1_COLOR_EXT },
            { GL_SUBTRACT, GL_SRC1_COLOR_EXT, GL_SRC1_COLOR_EXT },
        };
    ...
    void GenerateReferenceScreenshot() {
        ...
        const RGBA dst{ 255, 255, 255, 255 };
        const RGBA src_1 { 0, 208, 0, 128 };
        Palette& src_palette = red_palette;
        const GLenum blend_mode = GL_ADD;
        const GLenum blend_s_factor = GL_SRC1_COLOR_EXT;
        const GLenum blend_d_factor = GL_SRC1_COLOR_EXT;
        ...
        const auto base64_image = Base64EncodeBytes(png_data.data(),
    ...
    void RunNextTest() {
        ...
        // To run a single test indefinitely (i.e., for visual debugging),
        // comment out pop_back().
        _tests.pop_back();
                    ...
                    for (const RGBA& src_1 : src_1_palette) {
                        ...
                        for (const Palette& src_palette : test.src_palettes) {
                            ...
                            const auto new_result = RunTestIteration(
                                    result.greatest_variance,
                                    dst, src_palette, src_1, test.blend_mode,
                                    test.blend_s_factor, test.blend_d_factor);
    ...
    TestResult RunTestIteration(
            int epsilon,
            RGBA dst,
            const Palette &src_palette,
            RGBA src_1,
            GLenum blend_mode,
            GLenum blend_s_factor,
            GLenum blend_d_factor) {
        ...
        const std::vector<Tile> tiles = DrawTestFrame(dst, src_palette, src_1,
                blend_mode, blend_s_factor, blend_d_factor);
        ...
        if (variance > epsilon) {
            ...
            const auto png_data = _pixel_buffer->ToPNG();
            result.failure->base64_image = Base64EncodeBytes(png_data.data(),
                    static_cast<int>(png_data.size()));
        }
    ...
    std::vector<Tile> DrawTestFrame(RGBA dst,
                                    const Palette &src_palette,
                                    RGBA src_1,
                                    GLenum blend_mode,
                                    GLenum blend_s_factor,
        ...
        // Draw src palette as grid and record tile
        // (grid) locations for measuring
    ...
    // If the tile was drawn correctly, the return value should be <= 1,
    ...
    int MeasureVariance(std::unique_ptr<GLPixelBuffer>& buffer,
        ...
        for (const Tile& tile : tiles) {
            ...
            const RGBA result = buffer->ReadPixel(screen_coord.x,
                    screen_coord.y);
