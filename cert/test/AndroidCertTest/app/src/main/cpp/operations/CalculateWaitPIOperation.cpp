...
namespace {
    ...
    JSON_READER(configuration) {
        ...
        JSON_REQVAR(iterationWaitCount);
        JSON_REQVAR(iterationWaitTime);
...
private:
    ...
    void CalculatePi(configuration config) {
        ...
        for (auto start_time = SteadyClock::now();; i++, d += 2, ++current_pi.iterations) {
            if (++frameCountSinceRest == config.iterationWaitCount) {
                usleep(config.iterationWaitTime); //1E-6
                frameCountSinceRest = 0;
            }
