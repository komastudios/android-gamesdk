 ...
 */
std::vector<execution_configuration>
GeneratePermutations(const configuration config) {
...
class MarchingCubesGLES3Operation : public BaseGLES3Operation {
 public:
  ...
  void OnGlContextReady(const GLContextConfig& ctx_config) override {
    ...
    _configuration_permutations = GeneratePermutations(_configuration);
    Log::I(TAG,
 ...
 private:
  ...
  void BuildExecConfiguration(execution_configuration ex_config) {
    ...
    _job_queue = std::make_unique<mc::JobQueue>(
 ...
 private:
  ...
  unique_ptr<mc::JobQueue> _job_queue;
