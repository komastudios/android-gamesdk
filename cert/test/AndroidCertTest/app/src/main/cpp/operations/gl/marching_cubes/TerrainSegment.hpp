...
struct TerrainSegment {
 ...
 public:
  TerrainSegment(int size, std::shared_ptr<ThreadPool> threadPool, FastNoise &noise)
      : idx(-1), size(size), threadPool(threadPool), noise(noise) {
  ...
  void build(int idx, FastNoise &noise) {
    ...
    for (int i = 0; i < maxArches; i++) {
      ...
      vec3
          waypoint = arch.axisOrigin + arch.axisPerp * arch.innerRadius * rng.nextFloat(0.2F, 0.8F);
