...
enum class TestStage {
   ...
   * enabled at all, and the big.LITTLE core numbering was from biggest to littlest.
   */
      TestShutdown
};
...
struct datum {
  ...
  static datum on_thread_progress(const bool is_affinity_preserved) {
    datum datum;

    datum.stage = TestStage::ThreadInProgress;
    datum.cpuset_enabled = is_affinity_preserved;
...
void WriteDatum(report_writers::Struct writer, const datum &data) {
  switch (data.stage) {
    ...
    case TestStage::TestShutdown : {
      writer.AddItem("stage", "Test shutdown");
...
inline auto SetCpusetForThisThread(const cpu_set_t *cpuset) {
  return sched_setaffinity(THIS_THREAD_PID, sizeof(cpu_set_t), cpuset);
}

inline auto GetCpusetForThisThread(cpu_set_t *cpuset) {
...
bool NarrowCoresForThisThread(const std::set<int> &cores) {
  ...
  if (SetCpusetForThisThread(&cpuset_in)==OK && GetCpusetForThisThread(&cpuset_out)==OK) {
        ...
        ::Eulers_Number_Looper(
            ...
            [this, &affine_cores](const long double e, const unsigned long long iterations) {
              auto current_thread_is_on_affine_core =
                  affine_cores.find(sched_getcpu())!=affine_cores.end();
