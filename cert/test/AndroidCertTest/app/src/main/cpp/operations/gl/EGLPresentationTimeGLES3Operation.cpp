...
class EGLPresentationTimeGLES3Operation : public BaseGLES3Operation {
 ...
 private:
  ...
  void CheckAvailability() {
    ...
    _extension_datum.has_fp_get_next_frame_id = nullptr != fptrGetNextFrameId;
    ...
    if (_extension_datum.has_fp_get_frame_timestamp_supported) {
      ...
      const EGLBoolean request = fptrGetFrameTimestampSupported(
          display, surface, EGL_REQUESTED_PRESENT_TIME_ANDROID);
      _extension_datum.request_timestamp_supported = static_cast<bool>(request);

      const EGLBoolean present = fptrGetFrameTimestampSupported(
          display, surface, EGL_DISPLAY_PRESENT_TIME_ANDROID);
      _extension_datum.present_timestamp_supported = static_cast<bool>(present);
    }
  ...
  void CheckCanPerformTest() {
    bool can_perform_test =
        ...
        _extension_datum.has_fp_get_frame_timestamps &&
        _extension_datum.has_fp_get_frame_timestamp_supported &&
        _extension_datum.request_timestamp_supported &&
        _extension_datum.present_timestamp_supported;
  ...
  int64_t GetNextFramePresentTime() {
    ...
    const int64_t kFrameOffset = 1 * 1 * 1000 * 1000;  // ms
  ...
  void CheckFrames() {
    for (auto& frame : _frames) {
      if (!frame.timestamp_collection_finished) {
        ...
        // CombinedUpdateFrameTimestamps(frame);
      }
    }
  }
