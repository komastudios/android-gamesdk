...
class EGLPresentationTimeGLES3Operation : public BaseGLES3Operation {
 ...
 private:
  ...
  TimestampResult GetTimestamp(uint64_t frame_id, EGLint event_name,
    ...
    if (t == EGL_TIMESTAMP_INVALID_ANDROID) return TimestampResult::INVALID;
    if (t == EGL_TIMESTAMP_PENDING_ANDROID) return TimestampResult::PENDING;
 ...
 private:
  ...
  vector<frame_presentation_datum> _frames;
};
