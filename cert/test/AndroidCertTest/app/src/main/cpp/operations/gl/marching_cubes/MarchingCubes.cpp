...
int Polygonise(const GridCell &cell, float isolevel, Triangle<Vertex> *triangles) {
  ...
  */
  ...
  for (int i = 0; detail::kTriTable[cubeIndex][i] != -1; i += 3) {
    ...
    glm::vec3 n = normalize(cross(triangles[numTriangles].b.pos - triangles[numTriangles].a.pos,
                                  triangles[numTriangles].c.pos - triangles[numTriangles].a.pos));
