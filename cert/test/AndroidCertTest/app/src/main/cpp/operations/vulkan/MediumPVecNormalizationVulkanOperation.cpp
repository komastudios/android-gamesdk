 ...
 * Input:
 *
 * vertex_stage_configuration: configuration params for the test as run on
 * vertex processor enabled: [bool] is this test enabled? offset_steps:[int]
 * number of steps in range to test offset_scale:[float] each channel will be
 * incremented by (i / offset_steps) * offset_scale resulting in offset values
 * from [0, offset_scale] fragment_stage_configuration: configuration params for
 * running the test on the fragment processor - same fields as above
 ...
 *  expected_failure:[bool] if true, normalization failure is expected due to
 * precision expected_rgb:[rgb triplet] expected framebuffer rgb value for the
 * read fragment actual_rgb:[rgb triplet] value read from framebuffer
...
struct rgb_u8 {
  ...
  rgb_u8(uint8_t r, uint8_t g, uint8_t b) : r(r), g(g), b(b) {}
  ...
  rgb_u8(glm::u8vec4 v) : r(v.r), g(v.g), b(v.b) {}
...
struct Vertex {
  enum class Attributes : GLuint { Pos = 0, BaseColor = 1, ColorOffset = 2 };
...
struct StageTest {
 ...
 public:
  ...
  Result Initialize(Vulkan& vk, const char* vertex_shader,
                    const char* fragment_shader, ResourcesLayout& layout,
    ...
    _uniform_buffer
        .Initialize(vk, EResourceUse::CPUToGPU, sizeof(float), nullptr)
        .ok();
    ...
    VK_RETURN_FAIL(_pipeline.Builder()
                       .Shader(vertex_module)
                       .Shader(fragment_module)
                       .VertexBinding(0, sizeof(Vertex))
                       .VertexAttribute(0, 0, VK_FORMAT_R32G32B32_SFLOAT,
                                        offsetof(Vertex, pos))
                       .VertexAttribute(0, 1, VK_FORMAT_R32G32B32_SFLOAT,
                                        offsetof(Vertex, base_color))
                       .VertexAttribute(0, 2, VK_FORMAT_R32G32B32_SFLOAT,
                                        offsetof(Vertex, color_offset))
                       .PrimitiveTopology(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST)
                       .StaticViewport(0, 0, RenderWidth, RenderHeight, 0, 1, 0,
                                       0, RenderWidth, RenderHeight)
  ...
  Result Bind(GraphicsContext& context, float offset_scale) {
    ...
    *_uniform_buffer.Map<float>() = offset_scale;
...
enum class Test : int {
  VertexStageTest = 0,
  FragmentStageTest,
  VaryingPassthroughTest
};

static const char* TestNames[] = {"VertexStageTest", "FragmentStageTest",
                                  "VaryingPassthroughTest"};
...
class MediumPVecNormalizationVulkanOperation : public BaseVulkanOperation {
 public:
  ...
  ~MediumPVecNormalizationVulkanOperation() {
    std::lock_guard<std::mutex> lk(_draw_mutex);
  ...
  void Start() override {
    ...
    _color_buffer
        .Initialize(_vk, EResourceUse::GPU,
    ...
    _color_buffer_cpu.Builder()
        .ResourceUse(EResourceUse::GPUToCPU)
        .Extents(RenderWidth, RenderHeight, 1)
        .Linear(VK_FORMAT_R8G8B8A8_UNORM, false, true)
    ...
    _renderpass.Builder()
        .Color(0, VK_FORMAT_R8G8B8_UNORM)
        .Clear()
        .End()
        .Subpass(0)
        .Color(0)
    ...
    size_t stages[3] = {static_cast<size_t>(Test::VertexStageTest),
    ...
    for (size_t i = 0; i < 3; ++i) {
      _program_states[stages[i]]
          .Initialize(_vk, v_shaders[i], f_shaders[i], _dsl, _renderpass)
          .ok();
    }

    _context.Initialize(_vk, 3, "main").ok();
  ...
  void Draw(double delta_seconds) override {
    ...
    _context
        .BeginRenderPass(_renderpass, RenderWidth, RenderHeight, 1,
                         {_color_buffer.View()}, {ClearColor(0, 0, 0, 1)})
        .ok();
    ...
    _vk->cmdBindIndexBuffer(cbuf, _index_buffer.Handle(), 0,
                            _index_buffer.IndexType());
    ...
    _context.ChangeImageLayout(
        _color_buffer.Resource().Layout(VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL),
        VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
    _context.ChangeImageLayout(_color_buffer_cpu.Resource().Layout(
                                   VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL),
                               VK_IMAGE_LAYOUT_GENERAL);

    //_context.CopyImage(_color_buffer.Resource(),
    //_color_buffer_cpu.Resource());
    _context.BlitImage(_color_buffer.Resource(), _color_buffer_cpu.Resource(),
                       VK_FILTER_NEAREST);

    _context.ChangeImageLayout(_color_buffer.Resource(),
                               VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);
    _context.ChangeImageLayout(_color_buffer_cpu.Resource(),
                               VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);

    _context.End().ok();

    VkSemaphore semaphore;
    _vk.SubmitTemporaryCommandBuffers(Q_TRANSFER, semaphore).ok();
    if (semaphore != VK_NULL_HANDLE) {
      _context.Wait(semaphore, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT);
    }

    Fence fence;
    _context.GetFence(fence, true).ok();
    _vk.SubmitToQueue(_context, fence).ok();
 ...
 private:
  ...
  void CreateTestPattern(test_configuration conf, vec3 base_color) {
    ...
    Log::D(TAG, "CreatePattern[%s] - _currentTest: %d _currentColorMask: %d",
           TestNames[_currentTest], _currentTest, _currentColorMask);
    ...
    for (int i = 0; i < slices; i++) {
      ...
      const auto offset_color =
          vec3(base_color.r * i * offset_step, base_color.g * i * offset_step,
      ...
      Log::D(TAG,
             "generating pattern for %d slices; base_color(%.3f,%.3f,%.3f)"
             " offset_color(%.3f,%.3f,%.3f) summed(%.3f,%.3f,%.3f)"
             " expected(%.3f,%.3f,%.3f) failure_acceptable: %s",
             slices, base_color.r, base_color.g, base_color.b, offset_color.r,
             offset_color.g, offset_color.b, summed.r, summed.g, summed.b,
             expected.r, expected.g, expected.b,
             failure_acceptable ? "true" : "false");

      // generate a probe to read in ReadTestPattern()
      _probes.push_back(Probe{
          ivec2(middle_x_px, middle_y_px), failure_acceptable, summed, expected,
          (i * offset_step) * conf.offset_scale, squared_magnitude});
    }
    ...
    _index_buffer
        .Initialize(_vk, EResourceUse::GPU, VK_INDEX_TYPE_UINT16,
                    sizeof(uint16_t) * _num_indices, indices.data())
        .ok();

    _vertex_buffer
        .Initialize(_vk, EResourceUse::GPU, sizeof(Vertex) * vertices.size(),
                    vertices.data())
        .ok();
  }

  void ReadTestPattern(test_configuration conf, vec3 base_color) {
    _color_buffer_cpu.Allocation().Invalidate(_vk).ok();
    ...
    const uint8_t* _pixels =
        static_cast<const uint8_t*>(_color_buffer_cpu.Allocation().Map());

    for (auto const& probe : _probes) {
      // read value_rgb at probe, and compute delta r,g,b

      const int idx = (probe.pos.y * RenderWidth + probe.pos.x) * 4;

      glm::u8vec4 value_rgb;
      value_rgb.r = _pixels[idx];
      value_rgb.g = _pixels[idx + 1];
      value_rgb.b = _pixels[idx + 2];
      value_rgb.a = _pixels[idx + 3];

      const auto expected_value_rgb = rgb_u8{probe.expected_result_normalized};

      const auto dr = abs(static_cast<int>(value_rgb.r) -
                          static_cast<int>(expected_value_rgb.r));

      const auto dg = abs(static_cast<int>(value_rgb.g) -
                          static_cast<int>(expected_value_rgb.g));

      const auto db = abs(static_cast<int>(value_rgb.b) -
      ...
      // report our read
      const auto r =
          result{TestNames[_currentTest], failure,   probe.failure_acceptable,
                 expected_value_rgb,      value_rgb, probe.offset,
      ...
      if (!probe.failure_acceptable) {
        if (failure) {
          ...
          Log::E(TAG,
                 "ReadPattern[%s] MISMATCH (_currentTest: %d "
                 "_currentColorMask: %d)"
                 " base_color: (%f,%f,%f) offset: %f squared_magnitude: %f"
                 " expected_color_rgb: (%d,%d,%d) got (%d,%d,%d)",
                 TestNames[_currentTest], _currentTest, _currentColorMask,
                 base_color.r, base_color.g, base_color.b, probe.offset,
                 probe.squared_magnitude, expected_value_rgb.r,
                 expected_value_rgb.g, expected_value_rgb.b, value_rgb.r,
                 value_rgb.g, value_rgb.b);
        } else {
          Log::D(
              TAG,
