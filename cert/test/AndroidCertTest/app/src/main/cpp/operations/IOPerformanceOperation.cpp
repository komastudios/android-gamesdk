    ...
    void Start() override {
        ...
        _thread = std::thread{[this] {
            ...
            if (_config.work_scheme == WorkScheme::kGreedy) {
                DoGreedyWork(thread_configs);
            } else {
                DoEvenWork(thread_configs);
            }
...
// Putting everything together
    ...
    void DoGreedyWork(const ThreadConfigs& thread_configs) {
        ...
        for (Bytes work_done = {0} ; work_done < _config.total_read ;
            ...
            for (auto& thread_config : thread_configs) {
                threads.emplace_back([this, &greedy, thread_config,
        ...
        for (auto& thread_config : thread_configs) {
            threads.emplace_back([this, thread_config] {
                ...
                const auto num_iterations =
                        (_config.total_read / _config.total_area_size).count();
                Reporter<Datum> reporter{*this};
                reporter.datum.thread_id = thread_config.id;
    ...
    template <typename Scheme>
    void DoWork(Reporter<Datum>& reporter, Scheme& scheme) {
        if (_config.buffer_align == Bytes{-1}) {
            DirectBuffer buffer{_data, reporter.datum, _config, scheme};
            DoWork(reporter, _config.report_rate, scheme, buffer);
        } else {
    ...
    void DoWork(Reporter<Datum>& reporter, Milliseconds report_rate,
                Scheme& scheme, Buffer& buffer) {
        switch (_config.file_api) {
            case FileApi::kCApi: {
                CApi api{_config, scheme};
                return DoWork(reporter, report_rate, scheme, buffer, api);
            }
    ...
    void DoWork(Reporter<Datum>& reporter, Milliseconds report_rate,
                Scheme& scheme, Buffer& buffer, File& file) {
