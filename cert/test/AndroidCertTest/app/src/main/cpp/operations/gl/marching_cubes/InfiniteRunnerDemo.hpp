...
//    _lineMaterial = std::make_unique<LineMaterial>();
    ...
    CheckGlError("OnGlContextReady() - Created skydome cubemap");

    _skydomeMaterial = std::make_unique<SkydomeMaterial>(skyboxTexture);

    CheckGlError("OnGlContextReady() - Created skydome material");
...
//    //
//
//    const auto frequency = 1.0F / kSegmentSize;
//    _fastNoise.SetNoiseType(FastNoise::Simplex);
//    _fastNoise.SetFrequency(frequency);
//
//    constexpr auto COUNT = 3;
//    for (int i = 0; i < COUNT; i++) {
//      _segments.emplace_back(std::make_unique<TerrainSegment>(kSegmentSize, _threadPool, _fastNoise));
...
//    }
 ...
 private:
  ...
  static constexpr bool kAutopilot = false;
