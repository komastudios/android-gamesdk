...
// This two macros are to avoid using setjmp()/longjmp() straight in our code. With these macros we
// better express the intent of these powerful functions, which we use to deviate a SIGABRT in order
// to resume the normal test flow.
#define IS_CRASH_FLAG_RAISED()    setjmp(::jump_buffer)
#define RAISE_CRASH_FLAG()        longjmp(::jump_buffer, 1)

namespace {
// Please, if changing this TAG, change it also in NougatSigabrtHostActivity so that both log
// closely related messages under the same TAG filter. E.g.,
// $ adb logcat -s NougatCrashTest:I
constexpr Log::Tag TAG{"NougatCrashTest"};
}
...
void SigabrtHandler(int signal) {
  if (SIGABRT==signal) {
    ...
    // We are here just-in-time before a SIGABRT crash happens. Instead, we'll raise a flag and
    // resume the normal test flow.
    RAISE_CRASH_FLAG();
  } else {
 ...
 public:
  ...
  void Start() override {
    ...
    _thread = std::thread{[this] {
      ...
      while (not(IsStopped() || IS_CRASH_FLAG_RAISED()));
