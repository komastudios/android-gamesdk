 ...
 private:
    ...
    for (auto const& s : _sample_src) {
      ...
      if (_depth_clear_value >= s.depth) {
        if (value != s.written_value) {
          ...
          errors = true;
          Report(datum::create_error_incorrect_fragment_rejection(s.depth,
                                                                  _depth_clear_value));
        ...
        errors = true;
        Report(datum::create_error_incorrect_fragment_pass(s.depth,
                                                           _depth_clear_value));
  ...
  void CreatePattern(int slices) {
    ...
    for (int i = 0; i < slices; i++) {
      ...
      // create a sample value using the centroid of the quad
      ...
      Log::D(TAG,
             "i: %d -- mid_x: %f across_x: %f mid_x_px: %d, mid_y: %f across_y: %f mid_y_px: %d",
             i,
             middle_x,
             across_x,
             middle_x_px,
