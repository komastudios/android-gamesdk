...
const std::string kOperationTexturePath{"Textures/TextureCompression"};
}
...
std::vector<std::string> texture_family_names({
                                                  "avocados",
                                                  "ballons",
                                                  "bananas",
                                                  "cards",
...
std::vector<TextureMetadata *> slideshow_textures;

inline void PushTextureToSlideshow(TextureMetadata *const texture_metadata) {
  slideshow_textures.push_back(texture_metadata);
  Log::D(TAG,
...
class TextureLoadingGLES3Operation : public BaseGLES3Operation {
 public:
  ...
  ~TextureLoadingGLES3Operation() override {
    ...
    for (auto *texture_metadata : slideshow_textures) {
      delete texture_metadata;
    }
  }
...
//--------------------------------------------------------------------------------------------------
 ...
 private:
  ...
  std::thread TriggerSlideshowThread() {
    return std::thread{[this] {
      ...
      while (not this->IsStopped() && this->_slideshow_index < ::slideshow_textures.size()) {
        _new_slide = ::slideshow_textures[_slideshow_index]->Load();
  ...
  static GLuint LoadTexture(TextureMetadata *const texture_metadata) {
    int tex_width = 0;
    int tex_height = 0;

    const auto path_string = texture_metadata->ToString();
    const auto *path = path_string.c_str();
    GLuint tex_id = ::LoadTexture(path, &tex_width, &tex_height, nullptr);
    Log::I(TAG, "Loaded texture %s as texture id %u", path, tex_id);
