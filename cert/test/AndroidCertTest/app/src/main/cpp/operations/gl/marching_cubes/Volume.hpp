...
class OctreeVolume : public BaseCompositeVolume {
 ...
 public:
  OctreeVolume(int size,
               float fuzziness,
               int min_node_size,
               ancer::unowned_ptr<JobQueue> job_queue,
  ...
  */
  ...
  friend class MarchNodeWorkUnit;

  class MarchNodeWorkUnit : public JobQueue::WorkUnit {
   public:

    MarchNodeWorkUnit (OctreeVolume *volume, OctreeVolume::Node* node,
        ancer::unowned_ptr<glm::mat4> transform, bool compute_normals)
        : _volume(volume),
          _node(node),
          _transform(transform),
          _compute_normals(compute_normals) {}

    ~MarchNodeWorkUnit() = default;

    void DoWork(int thread_idx) override {
      auto tc_idx = thread_idx % _volume->_triangle_consumers.size();
      auto tc = _volume->_triangle_consumers[tc_idx];
      _volume->MarchNode(_node, *tc, _transform, _compute_normals);
    }

   private:
    ancer::unowned_ptr<OctreeVolume> _volume;
    OctreeVolume::Node* _node;
    ancer::unowned_ptr<glm::mat4> _transform;
 ...
 private:
  ...
  ancer::unowned_ptr<JobQueue> _job_queue;
