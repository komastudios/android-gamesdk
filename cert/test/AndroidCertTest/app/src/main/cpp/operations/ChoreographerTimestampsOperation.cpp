 ...
 public:
  ...
  void Wait() override {
    ...
    while (wait) {
      ...
      wait = _frame_counter < _configuration.last_frame_id;
    }
  }
 ...
 private:
  bool ReportAvailability() {
    if (getInstance == nullptr || postFrameCallback == nullptr) {
      FatalError(TAG,
                 "Required Choreographer library functions failed to load");
    }

    if (getNextFrameId == nullptr || getFrameTimestampSupported == nullptr ||
        getFrameTimestamps == nullptr) {
      FatalError(TAG, "Supporting EGL library functions failed to load");
    }

    const bool available = glh::CheckEglExtension(get_frame_timestamps_ext);
    const bool did_set_surface = EnableSurfaceTimestamps();

    return available && did_set_surface;
  }
