...
void main() {
    vec2 lastTexCoord = vTexCoord;
    outColor = texture(uTex, lastTexCoord);  // so far, the original texture read directly.
    ...
    for (int i = 0; i < uIndirections; ++i) {
        ...
        lastTexCoord += deltaXY;

        // Each iteration ends with an indirect reading.
        outColor = texture(uTex, lastTexCoord);
    }
}
