...
package com.google.androidcerttest.transport;

import android.content.Context;
import android.text.TextUtils;
import android.util.Log;

import com.google.androidcerttest.util.TimeParsing;
...
/**
 * Describes a set of stress tests to execute. Intended to be loaded from JSON; may be
...
public class Configuration {
    ...
    public static class StressTest {
        ...
        // "host", e.g., the activity which will be launched to execute the stressors/tests
        private String host;
        ...
        /**
         * @return the display name of the StressTest
         */
        ...
        /**
         * @return the host (read, Activity or some such environment) which will execute the stressors and tests
         */
        ...
        public int getDurationMillis() {
            return (int) TimeParsing.parseDurationString(duration, TimeParsing.Unit.Milliseconds);
        }
    ...
    /**
     * If true, when MainActivity starts and reads this configuration, it will immediately
     * start executing the StressTests one by one until exhausted, then will call stop()
     */
    ...
    /**
     * Delay (in milliseconds) to allow the device to "breath" between execution of stress tests
     */
    ...
    /**
     * @return an array of default monitors to inject into a StressTest if it hasn't specified its own
     */
    ...
    public static Configuration loadFromJson(String jsonString) {
        ...
        c.stressTests = Arrays.stream(c.stressTests).filter((st) -> st.enabled).toArray(StressTest[]::new);

        // for each enabled stress filter out disabled stressors
        for (StressTest st : c.stressTests) {
            st.stressors = Arrays.stream(st.stressors).filter((s) -> s.enabled).toArray(Operation[]::new);
     ...
     * @param jsonResourceId the id of the JSON resource, e.g., R.raw.configuration_json
     * @return The Configuration, or null if resource isn't valid
     */
    public static Configuration loadFromJsonResource(Context ctx, int jsonResourceId) {
        try (InputStream is = ctx.getResources().openRawResource(jsonResourceId)) {
            ...
            Log.d(TAG, "JSON Resource was not a properly encoded JSON file, error: " + e.getLocalizedMessage());
            e.printStackTrace();
        } catch (IOException e) {
            Log.d(TAG, "Unable to open json resource for reading, error: " + e.getLocalizedMessage());
            e.printStackTrace();
        }
        return null;
    }
