...
public class JsonManipulatorOperation extends BaseOperation {
    ...
    private static final String TAG = JsonManipulatorOperation.class.getSimpleName();
    ...
    public JsonManipulatorOperation(String suiteId, String configurationJson, Context context, Mode mode) {
        super("JsonManipulatorOperation", suiteId, configurationJson, context, mode);
        _gson = new Gson();
    ...
    public void start(long runForDurationMillis) {
        ...
        _thread = new Thread(() -> {
            ...
            if (userDataJsonString == null) {
                throw new IllegalStateException("JsonManipulatorOperation::run - unable to load json asset \"" + _configuration.json_asset_name + "\"");
            }
            ...
            while (!isStopped()) {
                ...
                if (isTest) {
                    if (elapsedMillis - datum.startTimeMillis > _configuration.performance_sample_period_millis) {
    ...
    private String loadString(String assetName) {
        try (InputStream stream = getContext().getAssets().open(assetName)) {
            ...
            Log.e(TAG, "Unable to open asset \"" + assetName + "\" for reading");
        }
    ...
    private UserDatum[] FrobulateUserData(UserDatum[] datums) {
        return Arrays.stream(datums).map(UserDatum::frobulate).toArray(UserDatum[]::new);
    }

    private static String frobulateString(String s) {
        // just make new string with first letter moved to end: "frobulate" -> "robulatef"
        return s.substring(1) + s.substring(0, 1);
    }

    private static String[] frobulateStrings(String[] strs) {
        return Arrays.stream(strs).map(JsonManipulatorOperation::frobulateString).toArray(String[]::new);
    }

    private static FriendDatum[] frobulateFriends(FriendDatum[] friends) {
        return Arrays.stream(friends).map(FriendDatum::frobulate).toArray(FriendDatum[]::new);
    }

    //
    //  FriendDatum and UserDatum map directly to the data represented in assets/generated.json
    //
