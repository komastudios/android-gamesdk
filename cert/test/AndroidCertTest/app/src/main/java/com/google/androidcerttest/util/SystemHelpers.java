...
public class SystemHelpers {
    ...
    /**
     * Get the current thermal status as reported by the system PowerManager
     *
     * @return a thermal status value as defined here https://developer.android.com/reference/android/os/PowerManager.html#THERMAL_STATUS_NONE
     */
    ...
    public String loadText(String path) {
        try {
            ...
            Log.e(TAG, "loadText - unable to load text from file \"" + path + "\"");
        }
    ...
    public static class MemoryInformation {
        ...
        // total memory in bytes "accessible to kernel" (via ActivityManager.MemoryInfo)
        public long totalMemory;

        // available memory in system via ActivityManager.MemoryInfo (via ActivityManager.MemoryInfo)
        public long availableMemory;

        // memory use threshold at which point system is going to enter a low memory condition (via ActivityManager.MemoryInfo)
        public long threshold;

        // true if system is in a low memory condition (via ActivityManager.MemoryInfo)
    ...
    public MemoryInformation getMemoryInfo() {
        ...
        info.commitLimit = getMemInfo("CommitLimit") * 1024; // commit limit is in kb, so make bytes
    ...
    public int getOomScore() {
        try {
            return Integer.parseInt(readFile(("/proc/" + _pid) + "/oom_score"));
