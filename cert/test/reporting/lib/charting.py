...
def should_chart_be_rendered(chart: Chart, restrict_to: List[str], \
    ...
    @param chart: the Chart whose rendering is to be determined.
    @param restrict_to: list of data points to include. If empty, it's assumed
                        that this chart is included.
    @param skip: list of data points to exclude.

    @returns True if the chart should be rendered.
    """
    yes_no = True
