...
class ChoreographerTimestampsSuiteHandler(SuiteHandler):
    ...
    def __init__(self, suite):
        ...
        for d in self.data:
            ...
            if ts:
                ...
                self.variance.append(sys_ts - ts)
    ...
    def render_plot(self) -> str:
        ...
        variance_ms = [i / 1_000_000 for i in self.variance]

        max_y = 2.0
        pass_cutoff_y = 1.0
        max_x = len(variance_ms)

        axes.axis([0, max_x, 0, max_y])
        axes.plot(variance_ms)

        eval_label_text = ""
        eval_label_y = 0
        if max(variance_ms) < pass_cutoff_y:
            eval_label_text = "PASS"
            eval_label_y = 0.25
            pass_indicator = Rectangle((0, 0), max_x, pass_cutoff_y, \
                facecolor='green', alpha=0.5)
            axes.add_patch(pass_indicator)
        else:
            eval_label_text = "FAIL"
            eval_label_y = 0.75
            fail_indicator = Rectangle((0, pass_cutoff_y), max_x, 1, \
                facecolor='red', alpha=0.5)
            axes.add_patch(fail_indicator)

        axes.text(0.5, eval_label_y, \
            eval_label_text, \
            fontsize=24, \
            alpha=0.75, \
