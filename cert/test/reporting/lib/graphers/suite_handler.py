...
class SuiteHandler(ABC):
    ...
    def plot(self,
        ...
        customzied to a high degree. Nevertheless, it may be overridden if the
...
class SuiteSummarizer:
    ...
    def __init__(self, reports: List[Suite]):
        ...
        Args:
            reports: _should be_ any Suite that passes the can_handle_report
                check. (It is the caller's responsibility to ensure this.)
        """
        # List of reports, each containing only data that passed the
        # datum_filter test.
    ...
    def synthesis(self, ctx: SummaryContext) -> List[fmt.Item]:
        ...
        property on itself, which can then be accessed in this `synthesis`
        method by iterating over `self.suite_handlers` to determine the total
        number of successes.
    ...
    def render_summary(self, ctx: SummaryContext) -> List[fmt.Item]:
        ...
        This method _can_ be overridden, but it is usually preferable to work
        directly with the `synthesis` method. (One might wish to override this
        method if the default reporting order or formatting is insufficient.)
        ...
        for handler in self.suite_handlers:
            reports.append(fmt.Heading(handler.suite.identifier(), 2))
            reports.extend(handler.render_report(ctx))
        ...
        # `synthesis` method may access computed results from any/all
        # of the SuiteHandlers in self.suite_handlers.)
        synthesis = self.synthesis(ctx)
        if synthesis:
