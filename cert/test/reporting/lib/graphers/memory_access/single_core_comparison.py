...
class SingleCoreComparisonHandler(SuiteHandler):
    def __init__(self, suite):
        ...
        self.suite_data = [
            ...
            and d.get_custom_field("event") == None
        ]

    # TODO: Split into single/multi-suite
    @classmethod
...
#-------------------------------------------------------------------------------
# TODO: Move to general helpers once that exists

    @classmethod
    def get_core_size(cls, suite, cpu_id):
        cpu_info = suite.suite.build._d
        if cpu_id in cpu_info['Big']['cpus']:
            return 'Big'
        elif cpu_id in cpu_info['Middle']['cpus']:
            return 'Middle'
        else:
            return 'Little'
...
#-------------------------------------------------------------------------------
    ...
    def render_report(cls, raw_suites: List['SuiteHandler']):
        # TODO(tmillican@google.com): Python
        suites = [
