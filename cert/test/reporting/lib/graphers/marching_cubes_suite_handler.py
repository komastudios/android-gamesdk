...
class MarchingCubesSuiteHandler(SuiteHandler):
    ...
    def __init__(self, suite):
        ...
        self.min_voxels_per_second = float('inf')
        self.max_voxels_per_second = 0

        for d in my_data:

            vps = voxels_per_second(d)
            self.min_voxels_per_second = min(self.min_voxels_per_second, vps)
            self.max_voxels_per_second = max(self.max_voxels_per_second, vps)
        ...
        self.min_voxels_per_second = round(
            self.min_voxels_per_second) if not math.isinf(
                self.min_voxels_per_second) else 0
        self.max_voxels_per_second = round(self.max_voxels_per_second)
