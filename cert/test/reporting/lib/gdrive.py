...
class GoogleDriveRelatedError(Exception):
    """Exception raised when some intended access to Google Drive failed.
    """
...
class SummaryInfo:
    ...
    def __init__(self, path_to_summary: Path):
        ...
        if metadata is None or len(metadata.groups()) != 2:
            raise GoogleDriveRelatedError(
                f"{path_to_summary.stem} doesn't follow the pattern "
                "summary_YYYYMMDD-HHMMSS_test_name.")
    ...
    class GDriveImpl:
        ...
        def __get_google_auth_token(self) -> GoogleAuth:
            ...
            try:
                ...
                raise GoogleDriveRelatedError(repr(error))

            return gauth_token

        def get_file_id(self, file_name: str, parent_id: str = None) \
            -> Union[str, type(None)]:
        ...
        def __upload_file(self, file_to_upload: GoogleDriveFile) -> str:
            """Performs the upload to Google Drive.

            Args:
                file_to_upload: file metada.

            Returns: the uploaded file id, or an exception is raised.
            """
            try:
                file_to_upload.Upload(param={'supportsTeamDrives': True})
                file_id = file_to_upload["id"]
            except ApiRequestError as error:
                raise GoogleDriveRelatedError(f"{repr(error)}")
            return file_id
        ...
        def create_folder_if_nonexistent(self,
            ...
            if folder_id is None:
                ...
                folder_id = self.__upload_file(folder)
            return folder_id
        ...
        def upload_file(self,
            ...
            file_to_upload = self.api.CreateFile({
                "title":
                    file_name,
                "parents": [{
                    "kind": "drive#fileLink",
            ...
            file_to_upload.SetContentFile(str(file_path))
            return self.__upload_file(file_to_upload)

        def fetch_file(self, file_id: str
                      ) -> Union[GoogleDriveFile, type(None)]:
            """Retrieves the file metadata for a given id.

            Args:
                file_id: a Google Drive file id.

            Returns: the file metadata or None if file_id is non-existent.
            """
            return self.api.CreateFile({"id": file_id})
        ...
        def delete_file(self, file_id: str) -> bool:
            ...
            """
            file_to_delete = self.fetch_file(file_id)
            if file_to_delete is None:
                return False
            try:
                file_to_delete.Delete(param={'supportsTeamDrives': True})
            except ApiRequestError as error:
                raise GoogleDriveRelatedError(f"{repr(error)}")
