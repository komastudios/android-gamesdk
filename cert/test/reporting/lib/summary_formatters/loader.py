...
def __generate_summary(report_files_dir: Path, format: str,
                       figure_dpi: int) -> Path:
    """
    ...
    report_summary_file_name = \
        f"summary_{get_indexable_utc()}_{str(report_files_dir.stem)}.{format}"
    report_summary_file = report_files_dir.joinpath(report_summary_file_name)

    summary_formatter = __create_summary_formatter(format)
