...
# TODO(baxtermichael@google.com): See if we can remove the Suite concept to
...
def normalize_report_name(report_file: Path) -> Path:
    ...
    while renamed_report_file.exists():
        renamed_report_file = report_file.with_name(
            f"{device_info.brand}_{device_info.model}"\
