...
class GpuThread {
 ...
 private:
    ...
    bool running_;
 public:
    GpuThread(duration fenceTimeout) : fence_(NO_SYNC), fenceTimeout_(fenceTimeout), waitTime_(0), running_(true) {
...
class Simulator {
  public:
    ...
    Simulator(const SwappyCommonSettings& settings, const Workload& workload,
            ...
            commonBase_(new SwappyCommon(settings)),
            workload_(workload),
            schedule_(schedule),
            tracers_ {
                preWaitTracer,
 ...
 private:
    std::unique_ptr<SwappyCommon> commonBase_;
    Workload workload_;
