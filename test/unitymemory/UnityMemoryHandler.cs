...
public class UnityMemoryHandler : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        _memoryAdvisor =
            new AndroidJavaObject("com.google.android.apps.internal.games.memoryadvice.MemoryAdvisor", context);

        JSONObject deviceInfo = ConvertJsonObject(_memoryAdvisor.Call<AndroidJavaObject>("getDeviceInfo", context));
        // deviceInfo is a C# object that can be inspected, or logged.
    }
    
    void Update()
    {
        // This example calls the advisor every frame. Because of the cost (~5 to 20 milliseconds) clients may
        // choose to poll less frequently.
        
        AndroidJavaObject adviceNative = _memoryAdvisor.Call<AndroidJavaObject>("getAdvice");
        // Memory advice is obtained as a native Java object.
        // This can be passed back into the library to get simple memory advice. 
        // Or, use ConvertJsonObject(adviceNative) to get a C# object that can be inspected for more detail.
        ...
        long availabilityEstimate = _memoryAdvisor.CallStatic<long>("availabilityEstimate", adviceNative);
        // availabilityEstimate contains an estimate in bytes of memory that could be safely allocated.
    }
