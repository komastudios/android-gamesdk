...
public class FirebaseMemoryTest : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        using (StreamReader r = new StreamReader(File.OpenRead("/sdcard/params.json")))
        {
            ...
            {
                first["build"] = ConvertJsonObject(helper.CallStatic<AndroidJavaObject>("getBuild"));
            }
    ...
    void Update()
    {
        ...
        JSONObject report = ConvertJsonObject(_info.Call<AndroidJavaObject>("getMemoryMetrics", context));
