...
The library is published on Google's Maven repository [GMaven](https://maven.google.com/web/index.html?q=com.google.android.games#com.google.android.games:memory-advice:0.13).

In the application root `build.gradle` file, ensure ```google()`` is specified as a repository for the project, as well as `jitpack.io` for some of its dependencies. For example:
...
and build locally with `gradle publishToMavenLocal` and use of `mavenLocal()` in `repositories`.
