...
void CheckString(const std::string& s) {
    ...
    ClearcutSerializer::writeString(&stream, &com_google_tuningfork_StringTest_fields[field_num-1], (void *const*)&value_ptr);
...
void CheckCpuFreqs(const std::vector<uint64_t>& arr) {
    ...
    ClearcutSerializer::writeCpuFreqs(&stream, &com_google_tuningfork_I64ArrTest_fields[field_num-1], (void *const*)&value_ptr);
...
void CheckAnnotation(const std::vector<uint8_t>& ann) {
    ...
    ClearcutSerializer::writeAnnotation(&stream, &com_google_tuningfork_BytesTest_fields[field_num-1], (void *const*)&value_ptr);
...
void CheckFPs(const std::vector<uint8_t>& fps) {
    ...
    const int field_num = com_google_tuningfork_BytesTest_value_tag;
    ClearcutSerializer::writeFidelityParams(&stream, &com_google_tuningfork_BytesTest_fields[field_num-1], (void *const*)&value_ptr);
...
void CheckHistogramCountsValidated(const Histogram& h, const std::vector<uint32_t>& cnts) {
    ...
    ClearcutSerializer::writeCountArray(&stream, &logs_proto_tuningfork_TuningForkHistogram_fields[field_num-1], (void *const*)&value_ptr);
...
void CheckDeviceInfo(const ExtraUploadInfo& info) {
    ...
    ClearcutSerializer::writeDeviceInfo(&stream, &logs_proto_tuningfork_TuningForkLogEvent_fields[field_num-1], (void *const*)&value_ptr);
    ...
    EXPECT_EQ(info.total_memory_bytes, evt.device_info().total_memory_bytes()) << "total_memory_bytes";
}
