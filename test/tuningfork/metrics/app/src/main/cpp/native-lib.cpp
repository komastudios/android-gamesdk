...
uint64_t getNativeHeapAllocatedSize(JNIEnv* env) {
    jclass android_os_debug = env->FindClass("android/os/Debug");
    jmethodID method =env->GetStaticMethodID(android_os_debug, "getNativeHeapAllocatedSize", "()J" );
    if (method!=NULL) {
...
Java_test_tuningfork_metricstest_MainActivity_stringFromJNI(
        JNIEnv* env,
        jobject /* this */) {
    std::stringstream ss;
    MemInfo memInfo;
    ...
    for (size_t i = 0; i < LOOP_TIMES; i++) {
        heap = getNativeHeapAllocatedSize(env);
    ...
    uint32_t avg_microseconds =
            std::chrono::duration_cast<std::chrono::microseconds>((endTime - startTime) / LOOP_TIMES).count();
    ALOGI("Ending metrics collection. Average time for %zu iterations: %u us", LOOP_TIMES, avg_microseconds);

    ss << "Average time: " << avg_microseconds << " us (" << LOOP_TIMES << " iterations)" << std::endl << std::endl;
    ss << "Values for last iteration:" << std::endl;
    ss << "getNativeHeapAllocatedSize: " << heap << std::endl;
