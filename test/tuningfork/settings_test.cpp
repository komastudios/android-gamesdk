...
bool operator==(const tf::Settings::Histogram& a, const tf::Settings::Histogram& b) {
    return a.bucket_max==b.bucket_max && a.bucket_min==b.bucket_min && a.n_buckets == b.n_buckets && a.instrument_key == b.instrument_key;
}
...
TEST(SettingsTest, Deserialize) {
    ...
    Settings_AggregationStrategy_Submission method = Settings_AggregationStrategy_Submission_TICK_BASED;
    ...
    aggregation_strategy->set_max_instrumentation_keys(max_instrumentation_keys);
    ...
    EXPECT_EQ(settings.aggregation_strategy.method, tf::Settings::AggregationStrategy::Submission::TICK_BASED);
    EXPECT_EQ(settings.aggregation_strategy.intervalms_or_count, intervalms_or_count);
    EXPECT_EQ(settings.aggregation_strategy.max_instrumentation_keys, max_instrumentation_keys);
    ASSERT_EQ(settings.aggregation_strategy.annotation_enum_size.size(), 3);
    int ix = 0;
    for(auto i: annotation_enum_sizes) {
        EXPECT_TRUE(settings.aggregation_strategy.annotation_enum_size[ix++] == i);
    }
    ASSERT_EQ(settings.histograms.size(),1);
    // For some reason, the operator== doesn't work in EXPECT_EQ, so use EXPECT_TRUE
