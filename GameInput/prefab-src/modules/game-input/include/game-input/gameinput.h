 ...
 * compose a glyph or word. Use functions GameInput_getState and
 * GameInput_setState to read and modify the state that an IME is editing.
 */
 ...
 */
void GameInputState_construct(GameInputState *state, const char *text_UTF8,
 ...
 * FindClass("com/google/androidgamesdk/gameinput/State") or equivalent.
 * @param state Input state to convert.
 * @return A Java object of class gameinput.State.
 */
jobject GameInputState_ToJava(JNIEnv *env, jclass stateClass,
                              const GameInputState *state);

/**
 * Convert from a Java gameinput.State object into a C GameInputState struct.
 * You are responsible for calling GameInputState_destruct on the returned
 * value.
 * @param env A JNI env valid on the calling thread.
 * @param stateClass A Java class obtained using
 * FindClass("com/google/androidgamesdk/gameinput/State") or equivalent.
 ...
 */
GameInputState GameInputState_FromJava(JNIEnv *env, jclass stateClass,
                                       jobject state);
...
/**
 * Get the current GameInput state. The state is modified by changes in the IME
 * and calls to GameInput_setState.
 * @param input A valid GameInput library handle.
 ...
 * gameinput.InputConnection, unless you are using GameActivity.
 * @param input A valid GameInput library handle.
...
// TODO (willosborn): can we avoid this by registering a native method with
// InputConnection directly?
/**
 * Call this function from your Java gameinput.Listener.stateChanged method to
