...
static const uint64_t MEMORY_CALLBACK_PERIOD = (5*1000);
...
MemoryConsumer::MemoryConsumer(bool startAsActive) {
    ...
    MemoryAdvice_ErrorCode error = MemoryAdvice_registerWatcher(MEMORY_CALLBACK_PERIOD,
...
void MemoryConsumer::Update() {
    if (mActive) {
        ...
        if (currentTimestamp - mPercentageAvailableCheckTimestamp > MEMORY_CALLBACK_PERIOD) {
