...
public class PGSManager {
    ...
    private final boolean mNewSnapshotPolicy = true;
    ...
    private Task<Void> loadSnapshot() {
        ...
        return mSnapshotClient.open(mSnapshotName, mNewSnapshotPolicy, mConflictResolutionPolicy)
    ...
    private Task<DataOrConflict<Snapshot>> saveSnapshot(byte[] data) {
        // Open Snapshot using its name
        return mSnapshotClient.open(mSnapshotName, mNewSnapshotPolicy, mConflictResolutionPolicy)
            .addOnFailureListener(new OnFailureListener() {
                @Override
