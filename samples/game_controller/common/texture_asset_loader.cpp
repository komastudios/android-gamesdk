...
namespace {
    ...
    TextureAssetHandle
    loadTextureAsset_LIBPNG(const char *filename, uint32_t *textureWidth, uint32_t *textureHeight) {
        TextureAssetHandle returnHandle = TextureAssetLoader::INVALID_TEXTURE;
                        ...
                        if (pngStruct != nullptr) {
                            ...
                            if (pngInfo != nullptr) {
                                ...
                                if (png_get_IHDR(pngStruct, pngInfo, &pngWidth, &pngHeight,
                                                 &pngDepth, &pngColorType, NULL, NULL, NULL) == 1) {
                                    ...
                                    if (pngWidth > 0 && pngHeight > 0 && pngDepth == 8 &&
                                        ...
                                        for (png_uint_32 yIndex = 0; yIndex < pngHeight; ++yIndex) {
                                            png_read_row(pngStruct,
                                                         static_cast<png_bytep>(rowPixels), NULL);
                                            const uint8_t *rowPixel = static_cast<const uint8_t *>(rowPixels);
...
#if defined USE_IMAGE_DECODER
    TextureAssetHandle loadTextureAsset_ID(const char *filename, uint32_t *textureWidth,
                                     uint32_t *textureHeight) {
        TextureAssetHandle returnHandle = TextureAssetLoader::INVALID_TEXTURE;
        if (assetManager != nullptr && filename != nullptr) {
            AAsset *textureAsset = AAssetManager_open(assetManager, filename, AASSET_MODE_STREAMING);
            if (textureAsset != nullptr) {
                AImageDecoder *decoder = nullptr;
                int result = AImageDecoder_createFromAAsset(textureAsset, &decoder);
                if (result == ANDROID_IMAGE_DECODER_SUCCESS) {
                    const AImageDecoderHeaderInfo *info = AImageDecoder_getHeaderInfo(decoder);
                    const AndroidBitmapFormat texFormat = static_cast<const AndroidBitmapFormat>(AImageDecoderHeaderInfo_getAndroidBitmapFormat(
                            info));
                    if (texFormat == ANDROID_BITMAP_FORMAT_RGBA_8888) {
                        const int32_t texWidth = AImageDecoderHeaderInfo_getWidth(info);
                        const int32_t texHeight = AImageDecoderHeaderInfo_getHeight(info);
                        const size_t texStride = AImageDecoder_getMinimumStride(decoder);
                        const size_t texSize = texHeight * texStride;
                        void *texPixels = malloc(texSize);

                        result = AImageDecoder_decodeImage(decoder, texPixels, texStride, texSize);
                        if (result == ANDROID_IMAGE_DECODER_SUCCESS) {
                            returnHandle = createTexture(texWidth, texHeight, texPixels);
                            if (returnHandle != TextureAssetLoader::INVALID_TEXTURE) {
                                if (textureWidth != nullptr) {
                            ...
                            ALOGE("AImageDecoder_decodeImage failed on %s with result %d", filename,
                                  result);
                        }
                        free(texPixels);
                    } else {
                        ALOGE("Texture not in RGBA8888");
                    }
                    AImageDecoder_delete(decoder);
                } else {
                    ALOGE("AImageDecoder_createFromAAsset failed with result %d", result);
                }
                AAsset_close(textureAsset);
            } else {
                ALOGE("Failed to open asset %s", filename);
            }
        }
        return returnHandle;
    }
...
#if defined USE_IMAGE_DECODER
    return loadTextureAsset_ID(filename, textureWidth, textureHeight);
#else
