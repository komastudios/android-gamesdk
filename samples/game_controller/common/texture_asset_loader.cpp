...
namespace {
    ...
    TextureAssetHandle loadTextureAsset_ID(const char *filename, uint32_t *textureWidth,
        ...
        TextureAssetHandle returnHandle = TextureAssetLoader::INVALID_TEXTURE;
        if (assetManager != nullptr && filename != nullptr) {
            AAsset *textureAsset = AAssetManager_open(assetManager,
            ...
            if (textureAsset != nullptr) {
                ...
                if (result == ANDROID_IMAGE_DECODER_SUCCESS) {
                    ...
                    const AndroidBitmapFormat texFormat =
                        static_cast<const AndroidBitmapFormat>(
                    ...
                    if (texFormat == ANDROID_BITMAP_FORMAT_RGBA_8888) {
                        ...
                        if (result == ANDROID_IMAGE_DECODER_SUCCESS) {
                            ...
                            ALOGE("AImageDecoder_decodeImage failed on %s with result %d",
                                  filename, result);
                        }
                        free(texPixels);
                    } else {
