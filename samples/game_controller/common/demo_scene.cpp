...
void DemoScene::RenderControllerTabs() {
    if (ImGui::BeginTabBar("ControllerTabBar", ImGuiTabBarFlags_NoTooltip)) {
        for (size_t index = 0; index < PADDLEBOAT_MAX_CONTROLLERS; ++index) {
            ...
            const ImVec4 tabTextColor = mActiveControllers[index] ? TEXTCOLOR_WHITE : TEXTCOLOR_GREY;
...
void DemoScene::RenderControllerPanel_InfoTab(const int32_t controllerIndex,
    ...
    ImVec4 deviceColor = ((controllerInfo.controllerFlags & PADDLEBOAT_CONTROLLER_FLAG_GENERIC_PROFILE) != 0)
...
void DemoScene::RenderControllerPanel_VibrationTab(const int32_t controllerIndex,
    ...
    if ((controllerInfo.controllerFlags & PADDLEBOAT_CONTROLLER_FLAG_VIBRATION) != 0) {
        if ((controllerInfo.controllerFlags & PADDLEBOAT_CONTROLLER_FLAG_VIBRATION_DUAL_MOTOR) != 0) {
