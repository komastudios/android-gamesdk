...
namespace {
    ...
    ImVec2
    GetScaledTexureSize(const ControllerUIPanelParams &panelParams, const UITextureInfo &texInfo) {
...
void ControllerUIUtil::Button(const ControllerUIPanelParams &panelParams,
    ...
    if (buttonTextures.textureHandles[buttonInfo.buttonState] !=
        ...
        const ImVec2 buttonTextureSize = GetScaledTexureSize(panelParams, buttonTextures);
...
void ControllerUIUtil::Thumbstick(const ControllerUIPanelParams &panelParams, const ImVec2 &basePos,
    ...
    if (stickRegionTexture.textureHandles[0] != TextureAssetLoader::INVALID_TEXTURE) {
        ...
        const ImVec2 stickRegionTextureSize = GetScaledTexureSize(panelParams, stickRegionTexture);
    ...
    if (stickTextures.textureHandles[stickState] != TextureAssetLoader::INVALID_TEXTURE) {
        const ImVec2 stickTextureSize = GetScaledTexureSize(panelParams, stickTextures);
...
void ControllerUIUtil::TriggerBar(const ControllerUIPanelParams &panelParams, ImDrawList *draw_list,
                                  const ControllerUIButtons buttonId,
