...
Java_com_prefabulated_bouncyball_OrbitActivity_nGetSwappyStats(JNIEnv * /* env */, jobject /* this */,
                                                                jint stat, jint bin) {
    static bool enabled = false;
    if (!enabled) {
        Swappy_setStatsMode(true);
        enabled = true;
    }
    ...
    if (stats.totalFrames) {
        switch (stat) {
            ...
            case 1:
                ...
                break;
            case 2:
                value = stats.offsetFromPrevFrame[bin];
                break;
            case 3:
        ...
        value = std::round((double)value * 100.0f / (double)stats.totalFrames);
    }
