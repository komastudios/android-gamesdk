...
void getMatrices(std::array<GLfloat, 16>& mvpMatrix, std::array<GLfloat, 16>& normalMatrix, GLfloat aspectRatio, Vector3 pos, Quaternion rot) {
...
int Sphere::numVerticesForWorkload(int workload, int& nPhi, int& nTheta) {
    ...
    nPhi = (nPhi/2)*2; // Make divisible by 2
...
void Sphere::draw(float aspectRatio, const std::vector<Sphere> &spheres, int workload) {
    ...
    for (auto &sphere : spheres) {
        ...
        std::array<float,16> mvpMatrix, normalMatrix;
...
std::vector<GLfloat> &Sphere::getVertices(int workload) {
    static auto vertices = initializeVertices(workload);
    static int prev = workload;
    if (prev != workload) {
...
void Scene::draw(float aspectRatio, int workload) {
    ...
    Sphere::draw(aspectRatio, spheres, workload);
}
