...
void Renderer::draw(State *state) {
    ...
    if (state->lastUpdate - std::chrono::steady_clock::now() <= 100ms) {
        deltaSeconds = (state->lastUpdate - std::chrono::steady_clock::now()).count() / 1e9f;
    }
    ...
    state->scene.draw(aspectRatio, mWorkload);
