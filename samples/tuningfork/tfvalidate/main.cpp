...
int validateApk(const char *apk_path, const std::string asset_path,
                bool enforce_enums_in_annotations, bool debug) {
#undef open
...
int main(int argc, char **argv) {
  ...
  constexpr char usage_string[] =
      "Usage: tfvalidate [-a <asset_path_in_apk>] [-N] [-d] <apk_path>";
  int opt;
  optind = 1;
  while((opt=getopt(argc, argv, "+a:Nd"))!=-1){
    switch(opt) {
      case 'a':
        asset_path = optarg;
        if(asset_path.size()>0 && asset_path.back()!='/') asset_path += '/';
        break;
      case 'N':
  ...
  if(optind<argc) {
    for(int i=optind; i<argc; ++i) {
      int ret = validateApk(argv[i], asset_path, enforce_enums_in_annotations, debug);
      if(ret!=NO_ERROR) {
