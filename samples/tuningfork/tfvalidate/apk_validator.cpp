...
int kMaxInstrumentationKeys = 16;
...
int ApkValidator::Validate(bool enforce_enums_in_annotations) {
  ...
  if(fpok!=NO_ERROR) ERROR(fpok, "Error in FidelityParams");
  ...
  return ValidateSettings(sdesc, def_ann_enum_size);
}
...
int ApkValidator::ValidateSettings(const pb::Descriptor* sdesc,
  ...
  std::unique_ptr<pb::io::ArrayInputStream> settings_bin(
      dynamic_cast<pb::io::ArrayInputStream*>(
          source_tree_.Open("tuningfork_settings.bin")));
  if(!settings_bin)
  ...
  for(int i=0;i<ann_enum_size.size();++i) {
    if(debug_)
      std::cout << "Annotation enum size: "
                << ann_enum_size[i] << " "
                << def_ann_enum_size[i] << std::endl;
