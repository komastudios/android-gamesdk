...
void NativeEngine::GameLoop() {
    ...
    while (1) {
        ...
        // TODO(florianrival): should be reworked with a real event structure (and potentially mutexes)
        if (mApp->motionEventsCount != 0) {
            for(uint64_t i = 0;i<mApp->motionEventsCount;++i) {
                ...
                uint64_t duration = currentTimeSpec.tv_sec * 1000 + (currentTimeSpec.tv_nsec  - motionEvent->eventTime)/1000000;
                LOGD("Duration before getting motion in render thread %" PRIu64 "ms\n", duration);

                // TODO(@florianrival): this is a partial, adapted copy of the `CookEvent_Motion` function.
                ...
                ev.motionX = GameActivityPointerInfo_getX(&motionEvent->pointers[ptrIndex]);
                ev.motionY = GameActivityPointerInfo_getY(&motionEvent->pointers[ptrIndex]);
...
JNIEnv *NativeEngine::GetAppJniEnv() {
    if (!mAppJniEnv) {
        ...
        LOGD("Attached current thread to JNI, %p", mAppJniEnv);
    }
