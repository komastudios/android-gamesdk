...
static std::string RequestJson(const RequestInfo& request_info,
    ...
    Json::object request_obj = Json::object{
        ...
        {"target_frame_time", JsonSerializer::DurationJsonFromNanos(
                                  1000000 * target_frame_time_ms)}};
...
static TuningFork_ErrorCode DecodeResponse(const std::string& response,
    ...
    if (!err.empty()) {
        ...
        return TUNINGFORK_ERROR_PREDICT_QUALITY_LEVELS_ERROR;
    }
    ...
    if (quality_levels.empty()) {
        ALOGE("No quality levels returned from server");
        return TUNINGFORK_ERROR_PREDICT_QUALITY_LEVELS_ERROR;
    }
