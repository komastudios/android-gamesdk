...
enum Paddleboat_ControllerButtonLayout {
    ...
    PADDLEBOAT_CONTROLLER_LAYOUT_ARCADE_STICK = 3, ///< X Y R1 L1
                                                   ///< A B R2 L2
    PADDLEBOAT_CONTROLLER_LAYOUT_MASK = 3 ///< Mask value, AND with `Paddleboat_Controller_Info.controllerFlags`
...
enum Paddleboat_LightType {
    PADDLEBOAT_LIGHT_PLAYER_INDEX = 0, ///< Light is a player index, `lightData` is the player number
...
enum Paddleboat_MouseStatus {
    ...
    PADDLEBOAT_MOUSE_CONTROLLER_EMULATED = 1, ///< A virtual mouse is connected
                                              ///< The virtual mouse is being simulated by a game controller
...
enum Paddleboat_Remap_Addition_Mode {
    PADDLEBOAT_REMAP_ADD_MODE_DEFAULT = 0, ///< If a vendorId/productId controller entry in the new remap table
                                           ///< exists in the current database, if the min/max API
                                           ///< ranges overlap, replace the existing entry, otherwise
                                           ///< add a new entry. Always adds a new entry if the
                                           ///< vendorId/productId does not exist in the current database.
                                           ///< These additions only persist for the current application session.
    PADDLEBOAT_REMAP_ADD_MODE_REPLACE_ALL ///< The current controller database will be erased and entirely replaced
                                          ///< by the new remap table.
                                          ///< This change only persists for the current application session.
};
...
typedef struct Paddleboat_Controller_Data {
    uint32_t updateSerial; /** @brief Incremental serial value that changes when new data is read */
...
typedef struct Paddleboat_Controller_Info {
    ...
    int32_t controllerNumber; /** @brief Controller number, maps to InputDevice.getControllerNumber() */
...
typedef struct Paddleboat_Vibration_Data {
    ...
    float intensityLeft; /** @brief Intensity of vibration of left motor, valid range is 0.0 to 1.0 */
    float intensityRight; /** @brief Intensity of vibration of right motor, valid range is 0.0 to 1.0 */
...
typedef struct Paddleboat_Controller_Mapping_Data {
    ...
    int16_t maximumEffectiveApiLevel; /** @brief Maximum API level required for this entry, 0 = no max */
    ...
    int32_t flags; /** @brief Flag bits, will be ORed with `Paddleboat_Controller_Info.controllerFlags`*/
    //
    ...
    uint16_t axisMapping[PADDLEBOAT_MAPPING_AXIS_COUNT]; /** @brief AMOTION_EVENT_AXIS value for the
                                                           * corresponding Paddleboat control axis,
                                                           * or PADDLEBOAT_AXIS_IGNORED if unsupported */
    //
    uint8_t axisPositiveButtonMapping[PADDLEBOAT_MAPPING_AXIS_COUNT]; /** @brief Button to set on positive axis value,
                                                                        * PADDLEBOAT_AXIS_BUTTON_IGNORED if none */
    uint8_t axisNegativeButtonMapping[PADDLEBOAT_MAPPING_AXIS_COUNT]; /** @brief Button to set on negative axis value,
                                                                        * PADDLEBOAT_AXIS_BUTTON_IGNORED if none */
    uint16_t buttonMapping[PADDLEBOAT_BUTTON_COUNT]; /** @brief AKEYCODE_ value corresponding
                                                       ...
                                                       * PADDLEBOAT_BUTTON_IGNORED if not supported. */
...
/**
 * @brief Processes an input event to see if it is from a device being
...
/**
 * @brief Processes a Game Activity intput event to see if it is from a
...
/**
 * @brief Retrieves the active axis ids being used by connected devices. This can be used
 * by the application to determine what axis values to provide to GameActivityPointerInfo_enableAxis
...
/**
 * @brief Sets whether Paddleboat consumes AKEYCODE_BACK key events from devices being
 ...
 * connected controller does not support the specified light.
 * at the specified index
 */
 bool Paddleboat_setControllerLight(const int32_t controllerIndex,
                                    const Paddleboat_LightType lightType, const uint32_t lightData);
...
/**
 * @brief Retrieves the current controller data from the controller with the specified index.
 * @param controllerIndex The index of the controller to read from, must be between
 * 0 and PADDLEBOAT_MAX_CONTROLLERS - 1
...
/**
 * @brief Retrieves the current controller device info from the controller with the specified index.
 * @param controllerIndex The index of the controller to read from, must be between
 * 0 and PADDLEBOAT_MAX_CONTROLLERS - 1
...
/**
 * @brief Sets vibration data for the controller with the specified index.
 * @param controllerIndex The index of the controller to read from, must be between
 * 0 and PADDLEBOAT_MAX_CONTROLLERS - 1
 ...
 * @param[out] mouseData a pointer to the mouse data struct to populate.
...
/**
 * @brief Retrieves the current table of controller remap entries.
 ...
 * @param[out] mappingData a pointer to an array of `Paddleboat_Controller_Mapping_Data` structures.
