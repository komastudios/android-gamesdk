...
namespace paddleboat {
    ...
    // set if axisMultiplier/axisAdjustment should be applied (raw device axis isn't -1.0 to 1.0)
    inline constexpr uint32_t GAMECONTROLLER_AXIS_FLAG_APPLY_ADJUSTMENTS = (1U << 0);
    // set if trigger is being faked as an analog axis (device just has a on/off button flag)
    ...
    public:
        ...
        struct GameControllerAxisInfo {
            ...
            // Button mask flag, if backed/shadowed by digital button when axis > 0.0
        ...
        int32_t processGameActivityKeyEvent(const Paddleboat_GameActivityKeyEvent *event,
                                            const size_t eventSize);

        int32_t processGameActivityMotionEvent(const Paddleboat_GameActivityMotionEvent *event,
                                               const size_t eventSize);
        ...
        Paddleboat_ControllerStatus getControllerStatus() const { return mControllerStatus; }
        ...
        void
        setConnectionIndex(const int32_t connectionIndex) { mConnectionIndex = connectionIndex; }
        ...
        const Paddleboat_Controller_Data &getControllerData() const { return mControllerData; }

        Paddleboat_Controller_Info &getControllerInfo() { return mControllerInfo; }

        const Paddleboat_Controller_Info &getControllerInfo() const { return mControllerInfo; }

        GameControllerDeviceInfo &getDeviceInfo() { return mDeviceInfo; }

        const GameControllerDeviceInfo &getDeviceInfo() const { return mDeviceInfo; }
    ...
    private:
        int32_t processKeyEventInternal(const int32_t eventKeyCode, const int32_t eventKeyAction);

        int32_t processMotionEventInternal(const float *axisArray, const AInputEvent *event);

        void setupAxis(const GameControllerAxis gcAxis, const int32_t preferredNativeAxisId,
        ...
        Paddleboat_ControllerStatus mControllerStatus = PADDLEBOAT_CONTROLLER_INACTIVE;
