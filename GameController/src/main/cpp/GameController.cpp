...
namespace paddleboat {
    ...
    void GameController::setupController(const Paddleboat_Controller_Mapping_Data *mappingData) {
        ...
        if (mappingData != nullptr) {
            ...
            for (int32_t i = 0; i < PADDLEBOAT_MAPPING_AXIS_COUNT; ++i) {
                if (mappingData->axisMapping[i] != PADDLEBOAT_AXIS_IGNORED) {
                    ...
                    const int32_t positiveButton = (mappingData->axisPositiveButtonMapping[i] ==
                                                    PADDLEBOAT_AXIS_BUTTON_IGNORED) ? 0 : (1
                            << mappingData->axisPositiveButtonMapping[i]);
                    const int32_t negativeButton = (mappingData->axisNegativeButtonMapping[i] ==
                                                    PADDLEBOAT_AXIS_BUTTON_IGNORED) ? 0 : (1
    ...
    void
    GameController::setupAxis(const GameControllerAxis gcAxis, const int32_t preferredNativeAxisId,
        ...
        if (preferredNativeAxisId < MAX_AXIS_COUNT && secondaryNativeAxisId < MAX_AXIS_COUNT) {
            ...
            // If we found a native axis, check its ranges and see if we need to set up
            // adjustment values if the ranges aren't -1.0 to 1.0 for a stick or 0.0 to 1.0
            // for a trigger
                ...
                if (!(diffMin <= FLT_MIN && diffMax <= FLT_MIN)) {
                    ...
                    if (isStickAxis) {
                        ...
                        if (rawCenter >= FLT_MIN) {
                            mAxisInfo[gcAxis].axisAdjust = -(rawCenter *
                                                             mAxisInfo[gcAxis].axisMultiplier);
                        }
                    } else {
                        // This case is hit on PS5 API <= 30 having weird trigger axis mappings of:
                        // L2=RX R2=RY with a -1.0 to 1.0 range
    ...
    int32_t
    GameController::processGameActivityKeyEvent(const Paddleboat_GameActivityKeyEvent *event,
    ...
    int32_t
    GameController::processMotionEventInternal(const float *axisArray, const AInputEvent *event) {
        int32_t handledEvent = IGNORED_EVENT;
        ...
        for (uint32_t axis = GAMECONTROLLER_AXIS_LSTICK_X;
            ...
            if (mAxisInfo[axis].axisIndex >= 0 &&
                ...
                // We take advantage of the GameControllerAxis matching the axis order in the
                // Paddleboat_Controller_Data struct to use the current axis as an index into
                // the axis entries in the Paddleboat_Controller_Data struct
                ...
                if (mAxisInfo[axis].axisButtonMask != 0 || mAxisInfo[axis].axisButtonNegativeMask) {
                    if (axisValue > -AXIS_BUTTON_THRESHOLD && axisValue < AXIS_BUTTON_THRESHOLD) {
                        const uint32_t buttonMask = mAxisInfo[axis].axisButtonMask |
