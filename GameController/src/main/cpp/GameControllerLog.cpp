...
namespace paddleboat {
    const char *paddleboat_LogGetInputSourceString(const int32_t eventSource) {
    ...
    void paddleboat_LogInputEvent(const AInputEvent *event) {
        const int32_t eventSource = AInputEvent_getSource(event);
#if defined LOG_FILTER_PADDLEBOAT_SOURCES
        if (!(eventSource == AINPUT_SOURCE_DPAD || eventSource == AINPUT_SOURCE_GAMEPAD ||
        ...
        const char *inputSourceString = paddleboat_LogGetInputSourceString(eventSource);
        ...
        if (eventType == AINPUT_EVENT_TYPE_KEY) {
            ...
            const char *actionString = (eventAction < ELEMENTS_OF(AKEY_ACTION_STRINGS) &&
                                        eventAction >= 0) ? AKEY_ACTION_STRINGS[eventAction]
                                                          : "AKEY_ACTION out of range";
            const char *keycodeString = (eventKeycode < ELEMENTS_OF(AKEYCODE_STRINGS) &&
                                         eventKeycode >= 0) ? AKEYCODE_STRINGS[eventKeycode]
                                                            : "AKEYCODE out of range";
            ALOGI("LogInputEvent\nAINPUT_EVENT_TYPE_KEY deviceId %d source %s\n%s %s %08x",
                  eventDeviceId, inputSourceString, actionString, keycodeString, eventFlags);
        } else if (eventType == AINPUT_EVENT_TYPE_MOTION) {
            const int32_t eventAction = AMotionEvent_getAction(event) & AMOTION_EVENT_ACTION_MASK;
            const int32_t eventFlags = AMotionEvent_getFlags(event);
            const char *actionString = (eventAction < ELEMENTS_OF(AMOTION_ACTION_STRINGS) &&
                                        eventAction >= 0) ? AMOTION_ACTION_STRINGS[eventAction]
