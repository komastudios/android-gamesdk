...
namespace paddleboat {
    // GameActivity needs to use its own event data structures for key and motion events.
    // We do not want to have a dependency on GameActivity. Unfortunately, this means we
    // need to internally mirror the relevant structures.
    // Paddleboat_processGameActivityInputEvent includes a struct size parameter, and we
    // know that the GameActivity structures will only ever add fields, so we can
    // determine if we are being passed a later version of the struct than the mirrored
    // internal version.
