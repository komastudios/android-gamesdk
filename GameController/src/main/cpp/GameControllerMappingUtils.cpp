...
namespace paddleboat {
    MappingTableSearch::MappingTableSearch()
            : mappingRoot(nullptr), vendorId(0), productId(0), minApi(0), maxApi(0), tableIndex(0),
              mapEntryCount(0), tableEntryCount(0),
              tableMaxEntryCount(GameControllerManager::getRemapTableSize()) {

    }

    MappingTableSearch::MappingTableSearch(Paddleboat_Controller_Mapping_Data *mapRoot,
                                           int32_t entryCount)
            : mappingRoot(mapRoot), vendorId(0), productId(0), minApi(0), maxApi(0), tableIndex(0),
              mapEntryCount(0), tableEntryCount(entryCount),
              tableMaxEntryCount(GameControllerManager::getRemapTableSize()) {

    }

    void
    MappingTableSearch::initSearchParameters(const int32_t newVendorId, const int32_t newProductId,
    ...
    bool GameControllerMappingUtils::findMatchingMapEntry(MappingTableSearch *searchEntry) {
        ...
        // Starting out with a linear search. Updating the map table is something that should only
        // ever be done once at startup, if it actually takes an appreciable time to execute
        // when working with a big remap dictionary, this is low-hanging fruit to optimize.
        const Paddleboat_Controller_Mapping_Data *mapRoot = searchEntry->mappingRoot;
        while (currentIndex < searchEntry->tableEntryCount) {
            const Paddleboat_Controller_Mapping_Data &mapEntry = mapRoot[currentIndex];
            if (mapEntry.vendorId > searchEntry->vendorId) {
                // Passed by the search vendorId value, so we don't already exist in the table,
                // set the current index as the insert point and bail
                ...
                if (mapEntry.productId > searchEntry->productId) {
                    // Passed by the search productId value, so we don't already exist in the table,
                    // set the current index as the insert point and bail
                    searchEntry->tableIndex = currentIndex;
                    return false;
                } else if (searchEntry->productId == mapEntry.productId) {
                    // Any overlap of the min/max API range is treated as matching an existing entry
    ...
    bool GameControllerMappingUtils::insertMapEntry(
        ...
        if (searchEntry->tableEntryCount < searchEntry->tableMaxEntryCount &&
            ...
            // Empty table, or inserting at the end, no relocation of existing data required,
            // otherwise shift existing data down starting at the insert index.
    ...
    const Paddleboat_Controller_Mapping_Data *GameControllerMappingUtils::validateMapTable(
            const Paddleboat_Controller_Mapping_Data *mappingRoot, const int32_t tableEntryCount) {
        // The map table is always assumed to be sorted by increasing vendorId. Each sequence
        // of entries with the same vendorId are sorted by increasing productId. Multiple entries
        // with the same vendorId/productId range are sorted by increasing min/max API ranges.
        // vendorId
        //   productId
        //      minApi
        int32_t currentIndex = 0;
