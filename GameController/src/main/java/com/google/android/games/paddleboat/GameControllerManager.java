...
public class GameControllerManager {
    ...
    private final ArrayList<GameControllerListener> gameControllerListeners;
    ...
    public GameControllerManager(Context appContext, boolean appPrintControllerInfo) {
        ...
        gameControllerListeners = new ArrayList<GameControllerListener>(MAX_GAMECONTROLLERS);
            ...
            if (batteryState != null) {
                if (batteryState.isPresent()) {
                    ...
                    String batteryString =
                            String.format("DEVICEFLAG_BATTERY %.2f", batteryState.getCapacity());
                    Log.d(TAG, batteryString);
                }
            }
        }
    ...
    GameControllerInfo onGameControllerAdded(int deviceId) {
        ...
        if (gameControllers.size() < MAX_GAMECONTROLLERS) {
            ...
            gameControllerListeners.add(gameControllerListener);
    ...
    void onGameControllerDeviceRemoved(int deviceId) {
        ...
        for (int index = 0; index < gameControllers.size(); ++index) {
            if (gameControllers.get(index).GetGameControllerDeviceId() == deviceId) {
                ...
                gameControllerListeners.get(index).shutdownListener();
                gameControllers.remove(index);
                gameControllerListeners.remove(index);
                break;
            }
        }
    }
            ...
            if (!foundDeviceId) {
                for (int index = 0; index < gameControllers.size(); ++index) {
                    if (gameControllers.get(index).GetGameControllerDeviceId() == deviceId) {
                        ...
                        int controllerFlags = gameControllers.get(index).GetGameControllerFlags();
                        GameControllerListener controllerListener =
                                gameControllerListeners.get(index);
                        controllerListener.resetListener(inputDevice, controllerFlags);
                        break;
                    }
                }
            }
    ...
    public void setReportMotionEvents() {
        ...
        for (GameControllerListener listener : gameControllerListeners) {
            listener.setReportMotionEvents();
        }
    }
    ...
    public void setLight(int deviceId, int lightType, int lightValue) {
        for (int index = 0; index < gameControllers.size(); ++index) {
            if (gameControllers.get(index).GetGameControllerDeviceId() == deviceId) {
                GameControllerListener controllerListener = gameControllerListeners.get(index);
                controllerListener.setLight(lightType, lightValue);
                break;
            }
        }
    }
