...
public class GameControllerManager {
    ...
    public GameControllerManager(Activity appActivity, String libraryName, boolean appPrintControllerInfo) {
        if (libraryName == null) {
    ...
    GameControllerInfo onGameControllerAdded(int deviceId, boolean hasVirtualMouse) {
        ...
        if (gameControllers.size() < MAX_GAMECONTROLLERS) {
            ...
            gameControllerInfo = new GameControllerInfo(InputDevice.getDevice(deviceId), hasVirtualMouse);
    ...
    void onMouseAdded(int deviceId) {
        if (mouseDeviceIds.size() < MAX_MICE) {
            if (printControllerInfo) {
                Log.d(TAG, "onMouseDeviceAdded id: " + deviceId + " name: " + InputDevice.getDevice(deviceId).getName());
                logControllerInfo(deviceId);
            }
    ...
    public void setNativeReady() {
        ...
        for (int deviceId : pendingControllerDeviceIds) {
            GameControllerInfo gcInfo = onGameControllerAdded(deviceId, isDeviceOfSource(deviceId, MOUSE_SOURCE_MASK));
            if (gcInfo != null) {
                if (printControllerInfo) {
        ...
        if (inputDevice != null) {
            ...
            if (isVibrationSupportedForDevice(deviceVibrator)) {
                if (leftIntensity == 0) {
                    ...
                    deviceVibrator.vibrate(VibrationEffect.createOneShot((long) leftDuration, leftIntensity));
                }
            }
        }
    }
