...
public class GameControllerInfo {
            ...
            if (axisIndex >= 0 && axisIndex < MAX_AXIS_COUNT) {
                ...
                if (axisSource == InputDevice.SOURCE_JOYSTICK || axisSource == InputDevice.SOURCE_GAMEPAD) {
                    if (axisIndex <= AXIS_COUNT_LOW) {
                        ...
                        mGameControllerDeviceInfoArray[DEVICEINFO_INDEX_AXISBITS_HIGH] |= (1 << (axisIndex - (AXIS_COUNT_LOW + 1)));
                    }
    ...
    private void EnumerateInfoArray(InputDevice inputDevice, boolean hasVirtualMouse) {
        ...
        mGameControllerDeviceInfoArray[DEVICEINFO_INDEX_CONTROLLERNUMBER] = inputDevice.getControllerNumber();
        if (hasVirtualMouse) {
