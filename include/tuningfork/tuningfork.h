...
// Internal macros to generate a symbol to track TuningFork version, do
// not use directly.
...
/// The instrumentation key identifies a tick point within a frame or a
/// trace segment
...
/**
 * @brief All the error codes that can be returned by Tuning Fork
 * functions.
 */
typedef enum TuningFork_ErrorCode {
    ...
    TUNINGFORK_ERROR_JNI_EXCEPTION = 15,      ///< Jni error - an exception was
                                          ///< thrown. See logcat output.
    ...
    TUNINGFORK_ERROR_DOWNLOAD_THREAD_ALREADY_STARTED =
        27,  ///< `TuningFork_startFidelityParamDownloadThread`
             ///< was called more than once, or called when
    ...
    TUNINGFORK_ERROR_GENERATE_TUNING_PARAMETERS_RESPONSE_NOT_SUCCESS =
        30,  ///< The response from generateTuningParameters was not a
             ///< success code
    TUNINGFORK_ERROR_NO_MORE_SPACE_FOR_LOADING_TIME_DATA =
        31,  ///< Not enough space for metric data was allocated at
             ///< start-up: increase
             ///< Settings.max_num_metrics.loading_time
    TUNINGFORK_ERROR_NO_MORE_SPACE_FOR_FRAME_TIME_DATA =
        32,  ///< Not enough space for metric data was allocated at
             ///< start-up:
    ...
    TUNINGFORK_ERROR_DUPLICATE_START_LOADING_EVENT =
        34,  ///< TuningFork_startRecordingLoadingTime was called with
             ///< the same parameters twice without a stop function
             ///< inbetween.
...
/**
 * @brief An object used to cache upload data when no connection is
 * available. If you do not supply one of these, data is saved to a
 * temporary file.
 */
 ...
 *
 * Fidelity parameters are serializations of FidelityParams messages
 * defined in a game's dev_tuningfork.proto file. The structure of this
 * message is completely up to the developer, with limitations outlined
...
typedef struct TuningFork_Settings {
     ...
     * If it is set, telemetry for 4 instrument keys is automatically
     * recorded.
     */
    SwappyTracerFn swappy_tracer_fn;
    /**
     * Callback
     * If set, this is called with the fidelity parameters that are
     * downloaded. If null, you need to call
     * TuningFork_getFidelityParameters yourself.
     */
    TuningFork_FidelityParamsCallback fidelity_params_callback;
    /**
     * A serialized protobuf containing the fidelity parameters to be
     * uploaded for training. Set this to nullptr if you are not using
     * training mode. In training mode, these parameters are taken to be
     * the parameters used within the game and they are used to help
     * suggest parameter changes for different devices. Note that these
     * override the default parameters loaded from the APK at startup.
     */
    const TuningFork_CProtobufSerialization* training_fidelity_params;
    /**
     * A null-terminated UTF-8 string containing the endpoint that
     * Tuning Fork will connect to for parameter, upload and debug
     * requests. This overrides the value in base_uri in the settings
     * proto and is intended for debugging purposes only.
     */
    ...
    /**
     * The number of each metric that is allowed to be allocated at any
     * given time. If any element is zero, the default for that metric
     * type will be used. Memory for all metrics is allocated up-front
     * at initialization. When all metrics of a given type are
     * allocated, further requested metrics will not be added and data
     * will be lost.
     */
...
/**
 * @brief Deallocate any memory owned by the procol buffer
 * serialization.
...
// Internal function to track TuningFork version bundled in a binary. Do
// not call directly. If you are getting linker errors related to
// TuningFork_version_x_y, you probably have a mismatch between the
// header used at compilation and the actually library used by the
// linker.
 ...
 * @see TuningFork_Settings for the semantics of how other settings
 ...
 * @return TUNINGFORK_ERROR_BAD_SETTINGS if your tuningfork_settings.bin
 * file was invalid or
 * @return TUNINGFORK_ERROR_ALREADY_INITIALIZED if tuningfork was
 * already initialized.
 */
static inline TuningFork_ErrorCode TuningFork_init(
    const TuningFork_Settings* settings, JNIEnv* env, jobject context) {
    // This call ensures that the header and the linked library are from
    // the same version (if not, a linker error will be triggered
    // because of an undefined symbol).
    TUNINGFORK_VERSION_SYMBOL();
    return TuningFork_init_internal(settings, env, context);
}

// The functions below will return
// TUNINGFORK_ERROR_TUNINGFORK_NOT_INITIALIZED if TuningFork_init
//  has not first been called.

/**
 * @brief A blocking call to get fidelity parameters from the server.
 * You do not need to call this if you pass in a
 * fidelity_params_callback as part of the settings to TuningFork_init.
 * Note that once fidelity parameters are downloaded, any timing
 * information is recorded as being associated with those parameters. If
 * you subsequently call GetFidelityParameters and a new set of
 * parameters is downloaded, any data that is already collected will be
 * submitted to the backend. Ownership of 'params' is transferred to the
 * caller, so they must call params->dealloc when they are done with it.
 * The parameter request is sent to:
 *  ${url_base}+'applications/'+package_name+'/apks/'+version_number+':generateTuningParameters'.
 * @param defaultParams these will be assumed current if no parameters
 * could be downloaded.
 * @param[out] params
 * @param timeout_ms time to wait before returning from this call when
 * no connection can be made. If zero or negative, the value in
 * Settings.initial_request_timeout_ms is used.
 * @return TUNINGFORK_ERROR_TIMEOUT if there was a timeout before params
 * could be downloaded.
 ...
 * @param annotation the protobuf serialization of the current
 * annotation.
 * @return TUNINGFORK_ERROR_INVALID_ANNOTATION if annotation is
 * inconsistent with the settings.
 * @return TUNINGFORK_ERROR_OK on success.
 */
...
/**
 * @brief Record a frame tick that will be associated with the
 * instrumentation key and the current annotation. NB: calling the tick
 * or trace functions from different threads is allowed, but a single
 ...
 * @return TUNINGFORK_ERROR_INVALID_INSTRUMENT_KEY if the instrument key
 * is invalid.
 * @return TUNINGFORK_ERROR_OK on success.
 */
...
/**
 * @brief Record a frame tick using an external time, rather than system
 * time.
 * @param key an instrument key
 * @see the reserved instrument keys above
 ...
 * @param handle this is a handle previously returned by
 * TuningFork_startTrace
 * @return TUNINGFORK_ERROR_INVALID_TRACE_HANDLE if the handle is
 * invalid.
 * @return TUNINGFORK_ERROR_OK on success.
 */
 ...
 * @return TUNINGFORK_ERROR_PREVIOUS_UPLOAD_PENDING if there is a
 * previous upload blocking this one.
 * @return TUNINGFORK_ERROR_UPLOAD_TOO_FREQUENT if less than a minute
 * has elapsed since the previous upload.
 */
TuningFork_ErrorCode TuningFork_flush();

/**
 * @brief Set a callback to be called on a separate thread every time
 * TuningFork performs an upload.
 * @param cbk
 * @return TUNINGFORK_ERROR_OK on success.
 * @return TUNINGFORK_ERROR_TUNINGFORK_NOT_INITIALIZED if Tuning Fork
 * wasn't initialized.
 */
 ...
 * This function overrides any parameters that have been downloaded if
 * in experiment mode. Use when, for instance, the player has manually
 * changed game quality  settings. This flushes (i.e. uploads) any data
 * associated with any previous parameters.
 ...
 * By default, memory telemetry recording is turned *off* at
 * initialization and currently the memory statistics are not shown in
 * the Play UI, so it is not recommended to enable.
 * @param enable If true, memory recording is enabled, if false it is
 * disabled.
 * @return TUNINGFORK_ERROR_OK on success.
 * @return TUNINGFORK_ERROR_TUNINGFORK_NOT_INITIALIZED if Tuning Fork
 * wasn't initialized.
...
typedef struct TuningFork_LoadingTimeMetadata {
    ...
    enum LoadingSource {
        ...
        PRE_ACTIVITY = 7,
        // Total time spent between process starting and first render
        // frame.
 ...
 * @param eventMetadataSize Size in bytes of the LoadingTimeMetadata
 *structure
 ...
 * @param eventMetadata A LoadingTimeMetadata structure.
 * @param eventMetadataSize Size in bytes of the LoadingTimeMetadata
 *structure (for versioning of the structure).
 * @param annotation The annotation to use with this event.
...
/**
 * @brief The set of states that the TuningFork_reportLifecycleEvent
 * method accepts.
 */
 ...
 * This method should be called every time the onCreate(), onStart(),
 * onStop() and onDestroy() methods are triggered for the app. This will
 * help tuning fork keep track of exactly when the app is on foreground
 * or background.
