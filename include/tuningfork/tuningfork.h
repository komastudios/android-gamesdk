...
typedef enum TuningFork_ErrorCode {
    ...
    TUNINGFORK_ERROR_FRAME_LOGGING_ALREADY_RUNNING =
        39,  ///< Cannot resume frame time logging because it is already
             ///< running.
    TUNINGFORK_ERROR_PREDICT_QUALITY_LEVELS_ERROR =
        40,  ///< An error occurred parsing the response to
             ///< predictQualityLevels
    TUNINGFORK_ERROR_PREDICT_QUALITY_LEVELS_RESPONSE_NOT_SUCCESS =
 ...
 * @param ser A protocol buffer array
 */
 ...
 * @return TUNINGFORK_ERROR_PREDICT_QUALITY_LEVELS_RESPONSE_NOT_SUCCESS if the call to the server failed.
 */
