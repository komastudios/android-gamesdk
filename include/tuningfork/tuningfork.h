 ...
 * fidelity parmeters and another array of predicted frame times in milliseconds.
 * The arrays are 1:1 mapped per index, so protobufs[i] corresponds to
 * predicted_time_ms[i]. There are no guarantees on how the returned data is
 ...
 *
 * The call when successful fills the \p qualityLevels struct. If \p qualityLevels
 * struct memory is owned by the caller, TuningFork_QualityLevelPredictions_free must
 * be called before freeing it.
 * @see TuningFork_QualityLevelPredictions for how the data in struct is arranged.
 *
 * @param[out] qlp          Quality levels prediction struct. Cannot be null.
 * @param      timeout_ms   time to wait before returning from this call when no
