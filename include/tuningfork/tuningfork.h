 ...
 * @see TuningFork_CProtobufSerialization_free for how to deallocate the memory once finished with the buffer.
 */
 ...
 */
typedef TuningFork_ErrorCode (* TuningFork_CacheGet )(uint64_t key, TuningFork_CProtobufSerialization* value,
                                     void* user_data);
 ...
 * @see TuningFork_Cache
 */
typedef TuningFork_ErrorCode (* TuningFork_CacheSet )(uint64_t key, const TuningFork_CProtobufSerialization* value,
                                     void* user_data);
 ...
 * @see TuningFork_Cache
 */
typedef TuningFork_ErrorCode (* TuningFork_CacheRemove )(uint64_t key, void* user_data);
...
/**
 * @brief Pointer to a function that can be attached to TuningFork_Settings::fidelity_params_callback
 *
 * Function that will be called with the fidelity parameters that are downloaded.
 * @see TuningFork_Settings
 */
typedef void (* TuningFork_FidelityParamsCallback )(const TuningFork_CProtobufSerialization*);
 ...
 * @see TuningFork_Settings
 */
typedef void (* TuningFork_UploadCallback )(const char*, size_t n);

struct SwappyTracer;

/**
 * @brief Pointer to Swappy_injectTracers that can be attached to TuningFork_Settings::swappy_tracer_fn.
 * @see TuningFork_Settings
 */
 ...
 */
inline void TuningFork_CProtobufSerialization_free(TuningFork_CProtobufSerialization* ser) {
    if (ser->dealloc) {
        ser->dealloc(ser);
        ser->dealloc = NULL;
    }
...
TuningFork_ErrorCode TuningFork_init_internal(const TuningFork_Settings *settings, JNIEnv* env, jobject context);
 ...
 */
static inline TuningFork_ErrorCode TuningFork_init(const TuningFork_Settings *settings, JNIEnv* env, jobject context) {
