 ...
 * other functions.
 *
 * This version of the init function will read the given params instead using
 * the library provided default params.
 *
 * @param env a JNIEnv
 * @param context the app context
 ...
 *
 * @param state an empty pointer for a MemoryAdvice_MemoryState, to which the
 ...
 *
 * @param advice an empty pointer for a const char*, to which the advice will be
 * written
 *
 * @return MEMORYADVICE_ERROR_OK if successful,
 * @return MEMORYADVICE_ERROR_NOT_INITIALIZED if Memory Advice was not yet
 ...
 * polled
 * @param callback the callback function that'll be invoked if memory goes
 * critical
 *
 * @return MEMORYADVICE_ERROR_OK if successful,
 * @return MEMORYADVICE_ERROR_NOT_INITIALIZED if Memory Advice was not yet
