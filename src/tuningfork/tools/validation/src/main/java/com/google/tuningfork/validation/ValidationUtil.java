...
final class ValidationUtil {
  ...
  public static void validateSettingsHistograms(Settings settings, ErrorCollector errors) {
    ...
    for (Settings.Histogram histogram: histograms) {
      ...
      boolean bucketMinCovers60fps = histogram.getBucketMin() < COVERS60FPS;
      boolean bucketMaxCovers30fps = histogram.getBucketMax() > COVERS30FPS;
      boolean bucketMinFitsBetween = histogram.getBucketMin() > COVERS60FPS && histogram.getBucketMin() < COVERS30FPS;
      boolean bucketMaxFitsBetween = histogram.getBucketMax() > COVERS60FPS && histogram.getBucketMax() < COVERS30FPS;
