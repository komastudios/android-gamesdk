...
/**
 * Histogram widget example that does not animate.
 *
 */
...
//            bar.text = data[i].name
            ...
            currentBars.add(HistogramBarMetaData(bar.id, data[i].height, barColour, data[i].barTextColour, bar.text.toString()))
            bars[bar.id] = bar
        }
     ...
     *
     * According to current API Level, it decide weather using system API or not. So you can use [ ][.generateViewId] and [View.generateViewId] in the
