...
data class DeviceSpec(val fingerprint: String, val total_memory_bytes: Long, val build_version: String,
                 val gles_version: GlesVersion, val cpu_core_freqs_hz: LongArray)

data class GenerateTuningParametersRequest(val name: String, val device: DeviceSpec,
                                      val serialized_tuning_parameters: ByteArray?)

data class DebugInfoRequest(val dev_tuningfork_descriptor: ByteArray?,
...
data class TuningParameters(val experiment_id: String, val serialized_fidelity_parameters: ByteArray)

data class TelemetryContext(val annotations: ByteArray, val tuning_parameters: TuningParameters,
...
data class SessionContext(val device: DeviceSpec, val game_sdk_info: GameSdkInfo,
                     val time_period: TimePeriod)

data class UploadTelemetryRequest(val name: String, val session_context: SessionContext,
...
class Deserializer {
    ...
    fun getDeviceSpec(json: JSONObject, field_name: String) : DeviceSpec {
        ...
        return DeviceSpec(spec_json.getString("fingerprint"),
    ...
    fun getTuningParameters(json: JSONObject, field_name: String) : TuningParameters {
        ...
        val serialized_fidelity_parameters = getByteArray(tuning_json,
            "serialized_fidelity_parameters")
        return TuningParameters(experiment_id, serialized_fidelity_parameters)
    }
//    fun getProtoDuration(json: JSONObject, field_name: String) : Duration {
//        val dur_str = json.getString(field_name)
//        if (dur_str.endsWith('s')) {
//            val secs = dur_str.substring(0,dur_str.length-1).toLong()
...
//    }
    ...
    fun getTimePeriod(json: JSONObject, field_name: String) : TimePeriod {
        ...
        // TODO(willosborn): replace this so we can go lower that API 26
