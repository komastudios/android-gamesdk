...
public class PluginLayout extends JPanel {
  ...
  public PluginLayout(String projectPath, ProtoCompiler protoCompiler)
  ...
  private void initComponents() throws IOException, CompilationException {
    ...
    annotationsLayout = new AnnotationTab(new AnnotationTabController(projectPath), protoCompiler);
    annotationsLayout.setSize(new Dimension(5 * SCREEN_SIZE.width / 6, SCREEN_SIZE.height / 2));
    annotationsLayout.setVisible(true);
    fidelitySettingsLayout = new FidelityTab(new FidelityTabController(projectPath), protoCompiler);
