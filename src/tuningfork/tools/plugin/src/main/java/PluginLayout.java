  ...
  private void initMenuTree() {
    ...
    menu.addTreeSelectionListener(treeSelectionEvent -> {
      ...
      if (node.equals(annotationsNode)) {
        ...
        menu.setMinimumSize(new Dimension(screenSize.width / 6, screenSize.height / 2));
      }
    });
    menu.setMinimumSize(new Dimension(screenSize.width / 6, screenSize.height / 2));
    menu.setSelectionModel(new LeafOnlySelectionModel());
    menuPanel.setSize(new Dimension(screenSize.width / 6, screenSize.height / 2));
    menuPanel.add(menu);
  }

  private void initComponents() {
    qualitySettingsLayout = new QualityTab();
    qualitySettingsLayout.setSize(new Dimension(5 *screenSize.width / 6, screenSize.height / 2));
    qualitySettingsLayout.setVisible(false);
    annotationsLayout = new AnnotationTab();
    annotationsLayout.setSize(new Dimension(5 *screenSize.width / 6, screenSize.height / 2));
    annotationsLayout.setVisible(true);
    fidelitySettingsLayout = new FidelityTab();
    fidelitySettingsLayout.setVisible(false);
    fidelitySettingsLayout.setSize(new Dimension(5 *screenSize.width / 6, screenSize.height / 2));
