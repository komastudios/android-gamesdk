        ...
        .run(
            new Task.Backgroundable(e.getProject(), "Starting Android Performance Tuner Plugin") {
              public void run(@NotNull ProgressIndicator progressIndicator) {
                ...
                try {
                  List<String[]> annotationData = DataModelTransformer
                      .initAnnotationData(projectPath, protoCompiler);
                  List<FidelityTableData> fidelityTableData = DataModelTransformer
                      .initFidelityData(projectPath, protoCompiler);
                  List<String> enumData = DataModelTransformer
                      .initEnumData(projectPath, enumController, protoCompiler);
