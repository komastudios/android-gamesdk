...
public class AssetsWriter {
  ...
  public boolean saveDevFidelityParams(QualityDataModel qualityDataModel, int fileNumber) {
    ...
    String toWrite = AUTO_GENERATED_TEXTPROTO + qualityDataModel.toString()
      + AUTO_GENERATED_TEXTPROTO;
    byte[] binDataToWrite = toWrite.getBytes();

    try (OutputStream outputStream = new FileOutputStream(file)) {
      outputStream.write(binDataToWrite);
      return true;
    } catch (IOException e) {
      e.printStackTrace();
    }
