...
public class TuningForkMethodsGeneration {
  ...
  private String generateAnnotationMessage(MessageDataModel messageDataModel) {
    if (messageDataModel.getMessageType().equals(Type.ANNOTATION)) {
      ...
      stringBuilder.append(")");
      stringBuilder.append("\n{\n");
      stringBuilder.append("com::google::tuningfork::").append(messageName).append(" ")
      ...
      stringBuilder.append(messageName.toLowerCase()).append(");\n");
      stringBuilder.append("if (TuningFork_setCurrentAnnotation(&ser)!=TUNINGFORK_ERROR_OK) {\n");
      stringBuilder.append("// Failed to set annotation\n");
      stringBuilder.append("}\n");
      stringBuilder.append("TuningFork_CProtobufSerialization_free(&ser);\n");
      stringBuilder.append("}\n");
  ...
  private String generateFidelityMessage(MessageDataModel messageDataModel) {
    if (messageDataModel.getMessageType().equals(Type.FIDELITY)) {
      ...
      stringBuilder.append("auto ser = tuningfork::TuningFork_CProtobufSerialization_Alloc(");
      stringBuilder.append(messageName.toLowerCase()).append(");\n");
      stringBuilder.append("if (TuningFork_setFidelityParameters(&ser) !=TUNINGFORK_ERROR_OK) {\n");
      stringBuilder.append("// Failed to set fidelity\n");
      stringBuilder.append("}\n");
      stringBuilder.append("TuningFork_CProtobufSerialization_free(&ser);\n");
      stringBuilder.append("}\n");
      return stringBuilder.toString();
  ...
  private String generateCode(MessageDataModel annotationMessage,
    ...
    StringBuilder stringBuilder = new StringBuilder();
    stringBuilder.append(generateAnnotationMessage(annotationMessage));
    stringBuilder.append("\n");
    stringBuilder.append(generateFidelityMessage(fidelityMessage));
    return stringBuilder.toString();
  }
}
