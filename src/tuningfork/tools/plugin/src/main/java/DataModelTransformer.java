...
public class DataModelTransformer {
  ...
  public static Optional<MessageDataModel> transformToAnnotation(Descriptor desc) {
    ...
    for (FieldDescriptor fieldDescriptor : fieldDescriptors) {
      EnumDataModel enumData = new EnumDataModel(fieldDescriptor.getEnumType().getName(),
          fieldDescriptor.getEnumType().getValues());
      //TODO(targintaru) add enum to controller
  ...
  public static Optional<QualityDataModel> transformToQuality(DynamicMessage message) {
    ...
    for (Entry<FieldDescriptor, Object> entry : fieldsMap.entrySet()) {
      ...
      if (fieldValue instanceof EnumValueDescriptor) {
        ...
        fieldName = ((EnumValueDescriptor) fieldValue).getType().getName();
        fieldValue = fieldDescriptor.getEnumType().getValues().get(index).getName();
      }

      if (!qualityDataModel.addField(fieldName, fieldValue.toString())) {
        return Optional.empty();
      }
    }
    ...
    for (FieldDescriptor fieldDescriptor : fieldDescriptors) {
      ...
      switch (type) {
        case ENUM: {
          ...
          EnumDataModel enumData = new EnumDataModel(typeToInsert,
              fieldDescriptor.getEnumType().getValues());
          //TODO(targintaru) add enum to controller
          break;
        }
