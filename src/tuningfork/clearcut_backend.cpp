...
bool ClearcutBackend::IsGooglePlayServiceAvailable(JNIEnv* env, jobject context) {
    ...
    jclass availabilityClass =
            jniHelper.FindClass(env, "com/google/android/gms/common/GoogleApiAvailability");
...
bool ClearcutBackend::InitWithClearcut(JNIEnv* env, jobject context, bool anonymousLogging) {
    ...
    jclass loggerClass = jniHelper.FindClass(env, "com/google/android/gms/clearcut/ClearcutLogger");
    if (CheckException(env)) return false;
    jclass stringClass = jniHelper.FindClass(env, "java/lang/String");
    if (CheckException(env)) return false;
    jclass builderClass = jniHelper.FindClass(env,
