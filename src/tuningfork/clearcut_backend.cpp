...
bool ClearcutBackend::AttachThread() {
    int envStatus  = vm_->GetEnv((void**)&env_, JNI_VERSION_1_6);
    ...
    if(envStatus == JNI_EDETACHED) {
        int attachStatus = vm_->AttachCurrentThread(&env_, (void*)NULL);
        if(attachStatus!= 0) {
            __android_log_print(
                    ANDROID_LOG_WARN,
                    LOG_TAG,
...
bool ClearcutBackend::Process(const ProtobufSerialization &evt_ser) {
    if(!AttachThread()) return  false;

    //Cast to jbytearray
    jsize length = evt_ser.size();
    jbyteArray  output = env_->NewByteArray(length);
    env_->SetByteArrayRegion(output, 0, length, reinterpret_cast<const jbyte *>(evt_ser.data()));

    //Send to Clearcut
    jobject newBuilder = env_->CallObjectMethod(clearcut_logger_, new_event_, output);
    env_->CallVoidMethod(newBuilder, log_method_);
    bool hasException = CheckException(env_);
...
bool ClearcutBackend::Init(JNIEnv *env, jobject activity) {
    ...
    try {
        ...
        __android_log_print(ANDROID_LOG_INFO, LOG_TAG, "Clearcut status: %s available", inited ? "" : "not");
        return  inited;
