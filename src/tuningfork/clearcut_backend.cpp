...
bool ClearcutBackend::InitWithClearcut(JNIEnv* env, jobject context, bool anonymousLogging) {
    ...
    if (anonymousLogging) {
        localClearcutLogger = env->CallStaticObjectMethod(loggerClass, anonymousLogger, appContext,
                                                          ccName);
    } else {
        localClearcutLogger = env->NewObject(loggerClass, loggerConstructor, appContext, ccName, NULL);
    }
    if (CheckException(env)) return false;
