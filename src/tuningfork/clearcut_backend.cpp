...
bool ClearcutBackend::Process(const ProtobufSerialization &evt_ser) {
    if(!GetEnv(&env_)) {
        __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "Env value is not available");
        return false;
    }
    ...
    return env_->ExceptionCheck() == JNI_FALSE;
}
...
bool ClearcutBackend::GetEnv(JNIEnv** env) {
    jint result = vm_->AttachCurrentThread(env, (void*)NULL);
