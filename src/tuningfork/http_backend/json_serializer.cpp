...
Json::object TelemetryReportJson(const Session& session,
    ...
    for (const auto& th : session.NonEmptyHistograms<FrameTimeMetricData>()) {
    ...
    for (const auto& th : session.NonEmptyHistograms<LoadingTimeMetricData>()) {
...
Json::object MemoryTelemetryReportJson(const Session& session,
    ...
    for (const auto& th : session.NonEmptyHistograms<MemoryMetricData>()) {
...
/*static*/ void JsonSerializer::SerializeEvent(const Session& session,
    ...
    for (const auto& p : session.NonEmptyHistograms<FrameTimeMetricData>()) {
        annotations.insert(p.second->metric_.annotation_);
    }
    for (const auto& p : session.NonEmptyHistograms<LoadingTimeMetricData>()) {
        annotations.insert(p.second->metric_.annotation_);
    }
