...
Json::object TelemetryReportJson(const Session& session,
    ...
    for (const auto& th : session.NonEmptyHistograms<FrameTimeData>()) {
    ...
    for (const auto& th : session.NonEmptyHistograms<LoadingTimeData>()) {
...
Json::object MemoryTelemetryReportJson(const Session& session,
    ...
    for (const auto& th : session.NonEmptyHistograms<MemoryData>()) {
...
/*static*/ void JsonSerializer::SerializeEvent(const Session& session,
    ...
    for (const auto& p : session.NonEmptyHistograms<FrameTimeData>()) {
        annotations.insert(p.second->metric_.annotation_);
    }
    for (const auto& p : session.NonEmptyHistograms<LoadingTimeData>()) {
        annotations.insert(p.second->metric_.annotation_);
    }
...
/*static*/ TuningFork_ErrorCode JsonSerializer::DeserializeAndMerge(
    ...
    for (auto& h : hists) {
        ...
        auto p = session.GetData<FrameTimeData>(id);
