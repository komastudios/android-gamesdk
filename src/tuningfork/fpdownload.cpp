...
const char play_url_main_staging[] =
    "https://staging-performanceparameters.sandbox.googleapis.com/v1/";
const char play_url_main_preprod[] =
    "https://preprod-performanceparameters.sandbox.googleapis.com/v1/";
const char play_url_rpcname[] = ":generateTuningParameters";
const char api_key[] = "AIzaSyDJI0sxe-QHy8EnWUozaDv4T0TEIqBCC48";
...
TFErrorCode DecodeResponse(const std::string& response, std::vector<uint8_t>& fps,
    ...
    auto iexperiment_id = inner.find("experiment_id");
    if (iexperiment_id==inner.end()) {
        ALOGE("No experiment_id");
        return TFERROR_NO_FIDELITY_PARAMS;
    }
    if (!iexperiment_id->second.is_string()) {
        ALOGE("experiment_id is not a string");
        return TFERROR_NO_FIDELITY_PARAMS;
    }
    experiment_id = iexperiment_id->second.string_value();
    auto ifps = inner.find("serialized_fidelity_parameters");
    if (ifps==inner.end()) {
        ALOGE("No serialized_fidelity_parameters");
        return TFERROR_NO_FIDELITY_PARAMS;
    }
    if (!ifps->second.is_string()) {
        ALOGE("serialized_fidelity_parameters is not a string");
        return TFERROR_NO_FIDELITY_PARAMS;
    }
...
TFErrorCode ParamsLoader::GetFidelityParams(JNIEnv* env, jobject context,
                                            ...
                                            std::string& experiment_id, uint32_t timeout_ms) {
    std::stringstream url;
    url << play_url_main_preprod;
    url << GetPartialURL(info);
    url << play_url_rpcname;
