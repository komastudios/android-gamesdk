...
jobject LocalObject::CallObjectMethod(const char* name, const char* sig, ...) const {
    jmethodID mid = Env()->GetMethodID(clz, name, sig);
    va_list argptr;
    va_start(argptr, sig);
    jobject o = Env()->CallObjectMethodV(obj, mid, argptr);
    va_end(argptr);
    return o;
}
jobject LocalObject::CallStaticObjectMethod(const char* name, const char* sig, ...) const {
    jmethodID mid = Env()->GetStaticMethodID(clz, name, sig);
    va_list argptr;
    va_start(argptr, sig);
    jobject o = Env()->CallStaticObjectMethodV(clz, mid, argptr);
    va_end(argptr);
    return o;
}
...
void LocalObject::CallVoidMethod(const char* name, const char* sig, ...) const {
    ...
    va_list argptr;
    va_start(argptr, sig);
    Env()->CallVoidMethodV(obj, mid, argptr);
    va_end(argptr);
}
int LocalObject::CallIntMethod(const char* name, const char* sig, ...) const {
    ...
    va_list argptr;
    va_start(argptr, sig);
    int r = Env()->CallIntMethodV(obj, mid, argptr);
    va_end(argptr);
        return r;
}
String NewString(const std::string& s) {
...
LocalObject LocalObject::GetObjectField(const char* field_name,
    ...
    jfieldID fid = Env()->GetFieldID(clz, field_name, sig);
    if(!RawExceptionCheck()) {
        auto out = Env()->GetObjectField(obj, fid);
...
int LocalObject::GetIntField(const char* field_name) const {
    jfieldID fid = Env()->GetFieldID(clz, field_name, "I");
    if(!RawExceptionCheck())
