...
public:
    DynamicSwappy(const char* libraryName) {
        ...
        static std::vector<const char*> libNames = {
...
public:
    ...
    static void startFrame(void* userPtr, int currentFrame, long currentFrameTimeStampMs) {
    ...
    static bool GetSettingsSerialization(JNIEnv* env, jobject activity,
        ...
        AAsset* asset = AAssetManager_open(mgr, "tuningfork/tuningfork_settings.bin",
                                           AASSET_MODE_BUFFER);
        if (asset == nullptr) {
