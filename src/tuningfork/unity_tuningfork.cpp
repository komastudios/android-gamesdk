...
static void* swappy_lib = nullptr;
SwappyTracerFn swappy_tracer_fn = nullptr;

void Unity_TuningFork_swappyIsAvailable() {
    swappy_lib = dlopen("libswappy.so", RTLD_NOW);
    if(swappy_lib != nullptr) {
        ALOGW("libswappy.so is found");
        swappy_tracer_fn = reinterpret_cast<SwappyTracerFn>(dlsym(swappy_lib, "SwappyGL_injectTracer"));
        if(swappy_tracer_fn != nullptr) {
            ALOGW("swappy_tracer_fn is found");

        } else {
            ALOGW("swappy_tracer_fn is not found");
        }
    } else {
        const char* error_msg = dlerror();
        ALOGW("libswappy.so is not found %s", error_msg);
    }
}
