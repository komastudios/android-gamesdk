...
namespace {
    ...
    SwappyTracerFn findTracerSwappy_0_1() {
        SwappyTracerFn tracer = findFunction<SwappyTracerFn>("libswappy.so", "Swappy_injectTracer");
        if(tracer == nullptr) return nullptr;

        SwappyIsEnabled swappyIsEnabledFunc =
    ...
    SwappyTracerFn findTracerSwappy_0_2() {
        // TODO Add Vulkan tracer and check if swappy is enabled
        return findFunction<SwappyTracerFn>("libswappy.so", "SwappyGL_injectTracer");
    }
...
bool Unity_TuningFork_swappyIsAvailable() {
    ...
    if(swappy_tracer_fn == nullptr) {
        swappy_tracer_fn = findTracerSwappy_0_1();
    }
    if(swappy_tracer_fn == nullptr) {
        swappy_tracer_fn = findTracerSwappy_0_2();
    }
