...
void MemoryTelemetry::SetUpAsyncWork(AsyncTelemetry &async,
    ...
    async.AddMetric(std::make_shared<DebugNativeHeapOp>(mem_info_provider));
    async.AddMetric(std::make_shared<OomScoreOp>(mem_info_provider));
    async.AddMetric(std::make_shared<MemInfoOp>(mem_info_provider));
}

void MemoryMetricOp::DoWork(Session *session) {
    if (mem_info_provider_ != nullptr && mem_info_provider_->GetEnabled()) {
        uint64_t value = Measure();
        auto d = session->GetData<MemoryData>(metric_id_);
        d->Record(value);
    }
}
...
#define UPDATE_OPT_HIST(label, metric)                                         \
    if (mem_info_provider_->IsMemInfo##metric##Available()) {                  \
        auto d =                                                               \
            session->GetData<MemoryData>(kMemoryMetricBase + ANDROID_##label); \
        d->Record(mem_info_provider_->GetMemInfo##metric##Bytes());            \
    }

void MemInfoOp::DoWork(Session *session) {
    if (mem_info_provider_ != nullptr && mem_info_provider_->GetEnabled()) {
