...
MemoryTelemetry::MemoryTelemetry(ITimeProvider* time_provider, IMemInfoProvider* meminfo_provider) : histograms_{
    MemoryHistogram { ANDROID_DEBUG_NATIVE_HEAP, // type
                      UPDATE_PERIOD_MS, // period_ms
                      HIST_START, // bucket_min_bytes
                      HIST_END, // bucket_max_bytes
                      Histogram<uint64_t> {HIST_START, HIST_END, NUM_BUCKETS} // histogram
    }
}, time_provider_(time_provider), meminfo_provider_(meminfo_provider)
{
...
void MemoryTelemetry::Ping(SystemTimePoint t) {
    if (meminfo_provider_ != nullptr && meminfo_provider_->IsEnabled()) {
        ...
        if (dt > std::chrono::milliseconds(UPDATE_PERIOD_MS)) {
            ...
            histograms_[0].histogram.Add(memory);
        }
        last_time_ = t;
    }
}
