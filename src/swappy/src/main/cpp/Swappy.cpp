...
void Swappy::overrideAutoSwapInterval(uint64_t swap_ns) {
    Swappy *swappy = getInstance();
    if (!swappy) {
        ALOGE("Failed to get Swappy instance in overrideAutoSwapInterval");
        return;
    }

    int newSwapInterval = std::round(float(swap_ns) / float(swappy->mRefreshPeriod.count()));

    std::lock_guard<std::mutex> lock(swappy->mFrameDurationsMutex);
    if (swappy->mAutoSwapIntervalEnabled &&
            swappy->mSwapInterval <= newSwapInterval &&
            newSwapInterval != swappy->mAutoSwapInterval) {
        swappy->mAutoSwapInterval = newSwapInterval;
        swappy->mFrameDurations.clear();
        swappy->mFrameDurationsSum = 0;
    }
}
...
void Swappy::waitForNextFrame(EGLDisplay display) {
    ...
    // while we wait for the target frame, the previous frame might have finished rendering early
...
bool Swappy::updateSwapInterval() {
    ...
    // apply hysteresis of 20%
    if (averageFrameTime > mAutoSwapInterval * 1.2) {
        mAutoSwapInterval++;
        return true;
    }

    if (mSwapInterval < mAutoSwapInterval && averageFrameTime < (mAutoSwapInterval - 1) * 0.8) {
        mAutoSwapInterval--;
        return true;
    }
