...
bool Swappy::updateSwapInterval() {
    ...
    // for a frame to cause config change, it needs to be at least FRAME_HYSTERESIS lower
    // or FRAME_HYSTERESIS higher.
    ...
    const auto [framesPerRefresh, framesPerRefreshReminder] =
            std::div((averageFrameTime.getTime(true) + FRAME_HYSTERESIS).count(),
                                                                          mRefreshPeriod.count());
    const int32_t newSwapInterval = framesPerRefresh + (framesPerRefreshReminder ? 1: 0);
    ...
    if (averageFrameTime.getTime(mPipelineMode) > upperBound) {
        swapSlower(averageFrameTime, upperBound, lowerBound, framesPerRefresh);
        configChanged = true;
    } else if (mSwapInterval < mAutoSwapInterval &&
                                            (averageFrameTime.getTime(true) < lowerBound)) {
        swapFaster(averageFrameTime, upperBound, lowerBound, framesPerRefresh);
        configChanged = true;
