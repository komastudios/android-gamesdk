...
private:
    ...
    public:
        FrameDuration(std::chrono::nanoseconds cpuTime, std::chrono::nanoseconds gpuTime) :
            ...
            if (mCpuTime > MAX_DURATION) {
                mCpuTime = MAX_DURATION;
            }

            if (mGpuTime > MAX_DURATION) {
                mGpuTime = MAX_DURATION;
            }
        }

        std::chrono::nanoseconds getCpuTime() { return mCpuTime; };
        std::chrono::nanoseconds getGpuTime() { return mGpuTime; };
        std::chrono::nanoseconds getTime(bool pipeline) {
            if (pipeline) {
        ...
        friend FrameDuration operator/(FrameDuration lhs, const int& rhs) {
            lhs.mCpuTime /= rhs;
            lhs.mGpuTime /= rhs;
            return lhs;
        }
    private:
        std::chrono::nanoseconds mCpuTime;
        std::chrono::nanoseconds mGpuTime;
    ...
    bool mPipelineModeAutoMode = true;
    ...
    std::unique_ptr<FrameStatistics> mFramestats;
