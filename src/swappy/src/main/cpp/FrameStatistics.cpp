...
void FrameStatistics::updateLatencyFrames(swappy::EGL::FrameTimestamps &frameStats,
    ...
    int64_t deltaTimeNano = frameStats.presented - frameStartTime.time_since_epoch().count();
    // add 1ms to smooth the results
    deltaTimeNano += 1000000;

    int64_t numLatencyFrames = deltaTimeNano / mRefreshPeriod.count();

    if (numLatencyFrames >= MAX_FRAME_BUCKETS)
        numLatencyFrames = MAX_FRAME_BUCKETS - 1;

    mStats.latencyFrames[numLatencyFrames]++;
}
...
void FrameStatistics::updateOffsetFromPrevFrame(swappy::EGL::FrameTimestamps &frameStats) {
    if (mPrevFrameTime != 0) {
        int64_t deltaTimeNano = frameStats.presented - mPrevFrameTime;
        // add 1ms to smooth the results
        deltaTimeNano += 1000000;
        ALOGE("frame offset: %.2f", deltaTimeNano / 1e6f);
...
void FrameStatistics::capture(EGLDisplay dpy, EGLSurface surface, TimePoint startFrameTime) {
    // first get the next frame id
    std::optional<EGLuint64KHR> nextFrameId = mEgl->getNextFrameId(dpy, surface);
    if (nextFrameId) {
        mPendingFrames.push_back({dpy, surface, nextFrameId.value(), startFrameTime});
    }
