...
void NDKChoreographerThread::looperThread()
{
    ...
    if (cpu.getNumberOfCpus() > 0) {
        ALOGI("Swappy found %d CPUs [%s].", cpu.getNumberOfCpus(), cpu.hardware().c_str());
        cpu_set = cpu.littleCoresMask();
    }

    ALOGI("Setting '%s' thread [%d-0x%x] affinity mask to 0x%x.", name.c_str(), tid, tid, to_mask(cpu_set));
