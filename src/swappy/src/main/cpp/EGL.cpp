...
std::unique_ptr<EGL> EGL::create(std::chrono::nanoseconds refreshPeriod) {
    ...
    if (eglGetNextFrameIdANDROID == nullptr) {
        ALOGE("Failed to load eglGetNextFrameIdANDROID");
        return nullptr;
    }
    ...
    if (eglGetFrameTimestampsANDROID == nullptr) {
        ALOGE("Failed to load eglGetFrameTimestampsANDROID");
        return nullptr;
    }
...
void EGL::resetSyncFence(EGLDisplay display) {
    ...
    // kick of the thread work to wait for the dence and measure its time
...
std::unique_ptr<EGL::FrameTimestamps> EGL::getFrameTimestamps(EGLDisplay dpy,
    ...
    // try again if we got some pendings stats
    for (auto i : values) {
        if (i == EGL_TIMESTAMP_PENDING_ANDROID) return nullptr;
    }

    std::unique_ptr<EGL::FrameTimestamps> frameTimestamps = std::make_unique<EGL::FrameTimestamps>();
...
EGL::FenceWaiter::~FenceWaiter() {
    mFenceWaiterRunning = true;
    mFenceWaiter.join();
}

void EGL::FenceWaiter::flush() {
    std::unique_lock lock(mFenceWaiterLock);
    mFenceWaiterCondition.wait(lock, [this](){ return !mFenceWaiterPending; });
}
...
nanoseconds EGL::FenceWaiter::getFencePendingTime() {
