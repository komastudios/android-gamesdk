...
void SwappyCommon::swapSlower(const FrameDuration& averageFrameTime,
    ...
    if (mPipelineMode == PipelineMode::Off &&
        ...
        if (pipelineModeNotNeeded(averageFrameTime.getTime(PipelineMode::Off), newUpperBound)) {
...
bool SwappyCommon::updateSwapInterval() {
    ...
    // For that we calculate two values:
    //     lowerBoundForAllRefresh     - The highest lower bound across all available refresh rates.
    //                                   This will be used to find a refresh rate with a shorter
    //                                   swap duration than the current one.
    //     canSwitchToLowerRefreshRate - Whether the is a lower refresh rate that can achieve the
    //                                   same swap period to optimize power consumption.
    nanoseconds lowerBoundForAllRefresh = 0ms;
    bool canSwitchToLowerRefreshRate = false;
    std::pair<std::chrono::nanoseconds, int> lowerIntervalConfig;
    std::pair<std::chrono::nanoseconds, int> sameIntervalLowerRefreshConfig;
    if (mSupportedRefreshRates) {
        for (auto i : *mSupportedRefreshRates) {
            ...
            if (lowerBoundForPeriod > lowerBoundForAllRefresh) {
                lowerBoundForAllRefresh = lowerBoundForPeriod;
                lowerIntervalConfig = i;
            }

            if (period > mRefreshPeriod && isSameDuration(period, swapIntervalForPeriod,
                    mRefreshPeriod, mAutoSwapInterval)) {
                canSwitchToLowerRefreshRate = true;
                sameIntervalLowerRefreshConfig = i;
            }
    ...
    if (mayNeedRefreshChange) {
        TRACE_INT("preferredRefreshPeriod", lowerIntervalConfig.first.count());
        setPreferredRefreshRate(lowerIntervalConfig.second);
    } else if (canSwitchToLowerRefreshRate) {
        TRACE_INT("preferredRefreshPeriod", sameIntervalLowerRefreshConfig.first.count());
        setPreferredRefreshRate(sameIntervalLowerRefreshConfig.second);
    }
