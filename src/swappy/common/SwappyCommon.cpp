...
void SwappyCommon::updateDisplayTimings() {
    ...
    int newSwapInterval = calculateSwapInterval(mNextTimingSettings.swapIntervalNS,
    ...
    const bool swapIntervalChangedBySettings = mSwapIntervalNS != mNextTimingSettings.swapIntervalNS;
...
bool SwappyCommon::updateSwapInterval() {
    ...
    if (mSupportedRefreshRates) {
        for (auto i : *mSupportedRefreshRates) {
            ...
            const int swapIntervalForPeriod =
                    calculateSwapInterval(pipelineFrameTime + FRAME_HYSTERESIS, period);
            const nanoseconds duration = period * swapIntervalForPeriod;
            if (duration < minSwapPeriod && duration >= mSwapIntervalNS) {
    ...
    if (betterRefreshFound) {
        nanoseconds lowerBoundForNewRefresh = betterRefreshConfig.first * betterRefreshSwapInterval;
        lowerBoundForNewRefresh -= 2 * FRAME_HYSTERESIS;
        ALOGV("lowerBoundForNewRefresh = %.2f", lowerBoundForNewRefresh.count() / 1e6f);
        if (pipelineFrameTime < lowerBoundForNewRefresh) {
        ...
        upperBoundForNewRefresh -= FRAME_HYSTERESIS;
