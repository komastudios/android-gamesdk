...
bool SwappyCommon::updateSwapInterval() {
    ...
    bool lowerRefreshRateExists = false;
    ...
    if (pipelineFrameTime < lowerBoundForAllRefresh || lowerRefreshRateExists) {
    ...
    if (mayNeedRefreshChange) {
        setPreferredRefreshRate(pipelineFrameTime + 2 * FRAME_HYSTERESIS);
    }
