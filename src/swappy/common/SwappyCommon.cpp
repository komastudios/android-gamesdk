...
void SwappyCommon::swapSlower(const FrameDuration& averageFrameTime,
    ...
    if (mPipelineMode == PipelineMode::Off &&
        ...
        const nanoseconds newUpperBound = mCommonSettings.refreshPeriod * mAutoSwapInterval;
...
bool SwappyCommon::updateSwapInterval() {
    ...
    nanoseconds upperBoundForThisRefresh = mCommonSettings.refreshPeriod * mAutoSwapInterval;
    nanoseconds lowerBoundForThisRefresh = mCommonSettings.refreshPeriod * (mAutoSwapInterval - 1);
    ...
    else if (mSwapIntervalNS <= mCommonSettings.refreshPeriod * (mAutoSwapInterval - 1) &&
