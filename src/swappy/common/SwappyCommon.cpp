...
constexpr float SwappyCommon::NON_PIPELINE_PERCENT;
...
bool SwappyCommon::updateSwapInterval() {
    ...
    const int missedFramesPercent = round(mMissedFrameCount * 100.f / mFrameDurationSamples);
    ...
    // To be very conservative, switch to non-pipeline if frame time * 50% fits
            ...
            nonPipelineFrameTime * (1 + NON_PIPELINE_PERCENT) < upperBoundForThisRefresh) {
