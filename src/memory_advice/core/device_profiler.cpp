...
Json::object DeviceProfiler::GetDeviceProfile() {
    Json::object profile;
...
std::string DeviceProfiler::MatchByFingerprint() {
    int best_score = -1;
    std::string best;

    for (Json::object::iterator it = lookup_table_->begin();
         it != lookup_table_->end(); ++it) {
        int mismatch_index =
            std::mismatch(fingerprint_.begin(), fingerprint_.end(),
                          it->first.begin())
                .first -
            fingerprint_.begin();
