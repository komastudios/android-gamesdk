...
public:
    SwappyVkBase(VkPhysicalDevice physicalDevice,
                 ...
                 SwappyVk                                &swappyVk,
                 void                                    *libVulkan,
                 std::shared_ptr<std::map<VkQueue, int>> pPerQueueFamiliyIndex) :
...
protected:
    ...
    SwappyVk         &mSwappyVk;
    void             *mLibVulkan;
    std::shared_ptr<std::map<VkQueue, int>> mpPerQueueFamiliyIndex = nullptr;
    bool             mInitialized;
    pthread_t mThread = 0;
...
private:
    ...
    void destroyVkSyncObjects(); // adyabr todo destroy all queues
...
VkResult SwappyVkGoogleDisplayTimingAndroid::initializeVkSyncObjects(VkQueue queue)
{
    ...
    if (mpPerQueueFamiliyIndex->find(queue) == mpPerQueueFamiliyIndex->end()) {
        ALOGE("queue %p was not obtained by SwappyVkGetDeviceQueue", queue);
...
private:
    SwappyVk() { InitVulkan(); } // Need to implement this constructor
...
void SwappyVk::GetDeviceQueue(VkDevice    device,
                                          uint32_t    queueFamilyIndex,
                                          uint32_t    queueIndex,
                                          VkQueue*    pQueue)
{
    ...
    if (!pPerQueueFamiliyIndex) {
        pPerQueueFamiliyIndex = std::make_shared<std::map<VkQueue, int>>();
    }
    (*pPerQueueFamiliyIndex)[*pQueue] = queueFamilyIndex;
}
        ...
        VkQueue*    pQueue)
{
    ATRACE_CALL();
    SwappyVk& swappy = SwappyVk::getInstance();
    swappy.GetDeviceQueue(device, queueFamilyIndex, queueIndex, pQueue);
}
