...
void createProto(androidgamesdk_deviceinfo::root& proto) {
  int cpuIndexMax = readCpuIndexMax();
  proto.set_cpu_max_index(cpuIndexMax);

  for (int cpuIndex = 0; cpuIndex <= cpuIndexMax; cpuIndex++) {
    androidgamesdk_deviceinfo::cpu_core* newCore = proto.add_cpu_core();
  ...
  androidgamesdk_deviceinfo::gl& gl = *proto.mutable_gl();
...
size_t getProtoSerialized(void* buffer, const size_t bufferSizeAllocated){
  ...
  createProto(proto);
  size_t bufferSizeNeeded = proto.ByteSize();
  if(buffer != 0 && bufferSizeAllocated >= bufferSizeNeeded){
    proto.SerializeToArray(buffer, bufferSizeNeeded);
  }
