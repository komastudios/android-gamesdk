...
constexpr int greenTextureExpectedRed = 34;
constexpr int greenTextureExpectedGreen = 177;
constexpr int greenTextureExpectedBlue = 76;
...
// Returns number of errors.
int flushGlErrors(StringVector& errors) {
  int numErrors = 0;
  char buffer[1024];
  while (GLenum e = glGetError() != GL_NO_ERROR) {
    numErrors++;
    snprintf(buffer, sizeof(buffer), "OpenGL: %#x", e);
...
// Returns number of errors.
int setupEGl(EGLDisplay& display, EGLContext& context, EGLSurface& surface, StringVector& errors) {
  ...
  EGLint pbufferAttribs[] = {EGL_WIDTH, VIEW_WIDTH, EGL_HEIGHT, VIEW_HEIGHT,
                             EGL_NONE};
...
void addOglConstsV2_0(finsky_dfe_GameSdkDeviceInfo_OpenGl& proto,
                      ProtoDataHolder::OpenGl& dataHolder) {
                      ALOGW("addOglConstsV2_0");
  ...
  for (size_t i = 0; i < dataHolder.compTexFormats.size; ++i) {
    ALOGW("Compressed texture format: %x", dataHolder.compTexFormats.data[i]);
  }
  ...
  ALOGW("addOglConstsV2_0 DONE");

}
// OpenGL constants requiring minimum version 3.0
void addOglConstsV3_0(finsky_dfe_GameSdkDeviceInfo_OpenGl& proto,
                      ProtoDataHolder::OpenGl& dataHolder) {
...
void addOglConstsV3_1(finsky_dfe_GameSdkDeviceInfo_OpenGl& proto) {
...
void addOglConstsV3_2(finsky_dfe_GameSdkDeviceInfo_OpenGl& proto,
                      ProtoDataHolder::OpenGl& dataHolder) {
...
// returns number of errors
int addGl(finsky_dfe_GameSdkDeviceInfo_OpenGl& proto,
...
bool protoEncodeCpuFreqs(pb_ostream_t* stream, const pb_field_t* field,
  ...
  for (int i = 0; i <= cpuFreqs.size; i++) {
    finsky_dfe_GameSdkDeviceInfo_CpuCore core =
        finsky_dfe_GameSdkDeviceInfo_CpuCore_init_zero;
    ...
    if (!pb_encode_submessage(
            stream, finsky_dfe_GameSdkDeviceInfo_CpuCore_fields, &core))
      return false;
  }
  return true;
}
 ...
 * specified EGLSurface, then compares that the output pixels match the expected pixels.
 * This is done to ensure that the compressed texture was properly loaded and rendered.
 */
bool testTextureRendering(StringVector& errors, EGLDisplay eglDisplay, EGLSurface eglSurface,
  GLenum internalformat, GLsizei width, GLsizei height, GLsizei imageSize, const void *data) {
  ...
  if (!basicTextureRenderer.loadProgramObject()) {
    ALOGW("Unable to load program object for OpenGL compressed texture format %#x",
      internalformat);
    return false;
  }

  if (!basicTextureRenderer.loadTexture(internalformat, width, height, imageSize, data)) {
    ALOGW("Unable to load texture for OpenGL compressed texture format %#x",
      internalformat);
    return false;
  }

  if (!basicTextureRenderer.draw(VIEW_WIDTH, VIEW_HEIGHT)) {
    ALOGW("Unable to draw texture for OpenGL compressed texture format %#x",
      internalformat);
    return false;
  }

  eglSwapBuffers(eglDisplay, eglSurface);
  flushGlErrors(errors);

  // Read the rendered pixels and make sure that texture was properly drawn
  unsigned char* pixelsData =  (unsigned char*) malloc(4 * VIEW_WIDTH * VIEW_HEIGHT);
  glReadPixels(0, 0, VIEW_WIDTH, VIEW_HEIGHT, GL_RGBA, GL_UNSIGNED_BYTE, pixelsData);
  flushGlErrors(errors);

  constexpr int renderedComponentMaxDelta = 10; // Allow some delta in the rendered colors.
  size_t i = 0;
  for(size_t y = 0; y<VIEW_HEIGHT; y++){
    for(size_t x = 0; x<VIEW_WIDTH; x++) {
        if (pixelsData[i+0] < greenTextureExpectedRed - renderedComponentMaxDelta ||
            pixelsData[i+0] > greenTextureExpectedRed + renderedComponentMaxDelta ||
            pixelsData[i+1] < greenTextureExpectedGreen - renderedComponentMaxDelta ||
            pixelsData[i+1] > greenTextureExpectedGreen + renderedComponentMaxDelta ||
            pixelsData[i+2] < greenTextureExpectedBlue - renderedComponentMaxDelta ||
            pixelsData[i+2] > greenTextureExpectedBlue + renderedComponentMaxDelta) {

          ALOGW("Bad pixel rendered for OpenGL compressed texture format %#x (received %d;%d;%d)",
            internalformat, pixelsData[i+0], pixelsData[i+1], pixelsData[i+2]);
          return false;
        }
        i+=4;
    }
...
int createProto(finsky_dfe_GameSdkDeviceInfoWithErrors& proto,
  ...
  int numErrors = 0;
                      ALOGW("createProto started");

  // Zero out everything in proto.
  proto = finsky_dfe_GameSdkDeviceInfoWithErrors_init_zero;

  proto.has_info = true;
  finsky_dfe_GameSdkDeviceInfo& info = proto.info;
  ...
  info.ro_build_fingerprint.funcs.encode = &protoEncodeString;
                      ...
                      ALOGW("setupEGl start2");
  EGLDisplay eglDisplay = nullptr;
  EGLContext eglContext = nullptr;
  EGLSurface eglSurface = nullptr;
  int numErrorsEgl = setupEGl(eglDisplay, eglContext, eglSurface, dataHolder.errors);
  numErrors += numErrorsEgl;
  if (numErrorsEgl == 0) {
    info.has_open_gl = true;
    finsky_dfe_GameSdkDeviceInfo_OpenGl& protoGl = info.open_gl;
    ...
    protoGl.gl_aliased_line_width_range.arg = &dataHolder.ogl.gl_aliased_line_width_range;
    protoGl.gl_aliased_line_width_range.funcs.encode = &protoEncodeFloatVector;

    protoGl.gl_aliased_point_size_range.arg = &dataHolder.ogl.gl_aliased_point_size_range;
    ...
    protoGl.gl_multisample_line_width_range.arg = &dataHolder.ogl.gl_multisample_line_width_range;
    protoGl.gl_multisample_line_width_range.funcs.encode = &protoEncodeFloatVector;
  } else {
    ALOGW("Error while setting up EGL");
  }

  if (numErrorsEgl == 0 && eglDisplay != nullptr && eglContext != nullptr && eglSurface != nullptr) {
    info.open_gl.has_rendered_compressed_texture_formats = true;
    auto& rendered_compressed_texture_formats = info.open_gl.rendered_compressed_texture_formats;

    // ASTC tests
    constexpr size_t astcHeaderSize = 16;
    if (dataHolder.ogl.compTexFormats.includes(GL_COMPRESSED_RGBA_ASTC_4x4_KHR)) {
      rendered_compressed_texture_formats.has_gl_compressed_rgba_astc_4x4_khr = true;
      rendered_compressed_texture_formats.gl_compressed_rgba_astc_4x4_khr =
        testTextureRendering(dataHolder.errors, eglDisplay, eglSurface,
          GL_COMPRESSED_RGBA_ASTC_4x4_KHR, greenTextureWidth, greenTextureHeight,
          green32_astc_4x4_astc_len - astcHeaderSize,
          &green32_astc_4x4_astc[astcHeaderSize]);
    }
    if (dataHolder.ogl.compTexFormats.includes(GL_COMPRESSED_RGBA_ASTC_6x6_KHR)) {
      rendered_compressed_texture_formats.has_gl_compressed_rgba_astc_6x6_khr = true;
      rendered_compressed_texture_formats.gl_compressed_rgba_astc_6x6_khr =
        testTextureRendering(dataHolder.errors, eglDisplay, eglSurface,
          ...
          &green32_astc_6x6_astc[astcHeaderSize]);
    }
    if (dataHolder.ogl.compTexFormats.includes(GL_COMPRESSED_RGBA_ASTC_8x8_KHR)) {
      rendered_compressed_texture_formats.has_gl_compressed_rgba_astc_8x8_khr = true;
      rendered_compressed_texture_formats.gl_compressed_rgba_astc_8x8_khr =
        testTextureRendering(dataHolder.errors, eglDisplay, eglSurface,
          ...
          &green32_astc_8x8_astc[astcHeaderSize]);
    }

    // PVRTC tests
    constexpr size_t pvrHeaderSize = 52; // No metadata in our PVR files
    if (dataHolder.ogl.compTexFormats.includes(GL_COMPRESSED_RGB_PVRTC_2BPPV1_IMG)) {
      rendered_compressed_texture_formats.has_gl_compressed_rgb_pvrtc_2bppv1_img = true;
      rendered_compressed_texture_formats.gl_compressed_rgb_pvrtc_2bppv1_img =
        testTextureRendering(dataHolder.errors, eglDisplay, eglSurface,
          GL_COMPRESSED_RGB_PVRTC_2BPPV1_IMG, greenTextureWidth, greenTextureHeight,
          green32_pvrtci_2bpp_RGB_pvr_len - pvrHeaderSize,
          &green32_pvrtci_2bpp_RGB_pvr[pvrHeaderSize]);
    }
    if (dataHolder.ogl.compTexFormats.includes(GL_COMPRESSED_RGBA_PVRTC_2BPPV1_IMG)) {
      rendered_compressed_texture_formats.has_gl_compressed_rgba_pvrtc_2bppv1_img = true;
      rendered_compressed_texture_formats.gl_compressed_rgba_pvrtc_2bppv1_img =
        testTextureRendering(dataHolder.errors, eglDisplay, eglSurface,
          GL_COMPRESSED_RGBA_PVRTC_2BPPV1_IMG, greenTextureWidth, greenTextureHeight,
          green32_pvrtci_2bpp_RGBA_pvr_len - pvrHeaderSize,
          &green32_pvrtci_2bpp_RGBA_pvr[pvrHeaderSize]);
    }
    if (dataHolder.ogl.compTexFormats.includes(GL_COMPRESSED_RGB_PVRTC_4BPPV1_IMG)) {
      rendered_compressed_texture_formats.has_gl_compressed_rgb_pvrtc_4bppv1_img = true;
      rendered_compressed_texture_formats.gl_compressed_rgb_pvrtc_4bppv1_img =
        testTextureRendering(dataHolder.errors, eglDisplay, eglSurface,
          GL_COMPRESSED_RGB_PVRTC_4BPPV1_IMG, greenTextureWidth, greenTextureHeight,
          green32_pvrtci_4bpp_RGB_pvr_len - pvrHeaderSize,
          &green32_pvrtci_4bpp_RGB_pvr[pvrHeaderSize]);
    }
    if (dataHolder.ogl.compTexFormats.includes(GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG)) {
      rendered_compressed_texture_formats.has_gl_compressed_rgba_pvrtc_4bppv1_img = true;
      rendered_compressed_texture_formats.gl_compressed_rgba_pvrtc_4bppv1_img =
        testTextureRendering(dataHolder.errors, eglDisplay, eglSurface,
          GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG, greenTextureWidth, greenTextureHeight,
          green32_pvrtci_4bpp_RGBA_pvr_len - pvrHeaderSize,
          &green32_pvrtci_4bpp_RGBA_pvr[pvrHeaderSize]);
    }

    // DXT tests
    constexpr size_t pvrDxtHeaderSize = 52; // No metadata in our PVR files
    if (dataHolder.ogl.compTexFormats.includes(GL_COMPRESSED_RGBA_S3TC_DXT1_EXT)) {
      rendered_compressed_texture_formats.has_gl_compressed_rgba_s3tc_dxt1_ext = true;
      rendered_compressed_texture_formats.gl_compressed_rgba_s3tc_dxt1_ext =
        testTextureRendering(dataHolder.errors, eglDisplay, eglSurface,
          GL_COMPRESSED_RGBA_S3TC_DXT1_EXT, greenTextureWidth, greenTextureHeight,
          green32_dxt1_bc1_pvr_len - pvrDxtHeaderSize,
          &green32_dxt1_bc1_pvr[pvrDxtHeaderSize]);
    }
    if (dataHolder.ogl.compTexFormats.includes(GL_COMPRESSED_RGBA_S3TC_DXT5_EXT)) {
      rendered_compressed_texture_formats.has_gl_compressed_rgba_s3tc_dxt5_ext = true;
      rendered_compressed_texture_formats.gl_compressed_rgba_s3tc_dxt5_ext =
        testTextureRendering(dataHolder.errors, eglDisplay, eglSurface,
          GL_COMPRESSED_RGBA_S3TC_DXT5_EXT, greenTextureWidth, greenTextureHeight,
          green32_dxt5_bc3_pvr_len - pvrDxtHeaderSize,
          &green32_dxt5_bc3_pvr[pvrDxtHeaderSize]);
    }

    // ETC2
    constexpr size_t pvrEtc2HeaderSize = 52; // No metadata in our PVR files
    if (dataHolder.ogl.compTexFormats.includes(GL_COMPRESSED_RGB8_ETC2)) {
      ...
      rendered_compressed_texture_formats.gl_compressed_rgb8_etc2 =
        testTextureRendering(dataHolder.errors, eglDisplay, eglSurface,
          GL_COMPRESSED_RGB8_ETC2, greenTextureWidth, greenTextureHeight,
    ...
    if (dataHolder.ogl.compTexFormats.includes(GL_ETC1_RGB8_OES)) {
      rendered_compressed_texture_formats.has_gl_etc1_rgb8_oes = true;
      rendered_compressed_texture_formats.gl_etc1_rgb8_oes =
        testTextureRendering(dataHolder.errors, eglDisplay, eglSurface,
          GL_ETC1_RGB8_OES, greenTextureWidth, greenTextureHeight,
          green32_etc1_rgb_pvr_len - pvrEtc1HeaderSize,
          &green32_etc1_rgb_pvr[pvrEtc1HeaderSize]);
    }
  } else {
    ALOGW("Skipping texture tests as EGL is not set up properly");
  }

  auto& rendered_compressed_texture_formats = info.open_gl.rendered_compressed_texture_formats;
  ALOGW("gl_compressed_rgba_astc_4x4_khr: %d", rendered_compressed_texture_formats.gl_compressed_rgba_astc_4x4_khr);
  ALOGW("gl_compressed_rgba_astc_6x6_khr: %d", rendered_compressed_texture_formats.gl_compressed_rgba_astc_6x6_khr);
  ALOGW("gl_compressed_rgba_astc_8x8_khr: %d", rendered_compressed_texture_formats.gl_compressed_rgba_astc_8x8_khr);
  ALOGW("gl_compressed_rgb_pvrtc_2bppv1_img: %d", rendered_compressed_texture_formats.gl_compressed_rgb_pvrtc_2bppv1_img);
  ALOGW("gl_compressed_rgba_pvrtc_2bppv1_img: %d", rendered_compressed_texture_formats.gl_compressed_rgba_pvrtc_2bppv1_img);
  ALOGW("gl_compressed_rgb_pvrtc_4bppv1_img: %d", rendered_compressed_texture_formats.gl_compressed_rgb_pvrtc_4bppv1_img);
  ALOGW("gl_compressed_rgba_pvrtc_4bppv1_img: %d", rendered_compressed_texture_formats.gl_compressed_rgba_pvrtc_4bppv1_img);
  ALOGW("gl_compressed_rgba_s3tc_dxt1_ext: %d", rendered_compressed_texture_formats.gl_compressed_rgba_s3tc_dxt1_ext);
  ALOGW("gl_compressed_rgba_s3tc_dxt5_ext: %d", rendered_compressed_texture_formats.gl_compressed_rgba_s3tc_dxt5_ext);
  ALOGW("gl_compressed_rgb8_etc2: %d", rendered_compressed_texture_formats.gl_compressed_rgb8_etc2);
  ALOGW("gl_etc1_rgb8_oes: %d", rendered_compressed_texture_formats.gl_etc1_rgb8_oes);

  proto.error.arg = &dataHolder.errors;
  proto.error.funcs.encode = &protoEncodeStringVector;

    ALOGW("Num errors: %d", (int)dataHolder.errors.size);
  for(int i = 0;i<dataHolder.errors.size;++i) {
    ALOGW("Error: %s", dataHolder.errors.data[i].data.get());
  }
