...
struct TestTextureRenderingConfiguration {
  ...
  int expectedPixelsColor[3];
  int allowedPixelsDelta[3];
};
...
bool testCompressedTextureRendering(StringVector& errors, EGLDisplay eglDisplay,
                                    ...
                                    TestTextureRenderingConfiguration& config) {
  ...
  unsigned char* pixelsData =
      (unsigned char*)malloc(4 * VIEW_WIDTH * VIEW_HEIGHT);
  glReadPixels(0, 0, VIEW_WIDTH, VIEW_HEIGHT, GL_RGBA, GL_UNSIGNED_BYTE,
               pixelsData);
  flushGlErrors("testCompressedTextureRendering (glReadPixels)", errors);

  size_t i = 0;
  for (size_t y = 0; y < VIEW_HEIGHT; y++) {
    for (size_t x = 0; x < VIEW_WIDTH; x++) {
      if (pixelsData[i + 0] <
              config.expectedPixelsColor[0] - config.allowedPixelsDelta[0] ||
          pixelsData[i + 0] >
              config.expectedPixelsColor[0] + config.allowedPixelsDelta[0] ||
          pixelsData[i + 1] <
              config.expectedPixelsColor[1] - config.allowedPixelsDelta[1] ||
          pixelsData[i + 1] >
              config.expectedPixelsColor[1] + config.allowedPixelsDelta[1] ||
          pixelsData[i + 2] <
              config.expectedPixelsColor[2] - config.allowedPixelsDelta[2] ||
          pixelsData[i + 2] >
              config.expectedPixelsColor[2] + config.allowedPixelsDelta[2]) {
        char renderingError[1024];
        snprintf(
            renderingError, sizeof(renderingError),
            "testCompressedTextureRendering: Bad pixel received (format: %#x, "
            "received %d;%d;%d)",
            config.internalformat, pixelsData[i + 0], pixelsData[i + 1],
            pixelsData[i + 2]);
...
void addGlCompressedTexturesRendering(
    ...
    finsky_dfe_GameSdkDeviceInfo_OpenGl& openglProto,
  ...
  if (openGlDataHolder.compTexFormats.includes(
    ...
    TestTextureRenderingConfiguration config{
        ...
        {greenTextureRed, greenTextureGreen, greenTextureBlue},
        {noDeltaAllowed, noDeltaAllowed, noDeltaAllowed}};
    ...
    rendered_compressed_texture_formats.gl_compressed_rgba_astc_4x4_khr =
        testCompressedTextureRendering(errors, eglDisplay, eglSurface, config);
  }
  if (openGlDataHolder.compTexFormats.includes(
  ...
  if (openGlDataHolder.compTexFormats.includes(
    ...
    TestTextureRenderingConfiguration config{
        ...
        {greenTextureRed, greenTextureGreen, greenTextureBlue},
        {pvrtcAllowedDelta[0], pvrtcAllowedDelta[1], pvrtcAllowedDelta[2]}};
  ...
  if (openGlDataHolder.compTexFormats.includes(
    ...
    TestTextureRenderingConfiguration config{
        ...
        {greenTextureRed, greenTextureGreen, greenTextureBlue},
        {dxtAllowedDelta[0], dxtAllowedDelta[1], dxtAllowedDelta[2]}};
