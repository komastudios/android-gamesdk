...
static void onTouchEvent(GameActivity* activity, GameActivityMotionEvent* event) {
    struct android_app* android_app = (struct android_app*)activity->instance;
    pthread_mutex_lock(&android_app->mutex);

    if (android_app->motionEvents == NULL) {
        android_app->motionEvents = (GameActivityMotionEvent**) malloc(1 * sizeof(GameActivityMotionEvent*)); 
        if (android_app->motionEvents) {
            android_app->motionEvents[0] = event;
            android_app->motionEventsCount = 1;
        }
    } else {
        GameActivityMotionEvent** newMotionEvents = (GameActivityMotionEvent**) realloc(
            android_app->motionEvents, 
            (android_app->motionEventsCount + 1) * sizeof(GameActivityMotionEvent*));
        if (newMotionEvents) {
...
static void onKeyDown(GameActivity* activity, GameActivityKeyEvent* event) {
    ...
    if (android_app->keyDownEvents == NULL) {
        android_app->keyDownEvents = (GameActivityKeyEvent**) malloc(1 * sizeof(GameActivityKeyEvent*)); 
...
static void onKeyUp(GameActivity* activity, GameActivityKeyEvent* event) {
    ...
    if (android_app->keyUpEvents == NULL) {
        android_app->keyUpEvents = (GameActivityKeyEvent**) malloc(1 * sizeof(GameActivityKeyEvent*)); 
...
static void onTextInputEvent(GameActivity *activity, const GameInputState* state) {
    struct android_app *android_app = (struct android_app *) activity->instance;
    pthread_mutex_lock(&android_app->mutex);
