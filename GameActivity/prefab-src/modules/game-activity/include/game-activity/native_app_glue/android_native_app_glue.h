...
struct android_app {
    ...
    // Pointer to an array of pointers to GameActivityMotionEvent, of size `motionEventsCount`.
    GameActivityMotionEvent** motionEvents;

    // The number of motion events in `motionEvents`.
    uint64_t motionEventsCount;

    // Pointer to an array of pointers to GameActivityKeyEvent, of size `keyUpEventsCount`.
    GameActivityKeyEvent** keyUpEvents;

    // The number of keyUp events in `keyUpEvents`.
    uint64_t keyUpEventsCount;

    // Pointer to an array of pointers to GameActivityKeyEvent, of size `keyDownEventsCount`.
...
/**
 * Clear the array of key down events that were waiting to be handled, and release
 * each of them.
 */
