...
/**
 * \brief Describe information about a pointer, found in a GameActivityMotionEvent.
 *
 ...
 *
 * The X axis and Y axis are enabled by default but any other axis that you want to
 * read **must** be enabled first, using `GameActivityPointerInfo_enableAxis`.
 *
...
inline float GameActivityPointerInfo_getX(GameActivityPointerInfo* pointerInfo) {
    return pointerInfo->axisValues[AMOTION_EVENT_AXIS_X];
}

/** \brief Get the current Y coordinate of the pointer. */
inline float GameActivityPointerInfo_getY(GameActivityPointerInfo* pointerInfo) {
 ...
 *
 * You must enable any axis that you want to read, apart from `AMOTION_EVENT_AXIS_X`
 * and `AMOTION_EVENT_AXIS_Y` that are enabled by default.
 */
 ...
 * @return The value of the axis, or 0 if the axis is invalid or was not enabled.
 */
inline float GameActivityPointerInfo_getAxisValue(GameActivityPointerInfo* pointerInfo, int32_t axis) {
 ...
 * and views.
 */
inline float GameActivityPointerInfo_getRawX(GameActivityPointerInfo* pointerInfo) {
 ...
 * and views.
 */
inline float GameActivityPointerInfo_getRawY(GameActivityPointerInfo* pointerInfo) {
...
typedef struct GameActivityCallbacks {
    ...
    /**
     * GameActivity is being destroyed.  See Java documentation for Activity.onDestroy()
     * for more information.
     */
     ...
     * Call GameActivity_getTextInputState to get the new state of the soft keyboard.
     */
 ...
 * activity.
 */
GameActivityMotionEvent* GameActivityMotionEvent_fromJava(JNIEnv *env, jobject motionEvent);
 ...
 * After this is called, any reference to the event is invalid and must be dropped.
 */
 ...
 * activity.
 */
GameActivityKeyEvent* GameActivityKeyEvent_fromJava(JNIEnv *env, jobject motionEvent);
 ...
 * this function.
 */
typedef void GameActivity_createFunc(GameActivity* activity,
 ...
 * *any* thread; it will send a message to the main thread of the process
 * where the Java call will take place.
 */
 ...
 */
void GameActivity_setTextInputState(GameActivity *activity, const GameInputState *state);

/* Get the last-received text entry state.
 * Ownership of the returned value is maintained by the GameActivity: do not delete it.
 */
