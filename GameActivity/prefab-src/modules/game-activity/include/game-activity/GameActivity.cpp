...
// TODO(florianrival@): these functions were extracted from the Game SDK
// (gamesdk/src/common/system_utils.h) and should be removed when integrated in the Game SDK.
namespace {
...
#ifndef SLOGW
#define SLOGW(...)                                                         \
  ((void)__android_log_print(ANDROID_LOG_WARN, LOG_TAG, \
                                 __VA_ARGS__))
#endif
...
/*
 * Write a command to be executed by the GameActivity on the application main thread.
 */
...
/*
 * Read commands to be executed by the GameActivity on the application main thread.
 */
...
extern "C" void GameActivity_setWindowFormat(
        GameActivity *activity, int32_t format) {
    NativeCode *code = static_cast<NativeCode *>(activity);
    write_work(code->mainWorkWrite, CMD_SET_WINDOW_FORMAT, format);
}

extern "C" void GameActivity_setWindowFlags(
        GameActivity *activity, uint32_t values, uint32_t mask) {
    NativeCode *code = static_cast<NativeCode *>(activity);
    write_work(code->mainWorkWrite, CMD_SET_WINDOW_FLAGS, values, mask);
}

extern "C" void GameActivity_showSoftInput(
        GameActivity *activity, uint32_t flags) {
    NativeCode *code = static_cast<NativeCode *>(activity);
    write_work(code->mainWorkWrite, CMD_SHOW_SOFT_INPUT, flags);
}

extern "C" void GameActivity_setTextInputState(
        GameActivity *activity, const GameInputState *state) {
    NativeCode *code = static_cast<NativeCode *>(activity);
    ...
    write_work(code->mainWorkWrite, CMD_SET_SOFT_INPUT_STATE, reinterpret_cast<int64_t>(state_copy), 0);
}
...
extern "C" void GameActivity_hideSoftInput(
        GameActivity *activity, uint32_t flags) {
    NativeCode *code = static_cast<NativeCode *>(activity);
...
static jlong
loadNativeCode_native(JNIEnv *env, jobject javaGameActivity, jstring path, jstring funcName,
    ...
    if (handle == nullptr) {
        g_error_msg = dlerror();
        ALOGE("GameActivity dlopen(\"%s\") failed: %s",
              pathStr,
              g_error_msg.c_str());
        return 0;
    }
    ...
    code = new NativeCode(handle, (GameActivity_createFunc *)
    ...
    const char *dirStr = internalDataDir ? env->GetStringUTFChars(internalDataDir, NULL) : "";
    ...
    GameInput_setEventCallback(code->gameInput, reinterpret_cast<void (*)(void *,
                                                                          const GameInputState *)>(code->callbacks.onTextInputEvent),
                               code);
...
static void
unloadNativeCode_native(JNIEnv *env, jobject javaGameActivity, jlong handle) {
...
static void
onStart_native(JNIEnv *env, jobject javaGameActivity, jlong handle) {
...
static void
onResume_native(JNIEnv *env, jobject javaGameActivity, jlong handle) {
...
static jbyteArray
onSaveInstanceState_native(JNIEnv *env, jobject javaGameActivity, jlong handle) {
...
static void
onPause_native(JNIEnv *env, jobject javaGameActivity, jlong handle) {
...
static void
onStop_native(JNIEnv *env, jobject javaGameActivity, jlong handle) {
...
static void
onConfigurationChanged_native(JNIEnv *env, jobject javaGameActivity, jlong handle) {
...
static void
onLowMemory_native(JNIEnv *env, jobject javaGameActivity, jlong handle) {
...
static void
onWindowFocusChanged_native(JNIEnv *env, jobject javaGameActivity, jlong handle, jboolean focused) {
...
static void
onSurfaceCreated_native(JNIEnv *env, jobject javaGameActivity, jlong handle, jobject surface) {
...
static void
onSurfaceChanged_native(JNIEnv *env, jobject javaGameActivity, jlong handle, jobject surface,
...
static void
onSurfaceRedrawNeeded_native(JNIEnv *env, jobject javaGameActivity, jlong handle) {
...
static void
onSurfaceDestroyed_native(JNIEnv *env, jobject javaGameActivity, jlong handle) {
...
static void
onContentRectChanged_native(JNIEnv *env, jobject javaGameActivity, jlong handle,
...
extern "C" GameActivityMotionEvent* GameActivityMotionEvent_fromJava(JNIEnv *env, jobject motionEvent) {
    ...
    if (!gMotionEventClassInfoInitialized) {
        ...
        gMotionEventClassInfo.getDeviceId = env->GetMethodID(
                motionEventClass, "getDeviceId", "()I");
        gMotionEventClassInfo.getSource = env->GetMethodID(
                motionEventClass, "getSource", "()I");
        gMotionEventClassInfo.getAction = env->GetMethodID(
                motionEventClass, "getAction", "()I");
        gMotionEventClassInfo.getEventTime = env->GetMethodID(
                motionEventClass, "getEventTime", "()J");
        gMotionEventClassInfo.getDownTime = env->GetMethodID(
                motionEventClass, "getDownTime", "()J");
        gMotionEventClassInfo.getFlags = env->GetMethodID(
                motionEventClass, "getFlags", "()I");
        gMotionEventClassInfo.getMetaState = env->GetMethodID(
                motionEventClass, "getMetaState", "()I");
        if (sdkVersion >= 23) {
            gMotionEventClassInfo.getActionButton = env->GetMethodID(
                    motionEventClass, "getActionButton", "()I");
        }
        if (sdkVersion >= 14) {
            gMotionEventClassInfo.getButtonState = env->GetMethodID(
                    motionEventClass, "getButtonState", "()I");
        }
        if (sdkVersion >= 29) {
            gMotionEventClassInfo.getClassification = env->GetMethodID(
                    motionEventClass, "getClassification", "()I");
        }
        gMotionEventClassInfo.getEdgeFlags = env->GetMethodID(
                motionEventClass, "getEdgeFlags", "()I");
        gMotionEventClassInfo.getPointerCount = env->GetMethodID(
                motionEventClass, "getPointerCount", "()I");
        gMotionEventClassInfo.getPointerId = env->GetMethodID(
                motionEventClass, "getPointerId", "(I)I");
        if (sdkVersion >= 29) {
            gMotionEventClassInfo.getRawX = env->GetMethodID(
                    motionEventClass, "getRawX", "(I)F");
            gMotionEventClassInfo.getRawY = env->GetMethodID(
                    motionEventClass, "getRawY", "(I)F");
        }
        gMotionEventClassInfo.getXPrecision = env->GetMethodID(
                motionEventClass, "getXPrecision", "()F");
        gMotionEventClassInfo.getYPrecision = env->GetMethodID(
                motionEventClass, "getYPrecision", "()F");
        gMotionEventClassInfo.getAxisValue = env->GetMethodID(
                motionEventClass, "getAxisValue", "(II)F");

        gMotionEventClassInfoInitialized = true;
    }

    uint32_t pointerCount = env->CallIntMethod(motionEvent, gMotionEventClassInfo.getPointerCount);
    ...
    for(uint32_t i = 0;i<pointerCount;++i) {
        pointers[i] = {
            ...
            /*rawX=*/ gMotionEventClassInfo.getRawX ?
                env->CallFloatMethod(motionEvent, gMotionEventClassInfo.getRawX, i) : 0,
            /*rawY=*/ gMotionEventClassInfo.getRawY ?
                env->CallFloatMethod(motionEvent, gMotionEventClassInfo.getRawY, i) : 0,
        };

        for(uint32_t axisIndex = 0;axisIndex<GAME_ACTIVITY_POINTER_INFO_AXIS_COUNT;++axisIndex) {
            if (enabledAxes[axisIndex]) {
                pointers[i].axisValues[axisIndex] =
                    env->CallFloatMethod(motionEvent, gMotionEventClassInfo.getAxisValue, axisIndex, i);
            }
        }
    }

    GameActivityMotionEvent* event = new GameActivityMotionEvent{
            /*deviceId=*/ env->CallIntMethod(motionEvent, gMotionEventClassInfo.getDeviceId),
            /*source=*/ env->CallIntMethod(motionEvent, gMotionEventClassInfo.getSource),
            /*action=*/ env->CallIntMethod(motionEvent, gMotionEventClassInfo.getAction),
            // TODO: introduce a millisecondsToNanoseconds helper:
            /*eventTime=*/env->CallLongMethod(motionEvent, gMotionEventClassInfo.getEventTime) *
                          1000000,
            /*downTime=*/env->CallLongMethod(motionEvent, gMotionEventClassInfo.getDownTime) *
                          1000000,
            /*flags=*/ env->CallIntMethod(motionEvent, gMotionEventClassInfo.getFlags),
            /*metaState=*/ env->CallIntMethod(motionEvent, gMotionEventClassInfo.getMetaState),
            /*actionButton=*/ gMotionEventClassInfo.getActionButton ?
                env->CallIntMethod(motionEvent, gMotionEventClassInfo.getActionButton) : 0,
            /*buttonState=*/ gMotionEventClassInfo.getButtonState ?
                env->CallIntMethod(motionEvent, gMotionEventClassInfo.getButtonState) : 0,
            /*classification=*/ gMotionEventClassInfo.getClassification ?
                env->CallIntMethod(motionEvent, gMotionEventClassInfo.getClassification) : 0,
            /*edgeFlags=*/ env->CallIntMethod(motionEvent, gMotionEventClassInfo.getEdgeFlags),
            pointerCount,
            pointers,
            /*precisionX=*/ env->CallFloatMethod(motionEvent, gMotionEventClassInfo.getXPrecision),
            /*precisionY=*/ env->CallFloatMethod(motionEvent, gMotionEventClassInfo.getYPrecision),
    };

    return event;
}

extern "C" void GameActivityMotionEvent_release(GameActivityMotionEvent* event) {
...
extern "C" GameActivityKeyEvent* GameActivityKeyEvent_fromJava(JNIEnv *env, jobject keyEvent) {
    ...
    if (!gKeyEventClassInfoInitialized) {
        ...
        gKeyEventClassInfo.getDeviceId = env->GetMethodID(
                keyEventClass, "getDeviceId", "()I");
        gKeyEventClassInfo.getSource = env->GetMethodID(
                keyEventClass, "getSource", "()I");
        gKeyEventClassInfo.getAction = env->GetMethodID(
                keyEventClass, "getAction", "()I");
        gKeyEventClassInfo.getEventTime = env->GetMethodID(
                keyEventClass, "getEventTime", "()J");
        gKeyEventClassInfo.getDownTime = env->GetMethodID(
                keyEventClass, "getDownTime", "()J");
        gKeyEventClassInfo.getFlags = env->GetMethodID(
                keyEventClass, "getFlags", "()I");
        gKeyEventClassInfo.getMetaState = env->GetMethodID(
                keyEventClass, "getMetaState", "()I");
        if (sdkVersion >= 13) {
            gKeyEventClassInfo.getModifiers = env->GetMethodID(
                    keyEventClass, "getModifiers", "()I");
        }
        gKeyEventClassInfo.getRepeatCount = env->GetMethodID(
                keyEventClass, "getRepeatCount", "()I");
        gKeyEventClassInfo.getKeyCode = env->GetMethodID(
                keyEventClass, "getKeyCode", "()I");
    ...
    GameActivityKeyEvent* event = new GameActivityKeyEvent{
            ...
            // TODO: introduce a millisecondsToNanoseconds helper:
            /*eventTime=*/env->CallLongMethod(keyEvent, gKeyEventClassInfo.getEventTime) *
                          1000000,
            /*downTime=*/env->CallLongMethod(keyEvent, gKeyEventClassInfo.getDownTime) *
                          1000000,
            /*flags=*/ env->CallIntMethod(keyEvent, gKeyEventClassInfo.getFlags),
            /*metaState=*/ env->CallIntMethod(keyEvent, gKeyEventClassInfo.getMetaState),
            /*modifiers=*/ gKeyEventClassInfo.getModifiers ?
                env->CallIntMethod(keyEvent, gKeyEventClassInfo.getModifiers) : 0,
            /*repeatCount=*/ env->CallIntMethod(keyEvent, gKeyEventClassInfo.getRepeatCount),
            /*keyCode=*/ env->CallIntMethod(keyEvent, gKeyEventClassInfo.getKeyCode)
    };
...
static void
onTouchEvent_native(JNIEnv *env, jobject javaGameActivity, jlong handle, jobject motionEvent) {
    if (handle == 0) return;
    NativeCode *code = (NativeCode *) handle;
    if (code->callbacks.onTouchEvent == nullptr) return;

    code->callbacks.onTouchEvent(code, GameActivityMotionEvent_fromJava(env, motionEvent));
}

static void
onKeyUp_native(JNIEnv *env, jobject javaGameActivity, jlong handle, jobject keyEvent) {
    if (handle == 0) return;
    NativeCode *code = (NativeCode *) handle;
...
static void
onKeyDown_native(JNIEnv *env, jobject javaGameActivity, jlong handle, jobject keyEvent) {
    if (handle == 0) return;
    NativeCode *code = (NativeCode *) handle;
...
static void
onTextInput_native(JNIEnv *env, jobject activity, jlong handle, jobject textInputEvent) {
    if (handle == 0) return;
    NativeCode *code = (NativeCode *) handle;
...
static void
setInputConnection_native(JNIEnv *env, jobject activity, jlong handle, jobject inputConnection) {
    NativeCode *code = (NativeCode *) handle;
    GameInput_setInputConnection(code->gameInput, inputConnection);
}

static const JNINativeMethod g_methods[] = {
        {"loadNativeCode",               "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;ILandroid/content/res/AssetManager;[B)J",
        ...
        {"onTouchEventNative",           "(JLandroid/view/MotionEvent;)V",                            (void *) onTouchEvent_native},
        {"onKeyDownNative",              "(JLandroid/view/KeyEvent;)V",                               (void *) onKeyDown_native},
        {"onKeyUpNative",                "(JLandroid/view/KeyEvent;)V",                               (void *) onKeyUp_native},
        {"onTextInputEventNative",       "(JLcom/google/androidgamesdk/gameinput/State;)V",           (void *) onTextInput_native},
        {"setInputConnectionNative",     "(JLcom/google/androidgamesdk/gameinput/InputConnection;)V", (void *) setInputConnection_native},
};
...
static int jniRegisterNativeMethods(JNIEnv *env, const char *className,
    ...
    LOG_FATAL_IF(clazz == nullptr, "Native registration unable to find class '%s'; aborting...",
                 className);
...
extern "C" int GameActivity_register(JNIEnv *env) {
    ...
    GET_METHOD_ID(gGameActivityClassInfo.finish,
                  clazz,
                  "finish", "()V");
    GET_METHOD_ID(gGameActivityClassInfo.setWindowFlags,
                  clazz,
                  "setWindowFlags", "(II)V");
    GET_METHOD_ID(gGameActivityClassInfo.setWindowFormat,
                  clazz,
                  "setWindowFormat", "(I)V");
    return jniRegisterNativeMethods(
            env, kGameActivityPathName,
            g_methods, NELEM(g_methods));
}

// Register this method so that GameActiviy_register does not need to be called manually.
extern "C" jlong
Java_com_google_androidgamesdk_GameActivity_loadNativeCode(JNIEnv *env, jobject javaGameActivity,
                                                           jstring path,
                                                           jstring funcName,
                                                           jstring internalDataDir, jstring obbDir,
                                                           jstring externalDataDir, int sdkVersion,
    ...
    jlong nativeCode = loadNativeCode_native(env, javaGameActivity, path, funcName, internalDataDir,
                                             obbDir, externalDataDir, sdkVersion, jAssetMgr,
                                             savedState);
    return nativeCode;
}
