...
float GameActivityPointerAxes_getAxisValue(const GameActivityPointerAxes* pointerInfo, int32_t axis) {
    if (axis < 0 || axis >= GAME_ACTIVITY_POINTER_INFO_AXIS_COUNT) {
        return 0;
    }
...
extern "C" void GameActivityMotionEvent_fromJava(
    ...
    int historySize = env->CallIntMethod(motionEvent, gMotionEventClassInfo.getHistorySize);
    ...
    for (int historyIndex = 0; historyIndex < historySize; historyIndex++) {
        out_event->historicalEventTimes[historyIndex] =
            env->CallLongMethod(motionEvent, gMotionEventClassInfo.getHistoricalEventTime, historyIndex);

        for (int i = 0; i < pointerCount; ++i) {
            int pointerOffset = i * GAME_ACTIVITY_POINTER_INFO_AXIS_COUNT;
            int historyAxisOffset = historyIndex * pointerCount * GAME_ACTIVITY_POINTER_INFO_AXIS_COUNT;
