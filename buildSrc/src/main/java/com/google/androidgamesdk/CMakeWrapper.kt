...
class CMakeWrapper {
    companion object {
        private fun ensureFoldersReady(project: Project, buildFolders: BuildFolders) {
            project.mkdir(buildFolders.workingFolder)
            project.mkdir(buildFolders.outputFolder)
        }

        @JvmStatic
        fun runAndroidCMake(project: Project, toolchain: Toolchain, buildOptions: BuildOptions, buildTuningFork: Boolean) {
            ...
            val toolchainFilePath = ndkPath + "/build/cmake/android.toolchain.cmake"
            ...
            if (buildOptions.stl == "gnustl_static" || buildOptions.stl == "gnustl_shared")
                cxx_flags += " -DANDROID_GNUSTL"

            val cmdLine = mutableListOf(toolchain.getCMakePath(),
                    ...
                    "-DGAMESDK_THREAD_CHECKS=" + (if (buildOptions.threadChecks) "1" else "0"),
                    "-DCMAKE_MAKE_PROGRAM=" + toolchain.getNinjaPath(),
                    "-GNinja")

            project.exec {
                val protocBinDir = toolchain.getProtobufInstallPath() + "/bin"
                environment("PATH", protocBinDir + ":" + environment.get("PATH"))
        ...
        @JvmStatic
        fun runHostCMake(project: Project, toolchain: Toolchain, buildFolders: BuildFolders, buildType: String) {
            ensureFoldersReady(project, buildFolders)

            val cmdLine = listOf(toolchain.getCMakePath(),
                    buildFolders.projectFolder,
