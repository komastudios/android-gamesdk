...
class LocalToolchain(override var project_: Project, override var androidVersion_: String) : Toolchain() {
    ...
    init {
        ...
        if(sdkPath_ == null && project_.rootProject.file("local.properties").isFile) {
            val properties = loadPropertiesFromFile(project_.rootProject.file("local.properties"))
            sdkPath_ = properties.getProperty("sdk.dir")
        }
        if(sdkPath_ == null)
            throw GradleException("Must set ANDROID_HOME or sdk.dir in local.properties")
        ...
        if (requestedNdkRevision != null) {
            val ndkSideBySidePath = joinPath(sdkPath_, "ndk", requestedNdkRevision);
        ...
        if(ndkPath_ == null && project_.rootProject.file("local.properties").isFile()) {
            val properties = loadPropertiesFromFile(project_.rootProject.file("local.properties"))
        ...
        if(ndkPath_ == null) {
            ...
            if (!project_.file(ndkPath_!!).exists())
                throw GradleException("No NDK found in SDK: must set ANDROID_NDK or ANDROID_NDK_REVISION with the version of a NDK installed in the SDK \"ndk\" folder (NDK side-by-side).")
        }
        ...
        if (requestedNdkRevision != null) {
            val requestNDKMajorVersion = extractNdkMajorVersion(requestedNdkRevision);
            if (ndkVersion_ != requestNDKMajorVersion) {
                throw GradleException("You specified NDK $requestedNdkRevision but only NDK $ndkVersion_ was found in $ndkPath_. Verify that you have the requested NDK installed with the SDK manager.")
            }
        }
    }
