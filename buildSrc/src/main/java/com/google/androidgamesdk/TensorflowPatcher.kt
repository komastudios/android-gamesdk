...
class TensorflowPatcher(val project: Project) {
    ...
    private fun patchCmakeDependencies() {
        ...
        val overridableFetchContent = project.file("../external/tensorflow/tensorflow/lite/tools/cmake/modules/OverridableFetchContent.cmake");
        val externalDependencies = project.file("src/memory_advice/ExternalDependencies.cmake");
        overridableFetchContent.writeText(externalDependencies.readText());

        val farmhashCMake = project.file("../external/tensorflow/tensorflow/lite/tools/cmake/modules/farmhash/CMakeLists.txt");
        ...
        val farmhashDownloader = project.file("../external/tensorflow/tensorflow/lite/tools/cmake/modules/farmhash.cmake");
        ...
        farmhashDownloaderContent = farmhashDownloaderContent.replace(
          ")\n" +
            ...
            "add_subdirectory(",
          ")\n" +
            "set (farmhash_SOURCE_DIR \"\${EXTERNAL_SOURCE_DIR}/libtextclassifier/native/utils/hash\")\n" +
            "set (FARMHASH_SOURCE_DIR \"\${farmhash_SOURCE_DIR}\")\n" +
            "add_subdirectory(");
        farmhashDownloader.writeText(farmhashDownloaderContent);

        val fft2dDownloader = project.file("../external/tensorflow/tensorflow/lite/tools/cmake/modules/fft2d.cmake");
        var fft2dDownloaderContent = fft2dDownloader.readText()
        fft2dDownloaderContent = fft2dDownloaderContent.replace(
          "source\")\n" +
            "add_subdirectory(",
          "source\")\n" +
            "set(fft2d_SOURCE_DIR \"\${fft2d_SOURCE_DIR}/src/fft2d/fft2d\")\n" +
            "set(FFT2D_SOURCE_DIR \"\${fft2d_SOURCE_DIR}\")\n" +
            "add_subdirectory(");
        fft2dDownloader.writeText(fft2dDownloaderContent);

        val flatbuffersCMake = project.file("../external/tensorflow/tensorflow/lite/tools/cmake/modules/flatbuffers.cmake");
    ...
    */
    private fun patchFarmhash() {
        val farmhash = project.file("../external/libtextclassifier/native/utils/hash/farmhash.cc");
