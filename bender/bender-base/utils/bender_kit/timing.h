...
struct Event {
  ...
  std::shared_ptr<Event> parent_event = nullptr;
  std::stack<std::shared_ptr<Event>> sub_events;
};
...
class EventTiming {
 public:
  ...
  void stopEvent();
  std::shared_ptr<Event> getLastMajorEvent();
  float getFramerate(int numFrames, int mostRecentFrame);

 private:
  std::vector<std::shared_ptr<Event>> major_events;
  int current_major_event_num = 0;
  std::shared_ptr<Event> current_event = nullptr;
