...
std::deque<Event>::iterator EventTiming::StartEvent(const char *name, EventType type) {
  TRACE_BEGIN_SECTION(name);
  event_pool_.emplace_back(name, ++current_event_level_, current_event_number_++, type,
                           std::chrono::high_resolution_clock::now() -
                           application_start_time_,
                           std::chrono::high_resolution_clock::duration::zero());
  event_buckets_[type].push_back(&event_pool_.back());

  if (current_event_number_ > event_limit_) {
    event_buckets_[event_pool_.front().type].pop_front();
    event_pool_.pop_front();
  }
  return event_pool_.end() - 1;
}

void EventTiming::StopEvent(std::deque<Event>::iterator event) {
  TRACE_END_SECTION();
...
void EventTiming::GetFramerate(int num_frames, int *fps, float *frame_time) {
  ...
  int most_recent_frame = (int)event_buckets_[START_FRAME].size() - 2;
