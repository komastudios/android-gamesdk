...
void EventTiming::startEvent(const char *name) {
  ...
  auto newSection = std::make_shared<Event>();
  ...
  if (current_event == nullptr) {
    ...
    current_event->sub_events.push(newSection);
...
std::shared_ptr<Event> EventTiming::getLastMajorEvent() {
...
void printEvent(std::shared_ptr<Event> event) {
  ...
  std::stack<std::shared_ptr<Event>> currentList;
  ...
  while (!currentList.empty()) {
    std::shared_ptr<Event> currEvent = currentList.top();
    currentList.pop();
