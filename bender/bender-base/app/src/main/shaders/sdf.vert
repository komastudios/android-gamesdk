...
void main() {
    ...
    uint current_char = push_consts.p.string_data[current_index / 4] >> (8 * (3 - (current_index % 4))) & 0xFF;
    vec2 position = vec2(push_consts.p.coordinates[2], push_consts.p.coordinates[3]);

    for(int i = 0; i < current_index; ++i) {
        uint char = push_consts.p.string_data[i / 4] >> (8 * (3 - (i % 4))) & 0xFF;
        position[0] += font.positions[char][1][2] * push_consts.p.coordinates[0];
    }
    ...
    position[0] += offset[0] * push_consts.p.coordinates[0];
    position[1] += offset[1] * push_consts.p.coordinates[1];
