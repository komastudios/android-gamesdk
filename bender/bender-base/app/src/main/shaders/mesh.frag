...
layout(location = 2) in vec3 ambientLightColor;
layout(location = 3) in vec3 cameraPos;
layout(location = 4) in vec3 pointLightPosition;
layout(location = 5) in vec3 pointLightColor;
layout(location = 6) in vec3 dirLightDirection;
...
void main(){
    ...
    res += calcDirectionLight(dirLightDirection, vec3(1, .839, .666));
    res += calcLightNoSpecular(-dirLightDirection, vec3(2, 1.6, 1.32));
