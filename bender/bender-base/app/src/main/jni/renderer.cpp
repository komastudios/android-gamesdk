...
void Renderer::end() {
  ...
  LOGI("current frame: %d", current_frame);
}

void Renderer::createRenderPass() {
    VkAttachmentDescription attachment_descriptions{
        .format = device_->getDisplayFormat(),
    ...
    CALL_VK(vkCreateRenderPass(device_->getDevice(), &render_pass_createInfo, nullptr,
                               &render_pass_));
}
...
VkCommandBuffer Renderer::getCommandBuffer(int i) {
  assert(i < cmd_buffer_len_ && i >= 0);
  return cmd_buffer_[i];
}
