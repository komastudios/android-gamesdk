...
void UserInterface::RegisterButton(float x_center, float x_extent, float y_center, float y_extent,
    ...
    Button button = Button(x_center, x_extent, y_center, y_extent, text);
    func(button);
    buttons_.push_back(button);
}

void UserInterface::RegisterTextField(float text_size, float x_corner, float y_corner,
        std::function<const char *()> func) {
    text_fields_.push_back({func, text_size, x_corner, y_corner});
}
...
void UserInterface::DrawUserInterface(VkRenderPass render_pass) const {
    ...
    for (TextField text_field : text_fields_) {
        font_.drawString(text_field.text_generator(),
