...
void Renderer::createPool() {
  uint32_t mvpBuffCount = device_.getDisplayImagesSize(); // multiplicative increase w/ more meshes
  uint32_t lightsBuffCount = device_.getDisplayImagesSize();

  std::array<VkDescriptorPoolSize, 2> poolSizes = {};
  poolSizes[0].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
  poolSizes[0].descriptorCount = device_.getDisplayImagesSize();
  poolSizes[1].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
  poolSizes[1].descriptorCount = mvpBuffCount + lightsBuffCount;
