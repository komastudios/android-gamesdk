...
Texture *Material::default_texture_ = nullptr;

void Material::createDefaultTexture(Renderer& renderer) {
  unsigned char imgData[4] = {255, 255, 255, 0};
  default_texture_ = new Texture(renderer.getDevice(), imgData, 1, 1, VK_FORMAT_R8G8B8A8_SRGB);
}

Material::Material(Renderer& renderer, std::shared_ptr<ShaderState> shaders, Texture *texture, const glm::vec3& color) :
    renderer_(renderer) {
  shaders_ = shaders;
  if (texture == nullptr) {
