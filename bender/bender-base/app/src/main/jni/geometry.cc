...
Geometry::Geometry(benderkit::Device &device,
  ...
  for (int x = 0; x < vertex_data.size() / 14; x++){
    ...
    if (isnan(glm::dot(glm::normalize(avgTangent), glm::normalize(avgTangent)))) {
      avgTangent = glm::vec3(1.0, 0.0, 0.0);
    }
    if (isnan(glm::dot(glm::normalize(avgBitangent), glm::normalize(avgBitangent)))) {
      avgBitangent = glm::vec3(1.0, 0.0, 0.0);
    }
