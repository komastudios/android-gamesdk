...
void createShaderState() {
  ...
  font_shaders->addVertexInputBinding(0, 4 * sizeof(float));
  font_shaders->addVertexAttributeDescription(0, 0, VK_FORMAT_R32G32_SFLOAT, 0);
  font_shaders->addVertexAttributeDescription(0, 1, VK_FORMAT_R32G32_SFLOAT, 2 * sizeof(float));
}
...
bool InitVulkan(android_app *app) {
  ...
  font = new Font(*renderer, font_shaders, androidAppCtx, FONT_SDF_PATH, FONT_INFO_PATH);
...
bool VulkanDrawFrame(Input::Data *inputData) {
  ...
  font->generateText("B3nDer V1", renderer->getCurrentCommandBuffer(), render_pass, renderer->getCurrentFrame());
