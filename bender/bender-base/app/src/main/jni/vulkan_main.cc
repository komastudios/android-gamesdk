...
UniformBufferObject<lightBlock> *lb;
...
void createUniformBuffers() {
  ...
  lb = new UniformBufferObject<lightBlock>(*device);
}
...
void updateUniformBuffer(uint32_t frameIndex) {
    ...
    lb->update(frameIndex, [](auto& lb) {
      lb.pl.position = {0.0f, 2.0f, 0.0f};
      lb.pl.color = {1.0f, 1.0f, 1.0f};
      lb.pl.intensity = 1.0f;
      lb.al.color = {1.0f, 1.0f, 1.0f};
      lb.al.intensity = 0.1f;
      lb.cameraPos = {0.0f, 0.0f, -2.0f};
    });
}
...
void createDescriptorSets() {
  ...
  for (size_t i = 0; i < device->getDisplayImagesSize(); i++) {
    ...
    lightBlockInfo.buffer = lb->getBuffer(i);
