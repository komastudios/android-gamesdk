...
void LoadDemoModels(AAsset *file) {
    ...
    packed_vertex *vertex_memory = Geometry::LockVertexBuffer(*device, total_vertex_count);
    data.read((char *)vertex_memory, sizeof(packed_vertex) * total_vertex_count);
    Geometry::UnlockVertexBuffer(*device);

    uint16_t *index_memory = Geometry::LockIndexBuffer(*device, total_index_count);
    data.read((char *)index_memory, sizeof(uint16_t) * total_index_count);
    Geometry::UnlockIndexBuffer(*device);
    render_graph_mutex.unlock();
}
