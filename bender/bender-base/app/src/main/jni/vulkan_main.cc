...
void LoadDemoModels() {
    ...
    std::vector<packed_vertex> vertex_buffer(total_vertex_count);
    std::vector<uint16_t> index_buffer(total_index_count);
    data.read((char *)vertex_buffer.data(), sizeof(packed_vertex) * total_vertex_count);
    data.read((char *)index_buffer.data(), sizeof(uint16_t) * total_index_count);
    Geometry::CreateVertexBuffer(*device, vertex_buffer, index_buffer);
    render_graph_mutex.unlock();
}
