...
void updateCamera(Input *inputData) {
  camera.rotation =
  ...
  if (inputData->doubleTapHold) {
    glm::vec3 forward = glm::normalize(camera.rotation * glm::vec3(0.0f, 0.0f, -1.0f));
    if (inputData->lastY > device->getDisplaySize().height / 2) {
      forward *= -1;
    }
...
void updateUniformBuffer(uint32_t frameIndex, Input *inputData) {
