...
class RenderGraph {
 ...
 public:
  ...
  void removeMesh() { if (meshes.size() > 0) meshes.pop_back(); }
  void clearMeshes() { meshes.clear(); }

  uint32_t getMaterialIdx() { return materials_idx; }
  uint32_t getPolyFacesIdx() { return poly_faces_idx; }
  void setPolyFacesIdx(int newIdx) { poly_faces_idx = newIdx; }
  void setMaterialsIdx(int newIdx) { materials_idx = newIdx; }
