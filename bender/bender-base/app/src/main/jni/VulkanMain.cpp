...
bool InitVulkan(android_app* app) {
  ...
  for (int bufferIndex = 0; bufferIndex < device->getSwapchainLength();
    ...
    vkCmdBindVertexBuffers(render.cmdBuffer_[bufferIndex], 0, 1, geometry->getVertexBuffer(), &offset);
    vkCmdBindIndexBuffer(render.cmdBuffer_[bufferIndex], *geometry->getIndexBuffer(), offset, VK_INDEX_TYPE_UINT16 );
    vkCmdDrawIndexed(render.cmdBuffer_[bufferIndex], static_cast<u_int32_t>(geometry->getIndexCount()), 1, 0, 0, 0);
