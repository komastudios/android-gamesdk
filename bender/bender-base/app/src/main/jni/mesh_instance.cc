...
MeshInstance::MeshInstance(const MeshInstance &other, Mesh& mesh) :
...
void MeshInstance::CreateMeshDescriptors() {
    std::vector<VkDescriptorSetLayout> layouts(renderer_->GetDevice().GetDisplayImages().size(),
                                               mesh_.GetMeshDescriptorSetLayout());
...
void MeshInstance::UpdateMVP(uint_t frame_index, glm::vec3 camera, glm::mat4 view, glm::mat4 proj) {
...
void MeshInstance::SubmitDraw(uint_t frame_index) const {
    mesh_.SubmitDraw(mesh_descriptor_sets_[frame_index], material_->GetMaterialDescriptorSet(frame_index));
}
